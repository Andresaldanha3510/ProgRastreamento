...
# SQLAlchemy
from sqlalchemy import or_, and_, func, extract, UniqueConstraint, LargeBinary, desc
...
def _get_painel_gestor_data(
    empresa_id, data_inicio, data_fim, motorista_id=None, veiculo_id=None
):
    """
    Função auxiliar que coleta, processa e consolida todos os dados para o Painel Gestor.
    É reutilizável por diferentes endpoints da API.
    """
    data_fim_datetime = datetime.combine(data_fim, datetime.max.time())

    # Query base de viagens
    query_viagens = Viagem.query.filter(
        Viagem.empresa_id == empresa_id,
        Viagem.status == "concluida",
        Viagem.data_fim >= data_inicio,
        Viagem.data_fim <= data_fim_datetime,
    ).options(
        joinedload(Viagem.motorista_formal),
        joinedload(Viagem.veiculo),
        joinedload(Viagem.abastecimentos),
        joinedload(Viagem.custo_viagem),
    )

    if motorista_id:
        query_viagens = query_viagens.filter(Viagem.motorista_id == motorista_id)
    if veiculo_id:
        query_viagens = query_viagens.filter(Viagem.veiculo_id == veiculo_id)

    viagens_periodo = query_viagens.all()

    # ============ CÁLCULOS PRINCIPAIS ============
    evolucao_temporal = defaultdict(lambda: {"receita": 0.0, "custo": 0.0})
    custo_total_geral = 0.0
    receita_total_geral = 0.0
    motoristas_stats = defaultdict(
        lambda: {"viagens": 0, "km": 0, "receita": 0, "custo": 0, "litros": 0}
    )
    veiculos_stats = defaultdict(lambda: {"km": 0, "litros": 0, "viagens": 0})
    clientes_stats = defaultdict(lambda: {"receita": 0, "custo": 0, "viagens": 0})

    for viagem in viagens_periodo:
        custo_real = getattr(viagem, "custo_real_completo", 0) or 0.0
        receita = getattr(viagem, "valor_recebido", 0) or 0.0
        distancia = getattr(viagem, "distancia_percorrida", 0) or 0.0

        custo_total_geral += custo_real
        receita_total_geral += receita

        if viagem.data_fim:
            dia = viagem.data_fim.strftime("%Y-%m-%d")
            evolucao_temporal[dia]["receita"] += receita
            evolucao_temporal[dia]["custo"] += custo_real

        if viagem.motorista_formal:
            mid = viagem.motorista_formal.nome
            motoristas_stats[mid]["viagens"] += 1
            motoristas_stats[mid]["km"] += distancia
            motoristas_stats[mid]["receita"] += receita
            motoristas_stats[mid]["custo"] += custo_real
            litros_viagem = sum(
                getattr(a, "litros", 0) or 0 for a in viagem.abastecimentos
            )
            motoristas_stats[mid]["litros"] += litros_viagem

        if viagem.veiculo:
            vid = viagem.veiculo.placa
            veiculos_stats[vid]["km"] += distancia
            veiculos_stats[vid]["viagens"] += 1
            litros_viagem = sum(
                getattr(a, "litros", 0) or 0 for a in viagem.abastecimentos
            )
            veiculos_stats[vid]["litros"] += litros_viagem

        cliente = getattr(viagem, "cliente", "Sem Cliente") or "Sem Cliente"
        clientes_stats[cliente]["receita"] += receita
        clientes_stats[cliente]["custo"] += custo_real
        clientes_stats[cliente]["viagens"] += 1

    # ============ KPIs ============
    distancia_total = sum(
        getattr(v, "distancia_percorrida", 0) or 0 for v in viagens_periodo
    )
    custo_medio_km = (
        (custo_total_geral / distancia_total) if distancia_total > 0 else 0.0
    )
    total_viagens = len(viagens_periodo)
    receita_liquida = receita_total_geral - custo_total_geral
    margem_lucro = (
        ((receita_liquida / receita_total_geral) * 100)
        if receita_total_geral > 0
        else 0
    )

    tempo_total_segundos = sum(
        getattr(v, "duracao_segundos", 0) or 0 for v in viagens_periodo
    )
    tempo_medio_horas = (
        (tempo_total_segundos / 3600 / total_viagens) if total_viagens > 0 else 0
    )

    km_medio_viagem = (distancia_total / total_viagens) if total_viagens > 0 else 0

    veiculos_manutencao = Veiculo.query.filter(
        Veiculo.empresa_id == empresa_id, Veiculo.status != "ativo"
    ).count()

    # ============ GRÁFICOS ============
    dias_ordenados = sorted(evolucao_temporal.keys())
    grafico_evolucao_temporal = {
        "labels": [
            datetime.strptime(d, "%Y-%m-%d").strftime("%d/%m") for d in dias_ordenados
        ],
        "receitas": [round(evolucao_temporal[d]["receita"], 2) for d in dias_ordenados],
        "custos": [round(evolucao_temporal[d]["custo"], 2) for d in dias_ordenados],
    }

    top_motoristas_km = sorted(
        motoristas_stats.items(), key=lambda x: x[1]["km"], reverse=True
    )[:10]
    grafico_km_motorista = {
        "labels": [m[0] for m in top_motoristas_km],
        "data": [round(m[1]["km"], 2) for m in top_motoristas_km],
    }

    eficiencia_veiculos = []
    for placa, stats in veiculos_stats.items():
        if stats["litros"] > 0:
            eficiencia = stats["km"] / stats["litros"]
            eficiencia_veiculos.append((placa, eficiencia))
    eficiencia_veiculos.sort(key=lambda x: x[1], reverse=True)

    grafico_eficiencia_veiculos = {
        "labels": [v[0] for v in eficiencia_veiculos[:10]],
        "data": [round(v[1], 2) for v in eficiencia_veiculos[:10]],
    }

    clientes_rentabilidade = [
        (cliente, stats["receita"] - stats["custo"])
        for cliente, stats in clientes_stats.items()
    ]
    clientes_rentabilidade.sort(key=lambda x: x[1], reverse=True)

    grafico_rentabilidade_cliente = {
        "labels": [c[0] for c in clientes_rentabilidade[:10]],
        "data": [round(c[1], 2) for c in clientes_rentabilidade[:10]],
    }

    ranking_motoristas = []
    for motorista, stats in sorted(
        motoristas_stats.items(), key=lambda x: x[1]["km"], reverse=True
    )[:5]:
        lucro = stats["receita"] - stats["custo"]
        ranking_motoristas.append(
            {
                "motorista": motorista,
                "viagens": stats["viagens"],
                "km": round(stats["km"], 2),
                "receita": round(stats["receita"], 2),
                "custo": round(stats["custo"], 2),
                "lucro": round(lucro, 2),
            }
        )
    ...
    # ============ BENCHMARK ============
    benchmark = {
        "custo_km": {"empresa": round(custo_medio_km, 2), "setor": 2.13},
        "eficiencia": {
            "empresa": round(
                sum(
                    v["km"] / v["litros"] if v["litros"] > 0 else 0
                    for v in veiculos_stats.values()
                )
                / max(len(veiculos_stats), 1),
                2,
            ),
            "setor": 10.4,
        },
        "margem_lucro": {"empresa": round(margem_lucro, 1), "setor": 18.2},
    }
    ...
    dados_para_ia = {
        ...
        "kpis_gerais": {
            "custo_medio_km": round(custo_medio_km, 2),
            "total_viagens": total_viagens,
            "receita_total": round(receita_total_geral, 2),
            "margem_lucro_percentual": round(margem_lucro, 1),
        },
        "top_5_motoristas_por_lucro": sorted(
            [
                {"nome": nome, "lucro": round(stats["receita"] - stats["custo"], 2)}
                for nome, stats in motoristas_stats.items()
            ],
            key=lambda x: x["lucro"],
            reverse=True,
        )[:5],
        "top_5_veiculos_por_eficiencia": [
            {"placa": v[0], "kml": round(v[1], 2)} for v in eficiencia_veiculos[:5]
        ],
        "top_5_clientes_por_rentabilidade": [
            {"nome": c[0], "rentabilidade": round(c[1], 2)}
            for c in clientes_rentabilidade[:5]
        ],
        "comparativo_benchmark": benchmark,
    }
    ...
    return {
        "kpis": {
            ...
            "km_medio_viagem": round(km_medio_viagem, 1),
        },
        "charts": {
            "evolucao_temporal": grafico_evolucao_temporal,
            "km_por_motorista": grafico_km_motorista,
            "eficiencia_veiculos": grafico_eficiencia_veiculos,
            "rentabilidade_por_cliente": grafico_rentabilidade_cliente,
            "tabela_ranking": ranking_motoristas,
        },
        "benchmark": benchmark,
        "ai_insights_data": dados_para_ia,  # Subconjunto de dados para enviar ao Gemini
    }
...
def oficina_borracharia_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if not current_user.is_authenticated or (
            current_user.role not in ["Admin", "Master", "Oficina/Borracharia"]
        ):
...
def master_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if not current_user.is_authenticated or (
            current_user.role not in ["Admin", "Master"]
        ):
...
if not GEMINI_API_KEY:
    ...
    print(
        "AVISO: GEMINI_API_KEY não encontrada no .env. A funcionalidade de IA estará desabilitada."
    )
else:
...
app.config.update(
    ...
    CLOUDFLARE_R2_PUBLIC_URL=(lambda url: url.split("=", 1)[-1])(
...
class Motorista(db.Model):
    ...
    )  # Mantido como opcional para flexibilidade
    ...
    viagens = db.relationship(
        "Viagem", foreign_keys="Viagem.motorista_id", backref="motorista_formal"
    )
...
class UnidadeNegocio(db.Model):
    ...
    lancamentos = db.relationship(
        "LancamentoFluxoCaixa", back_populates="unidade_negocio", lazy="dynamic"
    )

    __table_args__ = (
        db.UniqueConstraint("empresa_id", "nome", name="_empresa_nome_uc"),
    )
...
class ConfiguracaoBorracharia(db.Model):
    ...
    empresa_id = db.Column(
        db.Integer, db.ForeignKey("empresa.id"), nullable=False, unique=True
    )
    ...
    empresa = db.relationship(
        "Empresa", backref=db.backref("config_borracharia", uselist=False)
    )
...
class Pneu(db.Model):
    ...
    medicoes = db.relationship(
        "MedicaoSulco", backref="pneu", cascade="all, delete-orphan"
    )
    movimentacoes = db.relationship(
        "MovimentacaoPneu", backref="pneu", cascade="all, delete-orphan"
    )

    # Constraint para evitar duplicatas
    __table_args__ = (
        db.UniqueConstraint(
            "empresa_id", "numero_fogo", name="_empresa_numero_fogo_uc"
        ),
    )
    ...
    def precisa_alerta_dot(self):
        ...
        config = ConfiguracaoBorracharia.query.filter_by(
            empresa_id=self.empresa_id
        ).first()
        if not config:
            return False
    ...
    def precisa_descarte(self):
        ...
        if (
            self.sulco_atual_mm
            and self.sulco_atual_mm <= config.sulco_minimo_descarte_mm
        ):
            return True
...
class FolhaPagamento(db.Model):
    ...
    salario_base_registro = db.Column(
        db.Float, default=0.0
    )  # Salva o salário base no momento da criação
    ...
    status = db.Column(
        db.String(20), default="Em Aberto", index=True
    )  # Em Aberto, Fechada, Paga
    ...
    def recalcular_totais(self):
        ...
        proventos = (
            db.session.query(func.sum(ItemFolhaPagamento.valor))
            .filter(
...
class ItemFolhaPagamento(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    folha_pagamento_id = db.Column(
        db.Integer, db.ForeignKey("folha_pagamento.id"), nullable=False
    )

    # --- LINHA ADICIONADA AQUI ---
    viagem_id = db.Column(
        db.Integer, db.ForeignKey("viagem.id"), nullable=True, index=True
    )
...
class Manutencao(db.Model):
    ...
    empresa_id = db.Column(db.Integer, db.ForeignKey("empresa.id"), nullable=False)
    itens = db.relationship(
        "ManutencaoItem", backref="manutencao", lazy=True, cascade="all, delete-orphan"
    )
    ...
    tipo_manutencao = db.Column(
        db.String(50), nullable=False, default="Corretiva"
    ...
    veiculo_plano_associado = db.relationship(
        "VeiculoPlano", back_populates="manutencoes"
    )
...
class Licenca(db.Model):
    ...
    def __repr__(self):
        return (
            f"<Licenca {self.id} - Plano {self.plano} para Empresa {self.empresa_id}>"
        )
...
class Convite(db.Model):
    ...
    empresa = db.relationship("Empresa", backref="convites")
...
class Empresa(db.Model):
    ...
    licenca = db.relationship(
        "Licenca", backref="empresa", uselist=False, cascade="all, delete-orphan"
    )
...
class Cobranca(db.Model):
    ...
    usuario = db.relationship(
        "Usuario", backref=db.backref("cobrancas_geradas", cascade="all, delete-orphan")
    )
    ...
    def is_vencida(self):
        return (
            self.data_vencimento < datetime.utcnow().date()
...
class CentroCusto(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("empresa_id", "nome", name="_empresa_nome_cc_uc"),
    )
...
class Romaneio(db.Model):
    ...
    observacoes = db.Column(db.Text, nullable=True)
    viagem_id = db.Column(
        db.Integer, db.ForeignKey("viagem.id"), nullable=False, unique=True
    )
    ...
    itens = db.relationship(
        "ItemRomaneio", backref="romaneio", lazy=True, cascade="all, delete-orphan"
    )
...
class RateioCusto(db.Model):
    """Modelo polimórfico para rateio de despesas entre diferentes entidades (Veículo, Carreta, etc)."""

    __tablename__ = "rateio_custos"

    id = db.Column(db.Integer, primary_key=True)
    lancamento_id = db.Column(
        db.Integer, db.ForeignKey("lancamento_fluxo_caixa.id"), nullable=False
    )

    # Colunas que permitem a ligação com Veiculo OU Carreta
    rateavel_id = db.Column(
        db.Integer, nullable=False, index=True
    )  # Armazena o ID da entidade (ex: 5)
    rateavel_type = db.Column(
        db.String(50), nullable=False, index=True
    )  # Armazena o tipo (ex: 'veiculo' ou 'carreta')
    ...
    lancamento = db.relationship(
        ...
        backref=db.backref("rateios", cascade="all, delete-orphan"),
    )

    # Para acessar o objeto (Veiculo ou Carreta) dinamicamente, se necessário no futuro
    @property
    def rateavel(self):
        if self.rateavel_type == "veiculo":
            return Veiculo.query.get(self.rateavel_id)
        elif self.rateavel_type == "carreta":
            return Carreta.query.get(self.rateavel_id)
        return None
...
class Veiculo(db.Model):
    ...
    status = db.Column(db.String(50), nullable=True, default="Disponível")
    marca = db.Column(db.String(100), nullable=False)  # <<< MARCA DEVE SER OBRIGATÓRIA
    ...
    # Removemos unique=True pois vários veículos podem ter este campo vazio/nulo.
    ...
    viagens = db.relationship(
        "Viagem", backref="veiculo", lazy=True, cascade="all, delete-orphan"
    )
    ...
    planos_associados = db.relationship(
        "VeiculoPlano", back_populates="veiculo", cascade="all, delete-orphan"
    )
    ...
    def to_dict(self):
        ...
        km_display = (
            f"{int(self.km_atual)} km" if self.km_atual is not None else "Não informado"
        )
...
class PlanoDeManutencao(db.Model):
    ...
    veiculos = db.relationship(
        "VeiculoPlano", back_populates="plano", cascade="all, delete-orphan"
    )
    insumos_associados = db.relationship(
        "PlanoInsumo", back_populates="plano", cascade="all, delete-orphan"
    )
...
class VeiculoPlano(db.Model):
    ...
    plano_id = db.Column(
        db.Integer, db.ForeignKey("plano_de_manutencao.id"), primary_key=True
    )
    ...
    manutencoes = db.relationship(
        "Manutencao", back_populates="veiculo_plano_associado"
    )


class ManutencaoItem(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    manutencao_id = db.Column(
        db.Integer, db.ForeignKey("manutencao.id"), nullable=False
    )
...
class Insumo(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "descricao", "empresa_id", name="uq_insumo_descricao_empresa"
        ),
    )
...
class CustoViagem(db.Model):
    ...
    viagem = db.relationship(
        "Viagem", backref=db.backref("custo_viagem", uselist=False)
    )
...
class LancamentoFluxoCaixa(db.Model):
    ...
    unidade_negocio_id = db.Column(
        db.Integer, db.ForeignKey("unidade_negocio.id"), nullable=False, index=True
    )
    ...
    )  # Ex: Combustível, Manutenção, Frete, etc.
    valor_total = db.Column(db.Float, nullable=False)
    centro_custo_id = db.Column(
        db.Integer, db.ForeignKey("centro_custo.id"), nullable=True, index=True
    )
    ...
    status_pagamento = db.Column(
        db.String(20), default="PENDENTE", nullable=False, index=True
    )

    # Dados opcionais
    fornecedor_cliente = db.Column(
        db.String(255), nullable=True
    )  # Nome do fornecedor/cliente
    documento_numero = db.Column(
        db.String(50), nullable=True
    )  # Número da nota, boleto, etc.
    observacoes = db.Column(db.Text, nullable=True)
    meio_pagamento = db.Column(db.String(50), nullable=True)
    ...
    lancamento_pai_id = db.Column(
        db.Integer, db.ForeignKey("lancamento_fluxo_caixa.id"), nullable=True
    )

    veiculo_id = db.Column(
        db.Integer, db.ForeignKey("veiculo.id"), nullable=True, index=True
    )
    ...
    parcelas = db.relationship(
        "LancamentoFluxoCaixa", remote_side=[id], backref="lancamento_pai"
    )
    veiculo = db.relationship("Veiculo", backref="lancamentos_financeiros")

    # Anexos de comprovantes
    anexos_urls = db.Column(
    ...
    def is_vencido(self):
        ...
        return (
            self.data_vencimento < date.today() and self.status_pagamento == "PENDENTE"
        )
...
class LancamentoNotaFiscal(db.Model):
    ...
    nfe_importada_chave_acesso = db.Column(
        db.String(44), db.ForeignKey("nfe_importada.chave_acesso"), nullable=False
    )
    ...
    status_pagamento = db.Column(
        db.String(20), default="A Pagar", nullable=False, index=True
    )  # Ex: A Pagar, Pago
    data_pagamento = db.Column(db.Date, nullable=True)
    ...
    nfe_original = db.relationship(
        "NFeImportada", foreign_keys=[nfe_importada_chave_acesso]
    )
    ...
    parcela_total = db.Column(db.Integer, default=1)
    lancamento_pai_id = db.Column(
        db.Integer, db.ForeignKey("lancamento_nota_fiscal.id")
    )
...
class Viagem(db.Model):
    ...
    veiculo_id = db.Column(db.Integer, db.ForeignKey("veiculo.id"), nullable=False)
    carreta_id = db.Column(db.Integer, db.ForeignKey("carreta.id"), nullable=True)
    ...
    )  # Distância da API (pode ser usada como estimativa)
    ...
    public_tracking_token = db.Column(
    ...
    destinos = db.relationship(
        "Destino", backref="viagem", cascade="all, delete-orphan"
    )
    abastecimentos = db.relationship("Abastecimento", backref="viagem", lazy=True)
    localizacoes = db.relationship(
        "Localizacao", backref="viagem", lazy=True, cascade="all, delete-orphan"
    )
    ...
    carreta = db.relationship("Carreta", backref="viagens")
    ...
    def consumo_medio(self):
        ...
        if not distancia > 0 or not self.abastecimentos:
            return 0.0
        total_litros = sum(
            abast.litros for abast in self.abastecimentos if abast.litros is not None
        )
        if total_litros > 0:
    ...
    def custo_real_completo(self):
        ...
        if self.custo_viagem:
            custos_diretos_viagem = (
                (self.custo_viagem.pedagios or 0)
        ...
        if self.veiculo_id and self.distancia_percorrida > 0:
            ...
            custo_desgaste_veiculo = self.distancia_percorrida * (
                custo_fixo_km + custo_manutencao_km
            )
        ...
        if self.custo_motorista_variavel and self.custo_motorista_variavel > 0:
            ...
            self.motorista_formal
            and self.duracao_segundos
            and self.duracao_segundos > 0
        ):
            # Senão, calcula com base no salário fixo (para viagens normais).
            salario_base = self.motorista_formal.salario_base or 0.0
            custo_hora_motorista = (
                salario_base / 220
            )  # Custo por hora, considerando 220h/mês
...
def _get_resumo_viagens(empresa_id, **kwargs):
    """Busca resumos sobre viagens (contagens por status, etc.)."""
    query = Viagem.query.filter_by(empresa_id=empresa_id)

    # Exemplos de filtros que podem ser passados por kwargs
    if status := kwargs.get("status"):
        # Garante que o status seja minúsculo para comparação consistente
        status_lower = status.lower()
        # Mapeia possíveis variações de status para os valores do banco
        if status_lower in ["pendente", "pendentes"]:
            query = query.filter(Viagem.status == "pendente")
        elif status_lower in ["em andamento", "ativas", "rodando"]:
            query = query.filter(Viagem.status == "em_andamento")
        elif status_lower in ["concluída", "concluídas", "finalizada", "finalizadas"]:
            query = query.filter(Viagem.status == "concluida")
        # Adicione outros mapeamentos se necessário

    if motorista_id := kwargs.get("motorista_id"):
        query = query.filter(Viagem.motorista_id == motorista_id)
    if veiculo_id := kwargs.get("veiculo_id"):
        query = query.filter(Viagem.veiculo_id == veiculo_id)
    # ... outros filtros podem ser adicionados aqui (por data, cliente, etc.) ...

    # Contagens gerais baseadas nos filtros aplicados
    total_filtrado = query.count()
    pendentes = query.filter(Viagem.status == "pendente").count()
    em_andamento = query.filter(Viagem.status == "em_andamento").count()
    concluidas = query.filter(Viagem.status == "concluida").count()

    # Exemplo: Buscar a viagem mais recente que corresponde aos filtros
    viagem_mais_recente = query.order_by(Viagem.data_inicio.desc()).first()
    detalhe_recente = None
    if viagem_mais_recente:
        detalhe_recente = {
            "id": viagem_mais_recente.id,
            "cliente": viagem_mais_recente.cliente,
            "data_inicio": viagem_mais_recente.data_inicio.strftime("%d/%m/%Y"),
            "status": viagem_mais_recente.status,
        }

    return {
        "total_viagens_filtro": total_filtrado,
        "pendentes": pendentes,
        "em_andamento": em_andamento,
        "concluidas": concluidas,
        "viagem_mais_recente_nos_filtros": detalhe_recente,
        "filtros_aplicados": kwargs,  # Informa à IA quais filtros foram usados
    }


def _get_resumo_motoristas(empresa_id, **kwargs):
    """Busca resumos sobre motoristas (contagens, rankings, CNH)."""
    query = Motorista.query.filter_by(empresa_id=empresa_id)

    # Aplica filtros do kwargs
    if status := kwargs.get("status"):
        # Adapte 'situacao' para o nome real do campo e os valores para os seus status
        if status.lower() in ["ativo", "ativos", "normal", "liberado"]:
            query = query.filter(
                Motorista.situacao.ilike("%NORMAL%")
                | Motorista.situacao.ilike("%LIBERADO%")
            )  # Exemplo
        elif status.lower() in ["inativo", "inativos", "desativado"]:
            query = query.filter(Motorista.situacao.ilike("%INATIVO%"))  # Exemplo

    if nome := kwargs.get("nome"):
        query = query.filter(Motorista.nome.ilike(f"%{nome}%"))

    motoristas = query.order_by(Motorista.nome).all()
    total_filtrado = len(motoristas)

    # Contagem de ativos (ajuste a lógica conforme seus status)
    ativos = sum(
        1 for m in motoristas if m.situacao in ["NORMAL / LIBERADO", "ATIVO"]
    )  # Exemplo

    # Ranking por nome (poderia ser por KM, viagens, etc., com queries mais complexas)
    top_5_nomes = [m.nome for m in motoristas[:5]]

    # Busca por CNH
    vencimento_cnh = None
    motorista_cnh_encontrado = None
    if nome_especifico := kwargs.get("nome_cnh"):
        motorista_cnh = Motorista.query.filter(
            Motorista.empresa_id == empresa_id,
            Motorista.nome.ilike(f"%{nome_especifico}%"),
        ).first()
        if motorista_cnh:
            motorista_cnh_encontrado = motorista_cnh.nome
            if motorista_cnh.cnh_data_vencimento:
                vencimento_cnh = motorista_cnh.cnh_data_vencimento.strftime("%d/%m/%Y")
            else:
                vencimento_cnh = "Data não cadastrada"

    return {
        "total_motoristas_filtro": total_filtrado,
        "ativos_aproximado": ativos,
        "amostra_nomes": top_5_nomes,
        "motorista_pesquisado_cnh": motorista_cnh_encontrado,
        "vencimento_cnh_pesquisado": vencimento_cnh,
        "filtros_aplicados": kwargs,
    }


def _get_resumo_veiculos(empresa_id, **kwargs):
    """Busca resumos sobre veículos (contagens, status, KM, consumo)."""
    query = Veiculo.query.filter_by(empresa_id=empresa_id)

    # Aplica filtros do kwargs
    if status := kwargs.get("status"):
        query = query.filter(Veiculo.status.ilike(f"%{status}%"))
    if placa := kwargs.get("placa"):
        # Busca exata ou parcial pela placa
        query = query.filter(Veiculo.placa.ilike(f'%{placa.replace("-", "")}%'))

    veiculos = query.order_by(Veiculo.placa).all()
    total_filtrado = len(veiculos)

    # Contagens por status
    disponiveis = sum(1 for v in veiculos if v.status == "Disponível")
    em_manutencao = sum(1 for v in veiculos if v.status == "Em Manutenção")
    em_rota = sum(
        1 for v in veiculos if v.status == "Em Rota"
    )  # Ou 'em_viagem', ajuste conforme seu status

    # Busca dados de um veículo específico se a placa foi fornecida em 'placa_dados'
    veiculo_especifico_dados = None
    if placa_especifica := kwargs.get("placa_dados"):
        veiculo_obj = Veiculo.query.filter(
            Veiculo.empresa_id == empresa_id,
            Veiculo.placa.ilike(f'%{placa_especifica.replace("-", "")}%'),
        ).first()
        if veiculo_obj:
            # Reutiliza sua função existente para calcular consumo real
            consumo = calcular_consumo_medio_real(veiculo_obj.id)
            veiculo_especifico_dados = {
                "placa": veiculo_obj.placa,
                "modelo": veiculo_obj.modelo,
                "status_atual": veiculo_obj.status,
                "km_atual": veiculo_obj.km_atual,
                "consumo_medio_real_kml": (
                    round(consumo, 2) if consumo else "Não calculado (poucos dados)"
                ),
            }

    return {
        "total_veiculos_filtro": total_filtrado,
        "disponiveis": disponiveis,
        "em_manutencao": em_manutencao,
        "em_rota": em_rota,
        "dados_veiculo_especifico": veiculo_especifico_dados,
        "filtros_aplicados": kwargs,
    }


def _get_resumo_clientes(empresa_id, **kwargs):
    """Busca resumos sobre clientes (contagem, faturamento)."""
    query = Cliente.query.filter_by(empresa_id=empresa_id)

    if nome := kwargs.get("nome"):
        query = query.filter(Cliente.nome_razao_social.ilike(f"%{nome}%"))

    clientes = query.order_by(Cliente.nome_razao_social).all()
    total_filtrado = len(clientes)
    top_5_nomes = [c.nome_razao_social for c in clientes[:5]]

    # Calcula faturamento total se solicitado
    faturamento_total_calculado = None
    if kwargs.get("calcular_faturamento"):
        faturamento_total_calculado = (
            db.session.query(func.sum(Viagem.valor_recebido))
            .filter(
                Viagem.empresa_id == empresa_id,
                Viagem.status == "concluida",
                # Poderia adicionar filtro de data aqui se necessário (ex: no mês atual)
            )
            .scalar()
            or 0.0
        )
    ...
    return {
        "total_clientes_filtro": total_filtrado,
        "amostra_nomes": top_5_nomes,
        "faturamento_total_calculado_periodo": (
            round(faturamento_total_calculado, 2)
            if faturamento_total_calculado is not None
            else None
        ),
        "filtros_aplicados": kwargs,
    }
...
def _get_resumo_financeiro(empresa_id, **kwargs):
    """Busca resumos financeiros (contas a pagar/receber, vencidos, saldo)."""
    hoje = date.today()
    periodo_dias = kwargs.get("periodo_dias", 30)  # Padrão 30 dias
    data_limite_futura = hoje + timedelta(days=periodo_dias)

    # Query base para pendentes
    query_pendentes = LancamentoFluxoCaixa.query.filter(
        LancamentoFluxoCaixa.empresa_id == empresa_id,
        LancamentoFluxoCaixa.status_pagamento.in_(
            ["PENDENTE", "A Pagar"]
        ),  # Status de não pago
    )
    ...
    a_pagar_periodo = (
        query_pendentes.filter(
            LancamentoFluxoCaixa.tipo == "DESPESA",
            LancamentoFluxoCaixa.data_vencimento.between(hoje, data_limite_futura),
        )
        .with_entities(func.sum(LancamentoFluxoCaixa.valor_total))
        .scalar()
        or 0.0
    )
    ...
    a_receber_periodo = (
        query_pendentes.filter(
            LancamentoFluxoCaixa.tipo == "RECEITA",
            LancamentoFluxoCaixa.data_vencimento.between(hoje, data_limite_futura),
        )
        .with_entities(func.sum(LancamentoFluxoCaixa.valor_total))
        .scalar()
    ...
    saldo_realizado_calculado = (
        db.session.query(func.sum(LancamentoFluxoCaixa.valor_total))
        .filter(
            LancamentoFluxoCaixa.empresa_id == empresa_id,
            LancamentoFluxoCaixa.status_pagamento == "PAGO",  # Apenas pagos
            LancamentoFluxoCaixa.tipo == "RECEITA",
        )
        .scalar()
        or 0.0
    ...
    return {
        f"contas_a_pagar_proximos_{periodo_dias}_dias": round(a_pagar_periodo, 2),
        "despesas_vencidas_e_nao_pagas": round(despesas_vencidas, 2),
        f"contas_a_receber_proximos_{periodo_dias}_dias": round(a_receber_periodo, 2),
        "receitas_vencidas_e_nao_recebidas": round(receitas_vencidas, 2),
        "saldo_realizado_calculado": round(saldo_realizado_calculado, 2),
        "filtros_aplicados": kwargs,
    }
...
def api_search_rateio_destinos():
    """Busca unificada por veículos, carretas e centros de custo."""  # <-- Descrição atualizada
    ...
    veiculos = (
        Veiculo.query.filter(
            ...
            or_(Veiculo.placa.ilike(f"%{term}%"), Veiculo.modelo.ilike(f"%{term}%")),
        )
        .limit(5)
        .all()
    )
    ...
    # Busca Carretas
    carretas = (
        Carreta.query.filter(
            Carreta.empresa_id == current_user.empresa_id,
            or_(Carreta.placa.ilike(f"%{term}%"), Carreta.modelo.ilike(f"%{term}%")),
        )
        .limit(5)
        .all()
    )
    ...
    for v in veiculos:
        resultados.append(
            {
                "id": f"veiculo_{v.id}",  # Adicionado prefixo para diferenciar no frontend
    ...
    for c in carretas:
        resultados.append(
            {
                "id": f"carreta_{c.id}",  # Adicionado prefixo para diferenciar no frontend
                "nome": f"{c.placa} - {c.modelo} (Carreta)",
                "tipo": "carreta",
                "icone": "fas fa-trailer",
            }
        )
    # --- FIM DA ALTERAÇÃO ---
    ...
    for cc in centros_custo:
        resultados.append(
            {
                "id": f"centro_custo_{cc.id}",  # Adicionado prefixo para diferenciar no frontend
                "nome": cc.nome,
                "tipo": "centro_custo",
                "icone": "fas fa-folder",
            }
...
def api_marcar_pago_em_massa():
    ...
    if not all([item_ids, data_pagamento_str, meio_pagamento]):
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Dados incompletos para o pagamento em massa.",
                }
            ),
            400,
        )
    ...
    try:
        ...
        ids_manuais = [
            int(i.replace("manual_", "")) for i in item_ids if i.startswith("manual_")
        ]
        ...
        return jsonify(
            {
                "success": True,
                "message": f"{lancamentos_atualizados} lançamento(s) foram marcados como pagos.",
            }
        )
...
def api_criar_centro_custo():
    ...
    try:
        ...
        existente = CentroCusto.query.filter_by(
            empresa_id=current_user.empresa_id, nome=nome
        ).first()
        if existente:
...
def get_distancia_total_periodo(veiculo_id, dias=365):
    ...
    try:
        ...
        distancia_query = (
            db.session.query(func.sum(Viagem.odometro_final - Viagem.odometro_inicial))
            .filter(
        ...
        logger.error(
            f"Erro ao calcular distância total para veículo {veiculo_id}: {e}",
            exc_info=True,
        )
        return 0.0
...
def api_salvar_lancamento():
    try:
        ...
        nota = NFeImportada.query.filter_by(
            chave_acesso=chave_acesso, empresa_id=current_user.empresa_id
        ).first()
        if not nota:
            return (
        ...
        if nota.status == "PROCESSADA":
            return (
                jsonify({"success": False, "message": "Esta nota já foi processada."}),
                409,
            )
        ...
        if not unidade_negocio_id:
            return (
                jsonify(
                    {"success": False, "message": "A Unidade de Negócio é obrigatória."}
                ),
                400,
            )
        ...
        if not parcelas:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Os dados de parcelamento são obrigatórios.",
                    }
                ),
                400,
            )
        ...
        for i, parcela_info in enumerate(parcelas, 1):
            ...
            data_vencimento = datetime.strptime(
                parcela_info.get("data_vencimento"), "%Y-%m-%d"
            ).date()
                ...
                if not veiculos_rateio:
                    ...
                    return (
                        jsonify(
                            {
                                "success": False,
                                "message": "Para rateio, os veículos são obrigatórios.",
                            }
                        ),
                        400,
                    )
...
def unidades_negocio_page():
    ...
    unidades_objetos = (
        UnidadeNegocio.query.filter_by(empresa_id=current_user.empresa_id)
        .order_by(UnidadeNegocio.is_matriz.desc(), UnidadeNegocio.nome)
        .all()
    )
    ...
    if unidade.is_matriz:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Não é possível excluir a unidade matriz.",
                }
            ),
            400,
        )
    ...
    if LancamentoFluxoCaixa.query.filter_by(unidade_negocio_id=id).first():
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Não é possível excluir, pois existem lançamentos associados a esta unidade.",
                }
            ),
            409,
        )
...
def api_visualizar_xml(chave_acesso):
    ...
    if not nota.xml_content:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Conteúdo XML não disponível para esta nota.",
                }
            ),
            404,
        )
    ...
    try:
        ...
        logger.error(
            f"Erro ao visualizar XML da NFe {chave_acesso}: {e}", exc_info=True
        )
        return (
            jsonify({"success": False, "message": f"Erro ao processar XML: {e}"}),
            500,
        )
...
def api_excluir_rateio():
    ...
    try:
        ...
        if (
            not lancamento_clicado
            or lancamento_clicado.empresa_id != current_user.empresa_id
        ):
            return (
                jsonify({"success": False, "message": "Lançamento não encontrado."}),
                404,
            )

        # Encontra todos os lançamentos do mesmo grupo
        lancamentos_do_grupo = []
        if lancamento_clicado.documento_numero and (
            lancamento_clicado.documento_numero.startswith("NFE-")
            or lancamento_clicado.documento_numero.startswith("MANUAL-")
        ):
            ...
            if lancamento.status_pagamento == "PAGO":
                ...
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": "Não é possível excluir o rateio de um lançamento com parcelas já pagas.",
                        }
                    ),
                    400,
                )
        ...
        return jsonify(
            {
                "success": True,
                "message": "Rateio excluído com sucesso! O lançamento agora é uma despesa geral.",
            }
        )
        ...
        db.session.rollback()
        logger.error(
            f"Erro ao excluir rateio do lançamento {item_id_str}: {e}", exc_info=True
        )
        return jsonify({"success": False, "message": f"Erro interno: {str(e)}"}), 500
...
def visualizar_xml_page(chave_acesso):
    ...
    try:
        ...
        try:
            ...
            xml_formatado = "\n".join(
                [line for line in xml_formatado.split("\n") if line.strip()]
            )
        except:
            xml_formatado = nota.xml_content

        return render_template(
            "visualizar_xml.html", nota=nota, xml_content=xml_formatado
        )
...
def calcular_custo_manutencao_por_km(veiculo_id, dias=365):
    ...
    data_limite = datetime.utcnow() - timedelta(days=dias)
    custo_total_manutencoes = (
        db.session.query(func.sum(Manutencao.custo_total))
        .filter(
...
def calcular_custo_fixo_por_km(veiculo_id, dias=365):
    ...
    custo_total_fixo = (
        db.session.query(func.sum(DespesaVeiculo.valor))
        .filter(
            DespesaVeiculo.veiculo_id == veiculo_id, DespesaVeiculo.data >= data_limite
        )
        .scalar()
        or 0.0
    )
...
def lancar_frete_rapido():
    try:
        ...
        valor_por_tonelada_motorista = float(
            request.form.get("valor_por_tonelada_motorista")
        )
...
def folha_pagamento_dashboard():
    ...
    return render_template(
        "folha_pagamento_dashboard.html",
        folhas=folhas,
...
def calcular_consumo_medio_real(veiculo_id, periodo_dias=90):
    ...
    try:
        ...
        data_limite = datetime.now(UTC) - timedelta(days=periodo_dias)
        ...
        abastecimentos = (
            ...
            .order_by(Abastecimento.odometro.asc())
            .all()
        )
        ...
        for i in range(len(abastecimentos) - 1):
            ...
            distancia_trecho = (
                abastecimento_atual.odometro - abastecimento_anterior.odometro
            )
        ...
        logger.error(
            f"Erro ao calcular consumo médio para veiculo {veiculo_id}: {e}",
            exc_info=True,
        )
...
def obter_preco_medio_combustivel_recente(empresa_id, default=5.80, limit=20):
    ...
    try:
        ...
        preco_medio_query = (
            db.session.query(func.avg(Abastecimento.preco_por_litro))
            .join(Veiculo)
            .filter(Veiculo.empresa_id == empresa_id)
            .scalar()
        )
        ...
        logger.error(
            f"Erro ao obter preço médio do combustível para empresa {empresa_id}: {e}"
        )
        return default
...
def api_pneus_em_estoque():
    ...
    try:
        pneus = (
            Pneu.query.filter_by(empresa_id=current_user.empresa_id, status="Estoque")
            .order_by(Pneu.numero_fogo)
            .all()
        )
...
def calcular_consumo_medio_real(veiculo_id, periodo_dias=90):
    ...
    """
    try:
        data_limite = datetime.utcnow(UTC) - timedelta(days=periodo_dias)
...
def estimar_custo_viagem_api():
    ...
    try:
        ...
        if not all(
            ...
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Dados insuficientes para estimar o custo.",
                    }
                ),
                400,
            )
        ...
        if not veiculo or not motorista:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Veículo ou motorista não encontrado.",
                    }
                ),
                404,
            )
        ...
        preco_combustivel_para_calculo = obter_preco_medio_combustivel_recente(
        ...
        custo_desgaste_veiculo = (
            CUSTO_FIXO_POR_KM + CUSTO_MANUTENCAO_POR_KM
        ) * distancia_km

        # 4. Totalização dos custos
        custo_total_estimado = (
            custo_combustivel + custo_motorista + custo_desgaste_veiculo
        )
        ...
        return (
            jsonify({"success": False, "message": f"Ocorreu um erro interno: {e}"}),
            500,
        )
...
def gerar_folhas_mes():
    ...
    motoristas_ativos = Motorista.query.filter_by(
        empresa_id=current_user.empresa_id, situacao="NORMAL / LIBERADO"
    ).all()
    criadas = 0
    atualizadas = 0
    ...
    for motorista in motoristas_ativos:
        folha = FolhaPagamento.query.filter_by(
            motorista_id=motorista.id, mes_referencia=mes, ano_referencia=ano
        ).first()
    ...
    try:
        db.session.commit()
        flash(
            f"{criadas} folha(s) criada(s) e {atualizadas} atualizada(s) com os fretes do mês.",
            "success",
        )
    except Exception as e:
        db.session.rollback()
...
def detalhes_folha_pagamento(folha_id):
    folha = (
        FolhaPagamento.query.options(db.joinedload(FolhaPagamento.motorista))
        .filter_by(id=folha_id, empresa_id=current_user.empresa_id)
        .first_or_404()
    )
    ...
    if request.method == "POST":
        if folha.status != "Em Aberto":
            flash(
                "Esta folha de pagamento está fechada e não pode ser editada.", "error"
            )
        ...
        folha.salario_base_registro = request.form.get(
            "salario_base", type=float, default=0.0
        )
        ...
        if action == "fechar":
            ...
            folha.data_fechamento = datetime.utcnow(UTC)
            flash_msg = "Folha de pagamento fechada com sucesso!"
        elif action == "pagar":
            folha.status = "Paga"
            folha.data_pagamento = datetime.strptime(
                request.form.get("data_pagamento"), "%Y-%m-%d"
            )
...
@app.route("/oficina/ordem_servico/<int:manutencao_id>/pdf")
@login_required
@oficina_borracharia_required
def gerar_ordem_servico_pdf(manutencao_id):
    """
    Busca todos os dados de uma manutenção e renderiza um template HTML
    que será convertido em um PDF de Ordem de Serviço para exibição/impressão.
    """
    try:
        # Busca a manutenção e carrega os dados relacionados de forma otimizada
        manutencao = (
            Manutencao.query.options(
                db.joinedload(Manutencao.veiculo), db.joinedload(Manutencao.itens)
            )
            .filter(
                Manutencao.id == manutencao_id,
                Manutencao.empresa_id == current_user.empresa_id,
            )
            .first_or_404()
        )
        ...
        # Gera a URL completa para o QR Code (aponta para o dashboard do veículo)
        url_veiculo = url_for(
            "veiculo_dashboard", veiculo_id=manutencao.veiculo_id, _external=True
        )

        # Renderiza o template HTML da Ordem de Serviço com todos os dados
        html_renderizado = render_template(
            "ordem_servico_template.html",
            manutencao=manutencao,
            veiculo=manutencao.veiculo,
            empresa=empresa,
            url_veiculo_qrcode=url_veiculo,
        )

        # IMPORTANTE: Converter HTML para string se necessário
        if not isinstance(html_renderizado, str):
            html_renderizado = str(html_renderizado)

        # Converte o HTML para PDF em memória
        pdf_buffer = io.BytesIO()

        pisa_status = pisa.CreatePDF(io.StringIO(html_renderizado), dest=pdf_buffer)

        if pisa_status.err:
            erro_msg = f"Erro ao gerar PDF da OS #{manutencao_id}: {pisa_status.err}"
            logger.error(erro_msg)
            flash("Erro ao gerar o PDF. Tente novamente.", "error")
            return redirect(url_for("oficina"))

        # Verifica se o buffer tem conteúdo
        if pdf_buffer.tell() == 0 and pdf_buffer.getvalue() == b"":
            logger.error(f"PDF vazio gerado para OS #{manutencao_id}")
            flash("Falha ao gerar o PDF. O arquivo está vazio.", "error")
            return redirect(url_for("oficina"))

        pdf_buffer.seek(0)

        # Nome do arquivo com formatação
        nome_arquivo = f"OS_{manutencao_id:05d}_{manutencao.veiculo.placa}.pdf"

        logger.info(f"PDF gerado com sucesso: {nome_arquivo}")

        # Envia o PDF para o navegador PARA VISUALIZAÇÃO (inline=True, não as_attachment)
        return send_file(
            pdf_buffer,
            as_attachment=False,  # Abre no navegador em vez de fazer download
            download_name=nome_arquivo,
            mimetype="application/pdf",
        )

    except Exception as e:
        erro_msg = f"Erro crítico ao gerar Ordem de Serviço PDF para manutenção {manutencao_id}: {e}"
        logger.error(erro_msg, exc_info=True)
        flash("Ocorreu um erro inesperado ao gerar a Ordem de Serviço.", "error")
        return redirect(url_for("oficina"))


@app.route("/oficina/finalizar_mobile/<int:manutencao_id>")
@login_required
@oficina_borracharia_required
def finalizar_manutencao_mobile(manutencao_id):
    """Página mobile para o mecânico escanear e finalizar a OS."""
    manutencao = Manutencao.query.filter_by(
        id=manutencao_id,
        empresa_id=current_user.empresa_id,
        status="Em Andamento",  # Só pode finalizar o que está em andamento
    ).first_or_404()

    return render_template("finalizar_manutencao_mobile.html", manutencao=manutencao)


@app.route("/api/manutencao/finalizar_mecanico/<int:manutencao_id>", methods=["POST"])
@login_required
@oficina_borracharia_required
def api_finalizar_mecanico(manutencao_id):
    """API chamada pelo QR Code para o mecânico marcar como finalizado."""
    manutencao = Manutencao.query.get_or_404(manutencao_id)
    if manutencao.empresa_id != current_user.empresa_id:
        return jsonify({"success": False, "message": "Acesso negado."}), 403

    if manutencao.status == "Em Andamento":
        manutencao.status = "Aguardando Aprovação"
        db.session.commit()
        return jsonify(
            {
                "success": True,
                "message": "Manutenção marcada como finalizada! Aguardando aprovação do gestor.",
            }
        )
    ...
    return (
        jsonify(
            {
                "success": False,
                "message": 'Esta manutenção não está mais "Em Andamento".',
            }
        ),
        400,
    )
...
@app.route("/api/manutencao/aprovar_finalizacao/<int:manutencao_id>", methods=["POST"])
@login_required
@master_required  # Apenas Admin/Master podem aprovar
def api_aprovar_finalizacao(manutencao_id):
    """API para o gestor dar a aprovação final."""
    manutencao = Manutencao.query.get_or_404(manutencao_id)
    if manutencao.empresa_id != current_user.empresa_id:
        return jsonify({"success": False, "message": "Acesso negado."}), 403

    if manutencao.status == "Aguardando Aprovação":
        # Chama a mesma lógica do formulário de finalização
        manutencao.status = "Concluída"
        manutencao.data_saida = datetime.now(UTC)

        # Atualiza status do veículo, etc.
        veiculo = manutencao.veiculo
        ...
        if (
            ...
            and manutencao.veiculo_plano_associado
        ):
        ...
        # Baixa de estoque
        try:
            itens_utilizados = ManutencaoItem.query.filter_by(
                manutencao_id=manutencao.id
            ).all()
            for item in itens_utilizados:
                insumo = Insumo.query.filter_by(
                    descricao=item.descricao, empresa_id=current_user.empresa_id
                ).first()
                if insumo:
                    insumo.quantidade_em_estoque -= item.quantidade

            db.session.commit()
            return jsonify(
                {
                    "success": True,
                    "message": f"Manutenção do veículo {veiculo.placa} finalizada e aprovada com sucesso!",
                }
            )
        except Exception as e:
            db.session.rollback()
            return (
                jsonify(
                    {"success": False, "message": f"Erro ao dar baixa no estoque: {e}"}
                ),
                500,
            )

    return (
        jsonify({"success": False, "message": "Status inválido para aprovação."}),
        400,
    )


@app.route("/api/manutencao/reabrir/<int:manutencao_id>", methods=["POST"])
@login_required
@master_required
def api_reabrir_manutencao(manutencao_id):
    """API para reverter uma manutenção de 'Concluída' para 'Em Andamento'."""
    manutencao = Manutencao.query.get_or_404(manutencao_id)
    if manutencao.empresa_id != current_user.empresa_id:
        return jsonify({"success": False, "message": "Acesso negado."}), 403
    ...
    if manutencao.status == "Concluída":
        ...
        manutencao.veiculo.status = "Em Manutenção"
        db.session.commit()
        return jsonify(
            {"success": True, "message": f"Manutenção #{manutencao.id} foi reaberta."}
        )
    ...
    return (
        jsonify(
            {
                "success": False,
                "message": "Apenas manutenções concluídas podem ser reabertas.",
            }
        ),
        400,
    )
...
def gerar_holerite(folha_id):
    folha = FolhaPagamento.query.filter_by(
        id=folha_id, empresa_id=current_user.empresa_id
    ).first_or_404()
    proventos = folha.itens.filter_by(tipo="Provento").all()
    descontos = folha.itens.filter_by(tipo="Desconto").all()

    return render_template(
        "holerite.html", folha=folha, proventos=proventos, descontos=descontos
    )
...
class DespesaVeiculo(db.Model):
    ...
    veiculo = db.relationship(
        "Veiculo",
        backref=db.backref(
            "despesas_diversas", lazy=True, cascade="all, delete-orphan"
        ),
    )
...
class Documento(db.Model):
    ...
    )  # Essencial para os alertas
    url_anexo = db.Column(
        db.String(500), nullable=True
    )  # Link para o arquivo no Cloudflare R2
    ...
    motorista = db.relationship(
        "Motorista",
        backref=db.backref("documentos", lazy=True, cascade="all, delete-orphan"),
    )
    veiculo = db.relationship(
        "Veiculo",
        backref=db.backref("documentos", lazy=True, cascade="all, delete-orphan"),
...
def parse_nfe_xml(xml_file):
    ...
    try:
        ...
        cliente_info = {
            ...
            "inscricao_estadual": getattr(
                dest_element.find("nfe:IE", ns), "text", None
            ),
        ...
        endereco_saida_completo = (
            f"{saida_rua}, {saida_num} - {saida_bairro}, {saida_cidade} - {saida_uf}"
        )
        ...
        return {"viagem_info": viagem_info, "cliente_info": cliente_info}
...
def lancar_despesa_diversa(veiculo_id):
    ...
    veiculo = Veiculo.query.filter_by(
        id=veiculo_id, empresa_id=current_user.empresa_id
    ).first_or_404()
    ...
    try:
        ...
        logger.error(
            f"Erro ao adicionar despesa para veiculo {veiculo_id}: {e}", exc_info=True
        )
...
def painel():
    ...
    viagens_query = (
        Viagem.query.filter_by(empresa_id=current_user.empresa_id)
        .options(db.joinedload(Viagem.veiculo), db.joinedload(Viagem.motorista_formal))
        .order_by(Viagem.data_inicio.desc())
        .all()
    )

    # --- KPIs Operacionais ---
    viagens_em_andamento_kpi = sum(
        1 for v in viagens_query if v.status == "em_andamento"
    )
    viagens_pendentes_kpi = sum(1 for v in viagens_query if v.status == "pendente")
    veiculos_disponiveis_kpi = Veiculo.query.filter_by(
        empresa_id=current_user.empresa_id, status="Disponível"
    ).count()
    ...
    if current_user.role in ["Admin", "Master"]:
        ...
        viagens_do_mes = (
            ...
            .options(
                db.joinedload(Viagem.custo_viagem), db.joinedload(Viagem.abastecimentos)
            )
            .all()
        )
    ...
    if request.method == "POST":
        try:
            ...
            if Cliente.query.filter_by(
                cpf_cnpj=cpf_cnpj, empresa_id=current_user.empresa_id
            ).first():
                flash(
                    "Erro: Este CPF/CNPJ já está cadastrado para um cliente em sua empresa.",
                    "error",
                )
            ...
            files = request.files.getlist(
                "anexos"
            )  # O 'name' do input no HTML é 'anexos'
...
def consultar_clientes():
    ...
    nomes_clientes = [
        c.nome_razao_social
        for c in Cliente.query.filter_by(empresa_id=current_user.empresa_id).all()
    ]

    total_receita_clientes = (
        db.session.query(func.sum(Viagem.valor_recebido))
        .filter(
            Viagem.empresa_id == current_user.empresa_id,
...
def public_tracking_page(token):
    ...
    current_year = datetime.utcnow(UTC).year
...
def search_clientes_simplificado():
    ...
    clientes = (
        Cliente.query.filter(
            Cliente.empresa_id
            == current_user.empresa_id,  # <-- FILTRO DE SEGURANÇA ADICIONADO AQUI
            or_(
...
def iniciar_viagem_motorista(viagem_id):
    ...
    if viagem.status != "pendente":
        return (
            jsonify(
                {"success": False, "message": "Esta viagem não está mais pendente."}
            ),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Odômetro inicial inválido. Por favor, insira um número válido.",
                }
            ),
            400,
        )
    ...
    viagem.data_inicio = datetime.utcnow(UTC)
    ...
    socketio.emit(
        "status_viagem_atualizado",
        {"viagem_id": viagem.id, "status": "em_andamento"},
        room="admins",
    )
...
def configuracao_fiscal():
    ...
    certificados = (
        CertificadoDigital.query.filter_by(empresa_id=current_user.empresa_id)
        .order_by(CertificadoDigital.principal.desc(), CertificadoDigital.id.asc())
        .all()
    )

    hoje = date.today()
    return render_template(
        "configuracao_fiscal.html", certificados=certificados, hoje=hoje
    )


@app.route("/api/cliente/<int:cliente_id>/upload_anexo", methods=["POST"])
@login_required
def api_upload_anexo_cliente(cliente_id):
    ...
    try:
        ...
        file_url = f"{public_url_base}/{s3_path}"

        # Adiciona a nova URL à lista existente e salva imediatamente
        ...
        return jsonify(
            {"success": True, "message": "Arquivo salvo!", "file_url": file_url}
        )

    except Exception as e:
        db.session.rollback()
        logger.error(
            f"Erro no upload de anexo para cliente {cliente_id}: {e}", exc_info=True
        )
...
def upload_certificado():
    ...
    try:
        ...
        try:
            ...
            flash(
                "Erro ao validar certificado. Verifique o arquivo e a senha.", "error"
            )
            return redirect(url_for("configuracao_fiscal"))
        ...
        try:
            serial_number_attrs = certificate.subject.get_attributes_for_oid(
            ...
            if not cnpj_certificado:
                common_name_attrs = certificate.subject.get_attributes_for_oid(
                    NameOID.COMMON_NAME
                )
        ...
        if is_principal:
            flash(
                "Certificado enviado e definido como principal com sucesso!", "success"
            )
        else:
            flash(
                'Certificado enviado com sucesso! Use "Tornar Principal" para ativá-lo.',
                "success",
            )
...
def definir_certificado_principal(certificado_id):
    ...
    try:
        ...
        certificado = CertificadoDigital.query.filter_by(
            id=certificado_id, empresa_id=current_user.empresa_id
        ).first()
        ...
        sucesso = CertificadoDigital.definir_como_principal(
            certificado_id, current_user.empresa_id
        )

        if sucesso:
            db.session.commit()
            flash(
                f'Certificado "{certificado.nome_arquivo}" definido como principal!',
                "success",
            )
        else:
...
def excluir_certificado(certificado_id):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": "Certificado excluído com sucesso!"}
        )
...
def registrar_abastecimento():
    try:
        ...
        if not motorista_formal:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Perfil de motorista formal não encontrado para o usuário atual.",
                    }
                ),
                400,
            )
        ...
        if not viagem_ativa:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Nenhuma viagem ativa encontrada para associar o abastecimento.",
                    }
                ),
                400,
            )
        ...
        return jsonify(
            {"success": True, "message": "Abastecimento registrado com sucesso!"}
        )
...
def registrar_usuario(token):
    ...
    if not convite or convite.data_expiracao < datetime.now(UTC).replace(tzinfo=None):
    ...
    if request.method == "POST":
        ...
        # LINHA ADICIONADA: Captura o CPF/CNPJ do formulário
        cpf_cnpj = re.sub(r"\D", "", request.form.get("cpf_cnpj", ""))

        usuario_existente = Usuario.query.filter_by(email=convite.email).first()
        if usuario_existente:
            flash("Já existe um usuário com este e-mail.", "error")
            return redirect(url_for("login"))

        # NOVO BLOCO DE VALIDAÇÃO: Verifica se o CPF/CNPJ já existe
        if cpf_cnpj and Usuario.query.filter_by(cpf_cnpj=cpf_cnpj).first():
            flash("Este CPF/CNPJ já está cadastrado para outro usuário.", "error")
            # Passa o 'convite' de volta para o template renderizar a página de novo
            # ▼▼▼ CORREÇÃO APLICADA AQUI ▼▼▼
            return render_template("registrar_usuario.html", convite=convite)
        ...
        novo_usuario = Usuario(
            ...
            cpf_cnpj=cpf_cnpj,  # CAMPO ADICIONADO: Salva o CPF/CNPJ no banco
        )
...
def calcular_media_km_veiculo(veiculo_id):
    ...
    resultado = (
        db.session.query(
            ...
            func.count(func.distinct(func.date(Viagem.data_inicio))).label(
                "dias_com_viagem"
            ),
        )
        .filter(
...
def enviar_convite():
    if not current_user.empresa_id:
        flash(
            "Você precisa estar vinculado a uma empresa para enviar convites.", "error"
        )
    ...
    if empresa_admin.licenca:
        ...
        if usuarios_atuais >= max_permitido:
            flash(
                f"Limite de usuários atingido ({max_permitido}) para o plano da sua empresa.",
                "error",
            )
            return redirect(url_for("configuracoes"))
    ...
    papeis_permitidos = ["Motorista", "Master", "Admin", "Oficina/Borracharia"]
    if role not in papeis_permitidos:
        flash("Papel inválido. Escolha entre Motorista, Master ou Admin.", "error")
        return redirect(url_for("configuracoes"))

    if current_user.role == "Master" and role not in [
        "Motorista",
        "Oficina/Borracharia",
    ]:
        flash(
            "Usuários do tipo Master só podem convidar Motoristas ou perfis de Oficina/Borracharia.",
            "error",
        )
        return redirect(url_for("configuracoes"))
    ...
    # --- CORREÇÃO APLICADA AQUI ---
    data_expiracao = datetime.now(UTC) + timedelta(days=3)
    ...
    try:
        ...
        link_convite = url_for("registrar_usuario", token=token, _external=True)
        msg = Message(
...
def calcular_rota_otimizada_ors(enderecos: list) -> tuple:
    """
    Calcula rota otimizada usando OpenRouteService.

    Args:
        enderecos: Lista de endereços (origem + destinos)

    Returns:
        (rota_otimizada, distancia_km, duracao_segundos, geometria, erro)
    """
    try:
        # Validação básica
        if not enderecos or len(enderecos) < 2:
            return None, 0, 0, None, "Pelo menos 2 endereços são necessários"

        if not OPENROUTESERVICE_API_KEY:
            return None, 0, 0, None, "Chave API ORS não configurada"

        logger.info(f"Calculando rota para {len(enderecos)} endereços")

        # ========== ETAPA 1: GEOCODIFICAÇÃO DE ENDEREÇOS ==========
        ...
        for endereco in enderecos:
            try:
                # Chamada ao geocodificador do ORS
                url_geocode = f"https://api.openrouteservice.org/geocode/search?api_key={OPENROUTESERVICE_API_KEY}&text={urllib.parse.quote(endereco)}"
                response = requests.get(url_geocode, timeout=10)
                response.raise_for_status()
                ...
                if not data.get("features") or len(data["features"]) == 0:
                    logger.warning(f"Endereço não encontrado: {endereco}")
                    return None, 0, 0, None, f"Endereço não encontrado: {endereco}"

                # Pega o primeiro resultado (melhor match)
                feature = data["features"][0]
                lon, lat = feature["geometry"]["coordinates"]
                endereco_formatado = feature["properties"].get("label", endereco)

                coordenadas.append([lon, lat])
                enderecos_geocodificados.append(endereco_formatado)

                logger.debug(f"Geocodificado: {endereco} -> [{lon}, {lat}]")

            except requests.exceptions.RequestException as e:
                logger.error(f"Erro ao geocodificar {endereco}: {e}")
                return None, 0, 0, None, f"Erro ao geocodificar: {endereco}"
            except (KeyError, IndexError) as e:
                logger.error(
                    f"Resposta inesperada do geocodificador para {endereco}: {e}"
                )
                return None, 0, 0, None, f"Erro ao processar resposta do geocodificador"

        # Verificar se temos coordenadas válidas
        if len(coordenadas) < 2:
            return None, 0, 0, None, "Não foi possível geocodificar os endereços"

        logger.info(f"Coordenadas obtidas: {coordenadas}")

        # ========== ETAPA 2: CALCULAR ROTA ==========

        try:
            # Instanciar cliente ORS
            ors_client = client.Client(key=OPENROUTESERVICE_API_KEY)

            # Fazer requisição diretamente com os parâmetros corretos
            routes = ors_client.directions(
                coordinates=coordenadas, geometry=True, instructions=False
            )

            if not routes or "routes" not in routes or len(routes["routes"]) == 0:
                logger.error("Nenhuma rota encontrada na resposta ORS")
                return None, 0, 0, None, "Nenhuma rota encontrada"

            # Extrair primeira rota (melhor opção)
            rota = routes["routes"][0]
            distancia_km = (
                rota["summary"]["distance"] / 1000
            )  # Converter de metros para km
            duracao_segundos = rota["summary"]["duration"]
            geometria = rota.get("geometry", "")

            logger.info(f"Rota calculada: {distancia_km:.2f} km, {duracao_segundos}s")

            # ========== ETAPA 3: RETORNAR DADOS ==========

            rota_otimizada = enderecos_geocodificados  # Retorna endereços formatados

            return rota_otimizada, distancia_km, duracao_segundos, geometria, None

        except exceptions.ApiError as e:
            logger.error(f"Erro API ORS: {e.args}")
            return None, 0, 0, None, f"Erro ao chamar API ORS: {str(e)}"

        except Exception as e:
            logger.error(f"Erro inesperado no cálculo de rota ORS: {e}", exc_info=True)
            return None, 0, 0, None, f"Erro inesperado: {str(e)}"

    except Exception as e:
        logger.error(f"Erro geral na função calcular_rota: {e}", exc_info=True)
        return None, 0, 0, None, "Erro ao calcular rota"


@app.route("/api/manutencao/excluir/<int:manutencao_id>", methods=["POST"])
@login_required
@oficina_borracharia_required
def api_excluir_manutencao(manutencao_id):
    """API para excluir uma manutenção que foi lançada incorretamente."""
    try:
        manutencao = Manutencao.query.filter_by(
            id=manutencao_id, empresa_id=current_user.empresa_id
        ).first_or_404()

        # Regra de segurança: Só permite excluir se estiver 'Agendada' ou 'Em Andamento'
        if manutencao.status not in ["Agendada", "Em Andamento"]:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Apenas manutenções agendadas ou em andamento podem ser excluídas.",
                    }
                ),
                400,
            )
        ...
        # Se a exclusão desta manutenção deixa o veículo sem nenhuma outra manutenção ativa,
        # o status do veículo volta para 'Disponível'.
        if veiculo and veiculo.status == "Em Manutenção":
            outras_manutencoes_ativas = Manutencao.query.filter(
                Manutencao.veiculo_id == veiculo.id,
                Manutencao.id != manutencao_id,
                Manutencao.status == "Em Andamento",
            ).count()
            if outras_manutencoes_ativas == 0:
                veiculo.status = "Disponível"

        # Deleta a manutenção e seus itens associados (graças ao 'cascade' no modelo)
        ...
        db.session.rollback()
        logger.error(f"Erro ao excluir manutenção {manutencao_id}: {e}", exc_info=True)
        return jsonify({"success": False, "message": f"Erro interno: {str(e)}"}), 500
...
def ocr_process():
    ...
    if "image" not in request.files:
        return (
            jsonify({"success": False, "message": "Nenhum arquivo de imagem enviado."}),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify({"success": False, "message": f"Erro ao processar a imagem: {e}"}),
            500,
        )
...
def editar_viagem_page(viagem_id):
    viagem = (
        Viagem.query.options(db.joinedload(Viagem.destinos))
        .filter_by(id=viagem_id, empresa_id=current_user.empresa_id)
        .first_or_404()
    )

    motoristas = (
        Motorista.query.filter_by(empresa_id=current_user.empresa_id)
    ...
    veiculos_disponiveis = Veiculo.query.filter_by(
        status="Disponível", empresa_id=current_user.empresa_id, is_administrativo=False
    ).all()
...
def editar_viagem_api(viagem_id):
    ...
    try:
        ...
        viagem.data_inicio = datetime.strptime(
            data.get("data_inicio"), "%Y-%m-%dT%H:%M"
        )
        ...
        if not enderecos_destino:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "É necessário pelo menos um endereço de destino.",
                    }
                ),
                400,
            )
        ...
        # --- LINHA CORRIGIDA ---
        rota_otimizada, distancia_km, duracao_segundos, geometria, erro = (
            calcular_rota_otimizada_ors(todos_enderecos)
        )

        if erro:
            return (
                jsonify(
                    {"success": False, "message": f"Erro ao recalcular a rota: {erro}"}
                ),
                400,
            )
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": f"Ocorreu um erro inesperado ao salvar: {e}",
                }
            ),
            500,
        )
...
def consultar_cobrancas():
    ...
    cobrancas = (
        Cobranca.query.filter(Cobranca.empresa_id == current_user.empresa_id)
        .options(db.joinedload(Cobranca.cliente))
        .order_by(Cobranca.data_vencimento.desc())
        .all()
    )
    ...
    total_pendente = sum(
        c.valor_total for c in cobrancas if c.status in ["Pendente", "Vencida"]
    )
    total_pago = sum(c.valor_total for c in cobrancas if c.status == "Paga")

    return render_template(
        "consultar_cobrancas.html",
        cobrancas=cobrancas,
...
@login_required
@oficina_borracharia_required  # DECORADOR APLICADO
def borracharia_dashboard():
    """Dashboard principal da borracharia"""
    # KPIs básicos
    kpis = {
        "em_estoque": Pneu.query.filter_by(
            empresa_id=current_user.empresa_id, status="Estoque"
        ).count(),
        "em_uso": Pneu.query.filter_by(
            empresa_id=current_user.empresa_id, status="Em Uso"
        ).count(),
        "recapando": Pneu.query.filter_by(
            empresa_id=current_user.empresa_id, status="Recapando"
        ).count(),
        "descartados": Pneu.query.filter_by(
            empresa_id=current_user.empresa_id, status="Descartado"
        ).count(),
    }
    ...
    if config:
        # Pneus com problemas
        pneus_alertas = (
            Pneu.query.filter_by(empresa_id=current_user.empresa_id)
            .filter(Pneu.status.in_(["Estoque", "Em Uso"]))
            .all()
        )
    ...
    veiculos = (
        Veiculo.query.filter_by(empresa_id=current_user.empresa_id)
        .options(db.joinedload(Veiculo.pneus))
        .all()
    )
    ...
    pneus_recapando = (
        Pneu.query.filter_by(empresa_id=current_user.empresa_id, status="Recapando")
        .order_by(Pneu.numero_fogo)
        .all()
    )

    return render_template(
        "borracharia_dashboard.html",
        kpis=kpis,
        alertas=alertas,
        veiculos=veiculos,
...
def borracharia_configuracoes():
    ...
    if request.method == "POST":
        try:
            ...
            config.sulco_minimo_recapagem_mm = float(
                request.form.get("sulco_minimo_recapagem_mm", 3.0)
            )
            config.sulco_minimo_descarte_mm = float(
                request.form.get("sulco_minimo_descarte_mm", 1.6)
            )
            config.max_recapagens = int(request.form.get("max_recapagens", 2))
            config.km_alerta_recapagem = int(
                request.form.get("km_alerta_recapagem", 5000)
            )
...
@login_required
def pneu_novo():
    """Cadastrar novo pneu, com suporte para lotes (múltiplas quantidades)."""
    try:
        quantidade = int(request.form.get("quantidade", 1))
        numero_fogo_prefixo = request.form.get("numero_fogo").strip().upper()

        # --- Validação Prévia ---
        # 1. Gera a lista de todos os números de fogo que serão criados
        numeros_a_criar = []
        if quantidade == 1:
            numeros_a_criar.append(numero_fogo_prefixo)
        else:
            for i in range(1, quantidade + 1):
                numeros_a_criar.append(f"{numero_fogo_prefixo}-{i}")

        # 2. Verifica se algum desses números já existe no banco de dados de uma só vez
        pneu_existente = Pneu.query.filter(
            Pneu.empresa_id == current_user.empresa_id,
            Pneu.numero_fogo.in_(numeros_a_criar),
        ).first()

        if pneu_existente:
            flash(
                f'Erro: O número de fogo "{pneu_existente.numero_fogo}" (ou um de seus sufixos) já existe. Tente um prefixo diferente.',
                "error",
            )
            return redirect(url_for("borracharia_dashboard"))
        ...
        for i in range(quantidade):
            numero_fogo_final = numeros_a_criar[i]

            novo_pneu = Pneu(
                empresa_id=current_user.empresa_id,
                numero_fogo=numero_fogo_final,
                marca=request.form.get("marca"),
                modelo=request.form.get("modelo"),
                dimensao=request.form.get("dimensao"),
                dot=request.form.get("dot"),
                data_compra=datetime.strptime(
                    request.form.get("data_compra"), "%Y-%m-%d"
                ).date(),
            ...
            db.session.flush()  # Garante que o pneu tenha um ID para a movimentação

            # Registrar movimentação para cada pneu criado
        ...
        flash(f"{pneus_criados} pneu(s) cadastrado(s) com sucesso!", "success")

    except Exception as e:
        db.session.rollback()
        flash(f"Erro ao cadastrar pneu(s): {e}", "error")
        logger.error(f"Erro ao cadastrar pneu(s) em lote: {e}", exc_info=True)
...
def api_instalar_pneu():
    ...
    try:
        ...
        pneu = Pneu.query.filter_by(
            id=pneu_id, empresa_id=current_user.empresa_id
        ).first()
        veiculo = Veiculo.query.filter_by(
            id=veiculo_id, empresa_id=current_user.empresa_id
        ).first()
        ...
        if not pneu or not veiculo:
            return (
                jsonify(
                    {"success": False, "message": "Pneu ou veículo não encontrado"}
                ),
                404,
            )
        ...
        if pneu.status != "Estoque":
            return (
                jsonify(
                    {"success": False, "message": "Pneu não está disponível no estoque"}
                ),
                400,
            )
        ...
        pneu_existente = Pneu.query.filter_by(
            veiculo_id=veiculo_id, posicao=posicao, status="Em Uso"
        ).first()
        ...
        if pneu_existente:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": f"Já existe um pneu na posição {posicao}",
                    }
                ),
                400,
            )
...
def api_remover_pneu():
    ...
    try:
        ...
        if pneu.veiculo and (
            pneu.veiculo.km_atual is None or km_veiculo > pneu.veiculo.km_atual
        ):
...
def api_veiculo_pneus(veiculo_id):
    ...
    return jsonify({"success": True, "pneus": pneus_data, "km_atual": veiculo.km_atual})
...
def api_retorno_recapagem():
    ...
    try:
        ...
        if pneu.status != "Recapando":
            return (
                jsonify({"success": False, "message": "Pneu não está em recapagem"}),
                400,
            )
        ...
        db.session.commit()
        return jsonify(
            {"success": True, "message": "Retorno de recapagem registrado com sucesso!"}
        )
    ...
    try:
        ...
        for linha_num, row in enumerate(csv_reader, start=2):
            try:
                ...
                if Pneu.query.filter_by(
                    empresa_id=current_user.empresa_id, numero_fogo=numero_fogo
                ).first():
        ...
        if erros:
            flash(
                f'Erros encontrados: {"; ".join(erros[:5])}{"..." if len(erros) > 5 else ""}',
                "warning",
            )
...
def pneu_historico(pneu_id):
    ...
    movimentacoes = (
        MovimentacaoPneu.query.filter_by(pneu_id=pneu_id)
        .options(
            db.joinedload(MovimentacaoPneu.veiculo),
            db.joinedload(MovimentacaoPneu.usuario),
        )
        .order_by(MovimentacaoPneu.data_movimentacao.desc())
        .all()
    )

    medicoes = (
        MedicaoSulco.query.filter_by(pneu_id=pneu_id)
        .options(db.joinedload(MedicaoSulco.usuario))
        .order_by(MedicaoSulco.data_medicao.desc())
        .all()
    )

    return render_template(
        "pneu_historico.html", pneu=pneu, movimentacoes=movimentacoes, medicoes=medicoes
    )
...
def get_viagem_map_data(viagem_id):
    ...
    ultima_localizacao = (
        Localizacao.query.filter_by(viagem_id=viagem_id)
        .order_by(Localizacao.timestamp.desc())
        .first()
    )
    ...
    for destino in sorted(viagem.destinos, key=lambda x: x.ordem):
        ...
        destinos_com_coords.append(
            {"endereco": destino.endereco, "latitude": lat, "longitude": lon}
        )
...
def gerar_cobranca():
    if request.method == "POST":
        try:
            ...
            if not all([cliente_id, viagem_ids, data_vencimento_str]):
                flash(
                    "Cliente, data de vencimento e ao menos uma viagem são obrigatórios.",
                    "error",
                )
            ...
            flash(
                "Cobrança gerada com sucesso! Visualizando a Nota de Débito.", "success"
            )
            return redirect(
                url_for("visualizar_nota_debito", cobranca_id=nova_cobranca.id)
            )
    ...
    return render_template(
        "gerar_cobranca.html", clientes=clientes, active_page="cobrancas"
    )
...
def api_marcar_paga(cobranca_id):
    Cobranca.query.filter_by(
        id=cobranca_id, empresa_id=current_user.empresa_id
    ).first_or_404()
    try:
        data = request.get_json()
        meio_pagamento = data.get("meio_pagamento")
        ...
        if not meio_pagamento:
            return (
                jsonify(
                    {"success": False, "message": "Meio de pagamento é obrigatório."}
                ),
                400,
            )

        cobranca.status = "Paga"
        cobranca.data_pagamento = datetime.now(UTC)
...
def escolher_viagem_para_cte():
    ...
    viagens_sem_cte = (
        ...
        .order_by(Viagem.data_fim.desc())
        .all()
    )

    return render_template(
        "cte/escolher_viagem_cte.html", viagens=viagens_sem_cte, active_page="cte"
    )
...
def sucesso_cte(cte_id):
    ...
    cte = CTeEmitido.query.filter_by(
        id=cte_id, empresa_id=current_user.empresa_id
    ).first_or_404()
...
def api_viagens_nao_cobradas(cliente_id):
    ...
    if not cliente or cliente.empresa_id != current_user.empresa_id:
        return (
            jsonify({"error": "Cliente não encontrado ou não pertence à sua empresa."}),
            404,
        )
    ...
    viagens_data = [
        {
            ...
            "valor_formatado": f"R$ {v.valor_recebido or 0:,.2f}".replace(",", "X")
            .replace(".", ",")
            .replace("X", "."),
        }
        for v in viagens
...
def cadastrar_motorista():
    if request.method == "POST":
        try:
            ...
            if Motorista.query.filter_by(
                cpf=cpf, empresa_id=current_user.empresa_id
            ).first():
                flash("Um motorista com este CPF já foi cadastrado.", "error")
                return render_template(
                    "cadastrar_motorista.html", motorista=request.form
                )

            if Motorista.query.filter_by(
                cnh_numero=cnh_numero, empresa_id=current_user.empresa_id
            ).first():
            ...
            if anexos_files and any(f.filename for f in anexos_files):
                ...
                if not bucket_name:
                    flash(
                        "Erro de configuração: O nome do bucket do Cloudflare R2 não foi definido nas variáveis de ambiente.",
                        "error",
                    )
                    return render_template(
                        "cadastrar_motorista.html", motorista=request.form
                ...
                for anexo_file in anexos_files:
                    if anexo_file.filename:
                        ...
                        s3_client.upload_fileobj(
                            ...
                            s3_path,
                            ExtraArgs={
                                "ContentType": anexo_file.content_type
                                or "application/octet-stream",
                                "ContentDisposition": "attachment",
                            },
                        )
            ...
            usuario_correspondente = Usuario.query.filter_by(
                cpf_cnpj=cpf, empresa_id=current_user.empresa_id
            ).first()

            novo_motorista = Motorista(
                empresa_id=current_user.empresa_id,
                usuario_id=(
                    usuario_correspondente.id if usuario_correspondente else None
                ),
                ...
                contato_telefone=re.sub(
                    r"\D", "", request.form.get("contato_telefone")
                ),
    ...
    return render_template(
        "cadastrar_motorista.html", motorista={}, active_page="cadastrar_motorista"
    )
...
def gerar_dacte_pdf(cte_id):
    ...
    pisa_status = pisa.CreatePDF(
        io.StringIO(html_renderizado), dest=pdf_buffer  # Fonte do HTML
    )  # Destino (o buffer)
    ...
    if pisa_status.err:
        ...
        return (
            "<h1>Erro ao gerar PDF</h1><p>Ocorreu um problema com a biblioteca de conversão.</p>",
            500,
        )
...
def editar_cliente(cliente_id):
    ...
    if request.method == "POST":
        try:
            # --- Validação de CPF/CNPJ (permanece igual) ---
            ...
            if cliente_existente:
                flash(
                    "Erro: O CPF/CNPJ informado já pertence a outro cliente.", "error"
                )
                return redirect(url_for("editar_cliente", cliente_id=cliente_id))

            # --- Atualização dos dados do formulário ---
            # Esta função agora SÓ se preocupa com os dados de texto.
            ...
            cliente.nome_fantasia = (
                ...
                if cliente.pessoa_tipo == "juridica"
                else None
            )
            cliente.cpf_cnpj = novo_cpf_cnpj
            ...
            cliente.numero = request.form.get(
                "numero"
            )  # Este campo agora receberá o valor correto
            ...
            # <<< A LÓGICA DE UPLOAD DE ARQUIVOS FOI REMOVIDA DESTA FUNÇÃO >>>
            # Ela agora é tratada exclusivamente pela nova API /api/cliente/<id>/upload_anexo
            ...
            db.session.rollback()
            logger.error(f"Erro ao editar cliente {cliente_id}: {e}", exc_info=True)
            flash(f"Ocorreu um erro inesperado ao salvar as alterações: {e}", "error")
            return redirect(url_for("editar_cliente", cliente_id=cliente_id))

    # A parte 'GET' da função não muda
    return render_template(
        "editar_cliente.html", cliente=cliente, active_page="consultar_clientes"
    )
...
def visualizar_nota_debito(cobranca_id):
    ...
    empresa_emissora = (
        db.session.get(Empresa, cobranca.usuario.empresa_id)
        if cobranca.usuario
        else None
    )
...
def consultar_motoristas():
    ...
    for motorista in motoristas_list:
        viagem_ativa = Viagem.query.filter(
            Viagem.motorista_id == motorista.id, Viagem.status == "em_andamento"
        ).first()
...
def api_excluir_anexo_cliente():
    ...
    try:
        ...
        db.session.rollback()
        logger.error(
            f"Erro ao excluir anexo do cliente {cliente_id}: {e}", exc_info=True
        )
        return (
            jsonify({"success": False, "message": f"Erro ao remover anexo: {e}"}),
            500,
        )
...
def exportar_rentabilidade_excel():
    ...
    try:
        ...
        # Query de veículos
        veiculos_query = Veiculo.query.filter(
            Veiculo.empresa_id == current_user.empresa_id
        )
        ...
        viagens_periodo = (
            ...
            .options(db.joinedload(Viagem.veiculo))
            .all()
        )
                ...
                todos_custos.append(
                    {
                        ...
                        "data": abast.data_abastecimento.date(),
                        "categoria": "Combustível",
                        "descricao": f"{abast.litros:.2f}L @ R${abast.preco_por_litro:.2f}/L",
                        "valor": float(abast.custo_total or 0),
                    }
                )
                    ...
                    todos_custos.append(
                        {
                            ...
                            "categoria": "Pedágio",
                            "descricao": "Custo com pedágios",
                            "valor": float(viagem.custo_viagem.pedagios),
                        }
                    )
                    ...
                    todos_custos.append(
                        {
                            ...
                            "valor": float(viagem.custo_viagem.alimentacao),
                        }
                    )

        # B. Custos de Rateio
        rateios = (
            RateioVeiculo.query.join(LancamentoFluxoCaixa)
            .filter(
                RateioVeiculo.veiculo_id.in_(veiculo_ids),
                LancamentoFluxoCaixa.data_pagamento.between(data_inicio, data_fim),
            )
            .options(db.joinedload(RateioVeiculo.veiculo))
            .all()
        )
        for r in rateios:
            todos_custos.append(
                {
                    "veiculo": r.veiculo,
                    "data": r.lancamento.data_pagamento,
                    "categoria": r.lancamento.categoria,
                    "descricao": r.lancamento.descricao,
                    "valor": float(r.valor_rateado),
                }
            )
        ...
        if motoristas_ids:
            ...
            for f in folhas_pagas:
                for item in f.itens.filter(
                    ItemFolhaPagamento.tipo == "Provento",
                    ItemFolhaPagamento.viagem_id.isnot(None),
                ):
                    viagem_associada = db.session.get(Viagem, item.viagem_id)
                    if viagem_associada and viagem_associada.veiculo_id in veiculo_ids:
                        todos_custos.append(
                            {
                                "veiculo": viagem_associada.veiculo,
                                "data": f.data_pagamento.date(),
                                "categoria": "Pagamento Pessoal",
                                "descricao": f"{item.descricao} - {f.motorista.nome}",
        ...
        for placa, dados in sorted(dados_por_veiculo.items()):
            ...
            sheet.append(
                ["Data", "Cliente", "Destino", "Material", "Peso (TN)", "Valor (R$)"]
            )
            ...
            for r in dados["receitas"]:
                sheet.append(
                    [
                        ...
                        r.cliente,
                        r.endereco_destino,
                        r.material_transportado or "N/A",
                        r.peso_toneladas or 0,
                        r.valor_recebido or 0,
                    ]
                )
            ...
            resumo_geral.append(
                {
                    "placa": placa,
                    "receita": total_receitas_veiculo,
                    "custo": total_custos_veiculo,
                    "lucro": lucro_veiculo,
                }
            )
        ...
        for resumo in resumo_geral:
            sheet_resumo.append(
                [resumo["placa"], resumo["receita"], resumo["custo"], resumo["lucro"]]
            )
            grand_total_receita += resumo["receita"]
            grand_total_custo += resumo["custo"]
            for col in range(2, 5):
                sheet_resumo.cell(row=sheet_resumo.max_row, column=col).style = (
                    currency_style
                )
        ...
        sheet_resumo.append(
            [
                ...
                grand_total_receita,
                grand_total_custo,
                grand_total_receita - grand_total_custo,
            ]
        )
        for cell in sheet_resumo[sheet_resumo.max_row]:
            cell.font = Font(bold=True, size=12)
        for col in range(2, 5):
            sheet_resumo.cell(row=sheet_resumo.max_row, column=col).style = (
                currency_style
            )
        for col_idx, column_cells in enumerate(sheet_resumo.columns, 1):
            sheet_resumo.column_dimensions[get_column_letter(col_idx)].width = 25
...
def login():
    if current_user.is_authenticated:
        ...
        elif current_user.role == "Oficina/Borracharia":
            return redirect(url_for("oficina"))  # Redireciona para a tela da oficina
        else:
    ...
    if request.method == "POST":
        ...
        if usuario and usuario.check_password(senha):
            ...
            if usuario.role == "Owner":
                ...
                return redirect(url_for("motorista_dashboard"))
            # ADICIONE ESTA CONDIÇÃO TAMBÉM
            elif usuario.role == "Oficina/Borracharia":
                return redirect(url_for("oficina"))
            else:
                return redirect(url_for("painel"))
        else:
...
def editar_motorista(motorista_id):
    ...
    motorista = Motorista.query.filter_by(
        id=motorista_id, empresa_id=current_user.empresa_id
    ).first_or_404()
    ...
    if request.method == "POST":
        try:
            ...
            motorista_com_mesmo_cpf = Motorista.query.filter(
                Motorista.id
                != motorista_id,  # A CONDIÇÃO MAIS IMPORTANTE: Exclui o próprio registro da busca.
                Motorista.cpf == cpf,
            ...
            if motorista_com_mesmo_cpf:
                flash(
                    "Erro: Este CPF já está cadastrado para outro motorista.", "error"
                )
            ...
            if motorista_com_mesma_cnh:
                flash(
                    "Erro: Esta CNH já está cadastrada para outro motorista.", "error"
                )
                return redirect(url_for("editar_motorista", motorista_id=motorista_id))
            ...
            motorista.cnh_data_vencimento = to_date(
                request.form.get("cnh_data_vencimento")
            )
            ...
            # ▼▼▼ LINHA ADICIONADA AQUI ▼▼▼
            motorista.salario_base = request.form.get(
                "salario_base", type=float, default=0.0
            )
            ...
            motorista.contato_telefone = re.sub(
                r"\D", "", request.form.get("contato_telefone", "")
            )
...
def relatorio_financeiro_dre():
    ...
    try:
        ...
        dre = {
            ...
            "custos_fixos": {
                "Total": custos_fixos_total,
                "Detalhado": dict(custos_fixos),
            },
...
def limpar_links_anexos_command():
    ...
    try:
        ...
        for nome_modelo, colunas in modelos_e_colunas.items():
            ...
            for coluna in colunas:
                ...
                registros_afetados = (
                    ...
                    .filter(getattr(modelo, coluna).isnot(None))
                    .update({coluna: None}, synchronize_session=False)
                )

                if registros_afetados > 0:
                    click.echo(
                        f"  -> Limpando coluna '{coluna}' da tabela '{nome_modelo}': {registros_afetados} registro(s) atualizado(s)."
                    )
        ...
        if total_afetado > 0:
            click.secho(
                f"\nOperação concluída! {total_afetado} referências de anexos foram removidas do banco de dados.",
                fg="green",
                bold=True,
            )
        else:
            click.secho(
                "\nOperação concluída! Nenhum link de anexo foi encontrado no banco de dados para ser limpo.",
                fg="green",
            )
...
def adicionar_anexo_motorista(motorista_id):
    ...
    try:
        ...
        logger.error(
            f"Erro no upload de anexo para motorista {motorista_id}: {e}", exc_info=True
        )
...
def relatorio_desempenho_veiculos():
    ...
    try:
        ...
        for veiculo in veiculos:
            ...
            despesas_gerais = DespesaVeiculo.query.filter_by(
                veiculo_id=veiculo.id
            ).all()
            ...
            custo_total = (
                custo_combustivel
                + custo_despesas_viagens
                + custo_despesas_fixas
                + custo_manutencoes
            )
        ...
        flash(
            "Ocorreu um erro ao gerar o relatório de desempenho dos veículos.", "error"
        )
        return redirect(url_for("relatorios"))
...
def relatorio_contas_a_receber():
    ...
    try:
        ...
        contas = (
            ...
            .order_by(Cobranca.data_vencimento.asc())
            .all()
        )
...
@owner_required
def owner_dashboard():
    # Busca todas as empresas para a lista principal
    empresas = (
        Empresa.query.options(db.joinedload(Empresa.licenca))
    ...
    kpis = {
        ...
        "total_usuarios": Usuario.query.filter(Usuario.role != "Owner").count(),
        "total_veiculos": Veiculo.query.count(),
    }

    return render_template("owner_dashboard.html", empresas=empresas, kpis=kpis)

@app.route('/viagens/criar_lote', methods=['GET'])
@login_required
@master_required # Ou o decorator apropriado
def criar_viagem_lote_page():
    """Renderiza a página para criação de viagens em lote."""
    try:
        # Busca dados para preencher os selects do formulário
        ...
        carretas = Carreta.query.filter_by(status='Disponível', empresa_id=current_user.empresa_id).order_by(Carreta.placa).all()
        # Busca TODOS os clientes da empresa para o select de destino
        clientes = Cliente.query.filter_by(empresa_id=current_user.empresa_id).order_by(Cliente.nome_razao_social).all()

        # Busca endereço da empresa para preencher a origem padrão
        empresa = db.session.get(Empresa, current_user.empresa_id)
        endereco_empresa_parts = []
        if empresa:
            if getattr(empresa, 'endereco', None):
                endereco_empresa_parts.append(empresa.endereco)
            numero_empresa = str(getattr(empresa, 'numero', '')).strip()
            if numero_empresa:
                 endereco_empresa_parts.append(numero_empresa)
            if getattr(empresa, 'bairro', None):
                endereco_empresa_parts.append(empresa.bairro)
            if getattr(empresa, 'cidade', None):
                endereco_empresa_parts.append(empresa.cidade)
            if getattr(empresa, 'estado', None):
                endereco_empresa_parts.append(empresa.estado)
        endereco_empresa = ", ".join(filter(None, endereco_empresa_parts)).replace(" ,", ",")

        # Renderiza o template passando todos os dados necessários
        return render_template(
            'criar_viagem_lote.html',
            motoristas=motoristas,
            veiculos=veiculos,
            carretas=carretas,
            clientes=clientes, # Passa a lista de clientes para o template
            endereco_origem_padrao=endereco_empresa,
            ORS_API_KEY=OPENROUTESERVICE_API_KEY, # Passa a chave da API ORS
            csrf_token=generate_csrf() # Passa o token CSRF
        )
    except Exception as e:
        logger.error(f"Erro ao carregar página de criar viagem em lote: {e}", exc_info=True)
        flash("Erro ao carregar dados para a página. Tente novamente.", "error")
        return redirect(url_for('painel')) # Ajuste 'painel' se necessário

# --- ROTA API PARA GERAR PRÉVIA (Atualizada para nova lógica de cliente/destino) ---
@app.route('/api/viagens/gerar_lote_preview', methods=['POST'])
@login_required
@master_required # Ou o decorator apropriado
def api_gerar_lote_preview():
    """API para gerar a prévia das viagens em lote."""
    try:
        data = request.get_json()
        logger.debug(f"Dados recebidos para preview: {data}")

        # Validações básicas (ajustadas)
        motorista_id = data.get('motorista_id')
        veiculo_id = data.get('veiculo_id')
        cliente_nome_principal = data.get('cliente_nome_principal') # Nome do primeiro cliente
        endereco_saida = data.get('endereco_saida')
        enderecos_destino_lista = data.get('enderecos_destino', []) # Recebe a LISTA de endereços
        data_base_str = data.get('data_base')
        quantidade = data.get('quantidade')

        # Verifica campos obrigatórios
        if not all([motorista_id, veiculo_id, cliente_nome_principal, endereco_saida, data_base_str, quantidade]):
            return jsonify({'success': False, 'message': 'Motorista, Veículo, Cliente Principal, Origem, Data Base e Quantidade são obrigatórios.'}), 400
        if not enderecos_destino_lista:
             return jsonify({'success': False, 'message': 'Pelo menos um endereço de destino válido é obrigatório.'}), 400

        # Valida e converte quantidade e data
        try:
            quantidade = int(quantidade)
            if quantidade <= 0 or quantidade > 50: # Limite ajustado
                 raise ValueError("Quantidade deve ser entre 1 e 50.")
            # Tenta formatos diferentes para data/hora (com T ou espaço)
            try:
                data_base = datetime.fromisoformat(data_base_str.replace(' ', 'T'))
            except ValueError:
                data_base = datetime.strptime(data_base_str, '%Y-%m-%d %H:%M')
        except (ValueError, TypeError) as e:
            logger.error(f"Erro de conversão nos dados de preview (Quantidade ou Data Base): {e}")
            return jsonify({'success': False, 'message': f'Dados inválidos (Quantidade ou Data Base): {e}'}), 400

        # Busca objetos no banco (Motorista, Veículo, Carreta)
        motorista = db.session.get(Motorista, int(motorista_id))
        veiculo = db.session.get(Veiculo, int(veiculo_id))
        carreta = None
        carreta_id = data.get('carreta_id')
        if carreta_id:
            carreta = db.session.get(Carreta, int(carreta_id))

        # Valida existência dos objetos
        if not motorista or not veiculo:
            return jsonify({'success': False, 'message': 'Motorista ou Veículo não encontrado.'}), 404
        if carreta_id and not carreta:
            return jsonify({'success': False, 'message': 'Carreta selecionada não encontrada.'}), 404

        # Coleta dados de Frete (sem alterações)
        is_frete = data.get('is_frete', False)
        material = data.get('material') if is_frete else None
        peso_toneladas = data.get('peso_toneladas') if is_frete else None
        valor_por_tonelada = data.get('valor_por_tonelada') if is_frete else None # Valor/Ton Motorista

        viagens_preview = [] # Lista para guardar os dados das prévias
        intervalo_minutos = data.get('intervalo_minutos', 0)
        try:
            intervalo_minutos = int(intervalo_minutos) if intervalo_minutos else 0
        except ValueError:
            intervalo_minutos = 0

        # Gera a lista de prévias
        for i in range(quantidade):
            data_viagem_atual = data_base + timedelta(minutes=intervalo_minutos * i)
            viagens_preview.append({
                # IDs para facilitar o salvamento posterior
                'motorista_id': motorista.id,
                'veiculo_id': veiculo.id,
                'carreta_id': carreta.id if carreta else None,
                # Dados para exibição na grid
                'motorista_nome': motorista.nome,
                'veiculo_placa': veiculo.placa,
                'carreta_placa': carreta.placa if carreta else 'N/A',
                'cliente_nome_principal': cliente_nome_principal, # Nome do cliente principal
                'endereco_saida': endereco_saida,
                'destinos_list': enderecos_destino_lista, # Envia a lista completa de endereços de destino
                'data_hora_iso': data_viagem_atual.isoformat(sep=' ', timespec='minutes'), # Formato para input datetime-local
                # Dados de frete para a grid
                'is_frete': is_frete,
                'material': material,
                'peso_toneladas': peso_toneladas,
                'valor_por_tonelada': valor_por_tonelada # Valor/Ton Motorista
            })

        logger.info(f"Gerado preview de {len(viagens_preview)} viagens.")
        # Retorna a lista de prévias como JSON
        return jsonify({'success': True, 'viagens': viagens_preview})

    except Exception as e:
        logger.error(f"Erro ao gerar preview de viagens em lote: {e}", exc_info=True)
        return jsonify({'success': False, 'message': f'Erro interno inesperado: {str(e)}'}), 500

# --- ROTA API PARA SALVAR LOTE (Atualizada para nova lógica de cliente/destino) ---
@app.route('/api/viagens/salvar_lote', methods=['POST'])
@login_required
@master_required # Ou o decorator apropriado
def api_salvar_lote():
    """API para salvar o lote de viagens criadas na pré-visualização."""
    viagens_para_salvar = request.get_json() # Recebe a lista de viagens da grid
    if not viagens_para_salvar:
        return jsonify({'success': False, 'message': 'Nenhuma viagem para salvar.'}), 400

    viagens_criadas_ids = [] # Lista para guardar IDs das viagens criadas
    erros = [] # Lista para armazenar mensagens de erro específicas por viagem

    # Usa uma transação para garantir que ou todas são salvas, ou nenhuma
    try:
        # Inicia uma sub-transação (ou transação principal se não houver outra ativa)
        with db.session.begin_nested():
            # Itera sobre cada viagem enviada pela grid
            for index, viagem_data in enumerate(viagens_para_salvar):
                try:
                    # Coleta dados da viagem atual do loop (vindos dos inputs e data-*)
                    motorista_id = viagem_data.get('motorista_id')
                    veiculo_id = viagem_data.get('veiculo_id')
                    carreta_id = viagem_data.get('carreta_id') # Pode ser None
                    cliente_nome = viagem_data.get('cliente_nome') # Nome do cliente principal
                    endereco_saida = viagem_data.get('endereco_saida')
                    enderecos_destino_viagem = viagem_data.get('enderecos_destino', []) # Pega a LISTA de endereços
                    data_inicio_str = viagem_data.get('data_hora_iso') # Vem no formato ISO
                    valor_recebido_str = viagem_data.get('valor_recebido', '0') # Valor a cobrar
                    observacoes = viagem_data.get('observacoes', '')

                    # Coleta dados de Frete (editados na grid)
                    is_frete = viagem_data.get('is_frete', False)
                    material = viagem_data.get('material') if is_frete else None
                    peso_str = viagem_data.get('peso_toneladas') if is_frete else None
                    valor_ton_str = viagem_data.get('valor_por_tonelada') if is_frete else None # Valor/Ton Motorista

                    # --- Revalidações Essenciais ---
                    if not all([motorista_id, veiculo_id, cliente_nome, endereco_saida, data_inicio_str]):
                         raise ValueError("Dados essenciais faltando (motorista, veículo, cliente, origem, data).")
                    if not enderecos_destino_viagem:
                         raise ValueError("Lista de destinos não pode estar vazia.")

                    # Busca motorista e veículo no banco (garante que ainda existem)
                    motorista = db.session.get(Motorista, int(motorista_id))
                    veiculo = db.session.get(Veiculo, int(veiculo_id))
                    if not motorista or not veiculo:
                        raise ValueError(f"Motorista ({motorista_id}) ou Veículo ({veiculo_id}) não encontrado no banco.")
                    # Poderia adicionar verificação de status do veículo/motorista aqui se necessário
                    # if veiculo.status != 'Disponível': raise ValueError(...)

                    # Busca carreta se houver ID
                    carreta = None
                    if carreta_id:
                        carreta = db.session.get(Carreta, int(carreta_id))
                        if not carreta: raise ValueError(f"Carreta ({carreta_id}) não encontrada.")
                        # if carreta.status != 'Disponível': raise ValueError(...)

                    # Converte data/hora (formato ISO) e valor recebido
                    try:
                        data_inicio = datetime.fromisoformat(data_inicio_str) # Converte de ISO para datetime
                        valor_recebido = float(valor_recebido_str) if valor_recebido_str else 0.0
                    except ValueError as e_conv:
                        raise ValueError(f"Formato inválido para Data/Hora ou Valor a Cobrar: {e_conv}")

                    # --- Calcular Custo Variável do Motorista (Frete) ---
                    custo_motorista_frete = None
                    peso_toneladas_float = None
                    if is_frete and peso_str and valor_ton_str:
                        try:
                            # Converte peso e valor/ton para float
                            peso_toneladas_float = float(peso_str)
                            valor_ton_float = float(valor_ton_str) # Valor/Ton Motorista
                            # Calcula o custo se ambos forem válidos (permite valor/ton 0)
                            if peso_toneladas_float > 0 and valor_ton_float >= 0:
                                 custo_motorista_frete = peso_toneladas_float * valor_ton_float
                        except ValueError:
                             # Se a conversão falhar, loga um aviso mas continua (sem custo de frete)
                             logger.warning(f"Valores de frete inválidos para viagem {index+1}. Peso: {peso_str}, Valor/Ton: {valor_ton_str}. Custo do frete não será calculado.")

                    # --- Calcular Rota Otimizada com Múltiplos Destinos ---
                    # Monta a lista completa de endereços para a API de rota
                    todos_enderecos_rota = [endereco_saida] + enderecos_destino_viagem
                    # Chama a função que usa a API ORS
                    rota_otimizada, distancia_km, duracao_segundos, geometria, erro_rota = calcular_rota_otimizada_ors(todos_enderecos_rota)

                    # Verifica se houve erro no cálculo da rota
                    if erro_rota:
                        raise ValueError(f"Erro ao calcular a rota: {erro_rota}")
                    # Verifica se a rota retornada é válida
                    if not rota_otimizada or len(rota_otimizada) < 2:
                        raise ValueError("Cálculo de rota retornou um resultado inválido (sem origem/destino).")
                    ...
                    nova_viagem = Viagem(
                        ...
                        veiculo_id=veiculo.id,
                        carreta_id=carreta.id if carreta else None,
                        cliente=cliente_nome, # Nome do cliente principal
                        valor_recebido=valor_recebido if valor_recebido > 0 else None, # Salva valor a cobrar
                        forma_pagamento=None, # Definir um padrão ou deixar nulo? Pode vir do form inicial se desejado
                        endereco_saida=endereco_saida, # Usa a origem original
                        endereco_destino=rota_otimizada[-1], # O último ponto da rota otimizada é o destino final principal
                        distancia_km=distancia_km, # Distância estimada pela API
                        duracao_segundos=duracao_segundos, # Duração estimada pela API
                        data_inicio=data_inicio, # Data/hora definida na grid
                        status='pendente', # Status inicial
                        observacoes=observacoes, # Observações da grid
                        empresa_id=current_user.empresa_id, # ID da empresa do usuário logado
                        route_geometry=geometria, # Geometria da rota (para mapa)
                        # Campos de frete
                        peso_toneladas=peso_toneladas_float,
                        material_transportado=material,
                        custo_motorista_variavel=custo_motorista_frete # Custo do frete calculado
                    )
                    db.session.add(nova_viagem) # Adiciona o objeto à sessão do SQLAlchemy
                    db.session.flush() # Importante para obter o nova_viagem.id antes de criar os destinos

                    # --- Criação dos Destinos no Banco de Dados ---
                    # Salva todos os pontos retornados pela API ORS como destinos, exceto a origem (índice 0)
                    for ordem, endereco_formatado in enumerate(rota_otimizada[1:], 1):
                        destino_obj = Destino(
                            viagem_id=nova_viagem.id, # Vincula ao ID da viagem recém-criada
                            endereco=endereco_formatado, # Endereço formatado pela API ORS
                            ordem=ordem # Ordem otimizada da rota
                        )
                        db.session.add(destino_obj) # Adiciona cada destino à sessão

                    viagens_criadas_ids.append(nova_viagem.id) # Guarda o ID da viagem criada
                    logger.info(f"Viagem em lote {index+1} pré-adicionada (ID será {nova_viagem.id} após commit).")

                # Se ocorrer um erro DENTRO do loop para uma viagem específica
                except Exception as e_inner:
                    # Captura o erro, formata a mensagem e adiciona à lista de erros
                    error_msg = f"Erro ao processar viagem {index + 1}: {str(e_inner)}"
                    logger.error(error_msg, exc_info=False) # Loga o erro
                    erros.append(error_msg)
                    # Re-lança a exceção para interromper o processo e acionar o rollback da transação
                    raise

        # Se o loop terminou sem nenhuma exceção, o 'with' commita a sub-transação automaticamente
        logger.info(f"Commit realizado com sucesso para {len(viagens_criadas_ids)} viagens em lote.")
        # Retorna sucesso com a lista de IDs criados
        return jsonify({
            'success': True,
            'message': f'{len(viagens_criadas_ids)} viagens criadas com sucesso!',
            'ids': viagens_criadas_ids
        })

    # Se ocorreu uma exceção (dentro do loop ou erro geral)
    except Exception as e_outer:
        db.session.rollback() # Garante rollback explícito da transação principal
        if erros:
            # Se a lista 'erros' foi populada, o erro foi dentro do loop
            logger.warning(f"Rollback realizado devido a erros em {len(erros)}/{len(viagens_para_salvar)} viagens durante o salvamento em lote.")
            # Retorna falha com a lista de erros específicos
            return jsonify({
                'success': False,
                'message': f'Falha ao criar viagens. {len(erros)}/{len(viagens_para_salvar)} viagens tiveram problemas. Nenhuma viagem foi salva.',
                'errors': erros # Envia a lista de erros para o frontend
            }), 400 # HTTP 400 Bad Request
        else:
            # Se a lista 'erros' está vazia, foi um erro geral (ex: erro de banco ao commitar)
            logger.error(f"Erro GERAL ao salvar viagens em lote: {e_outer}", exc_info=True)
            # Retorna falha com erro genérico
            return jsonify({'success': False, 'message': f'Erro interno inesperado ao salvar: {str(e_outer)}'}), 500 # HTTP 500 Internal Server Error


@app.route("/owner/empresa/<int:empresa_id>/usuarios")
@login_required
@owner_required
def owner_listar_usuarios(empresa_id):
    """Página que lista todos os usuários de uma empresa específica."""
    empresa = Empresa.query.get_or_404(empresa_id)
    usuarios = (
        Usuario.query.filter_by(empresa_id=empresa.id).order_by(Usuario.nome).all()
    )
    return render_template(
        "owner_listar_usuarios.html", usuarios=usuarios, empresa=empresa
    )


@app.route("/owner/impersonate/<int:user_id>")
@login_required
@owner_required
def impersonate(user_id):
    """Inicia a personificação (acessar como outro usuário)."""
    user_to_impersonate = Usuario.query.get_or_404(user_id)

    # Salva o ID original do Owner na sessão
    session["_original_user_id"] = current_user.id
    session["_impersonating"] = True
    session.permanent = True

    # Faz o login como o usuário alvo
    logout_user()
    login_user(user_to_impersonate, remember=True)

    flash(
        f"Agora você está visualizando o sistema como {user_to_impersonate.nome}.",
        "info",
    )

    # Redireciona para o painel correto do usuário
    if user_to_impersonate.role in ["Admin", "Master"]:
        return redirect(url_for("painel_gestor"))
    elif user_to_impersonate.role == "Motorista":
        return redirect(url_for("motorista_dashboard"))
    else:
        return redirect(url_for("painel"))


@app.route("/owner/stop_impersonating")
@login_required
def stop_impersonating():
    """Para a personificação e retorna para a conta do Owner."""
    if not session.get("_impersonating"):
        return redirect(url_for("index"))

    original_user_id = session.pop("_original_user_id", None)
    session.pop("_impersonating", None)

    if original_user_id:
        owner = db.session.get(Usuario, original_user_id)
        logout_user()  # Desloga o usuário que estava sendo personificado
        login_user(owner)  # Loga de volta como Owner
        flash("Você voltou para sua conta de Proprietário.", "success")

    return redirect(url_for("owner_dashboard"))


@app.route("/owner/create_empresa", methods=["POST"])
@login_required
@owner_required
def owner_create_empresa():
    razao_social = request.form.get("razao_social", "").strip()
    cnpj = re.sub(r"\D", "", request.form.get("cnpj", ""))
    admin_nome = request.form.get("admin_nome", "").strip()  # <-- CAMPO ADICIONADO
    admin_email = request.form.get("admin_email", "").strip().lower()
    nome_fantasia = request.form.get("nome_fantasia", "").strip() or razao_social

    # Validação de campos obrigatórios (agora inclui admin_nome)
    if not all([razao_social, cnpj, admin_nome, admin_email]):
        flash(
            "Razão Social, CNPJ, Nome do Admin e E-mail do Admin são obrigatórios.",
            "error",
        )
        return redirect(url_for("owner_dashboard"))

    # Valida comprimento do CNPJ
    if len(cnpj) != 14:
    ...
    if Empresa.query.filter_by(cnpj=cnpj).first():
        flash("Uma empresa com este CNPJ já existe.", "error")
        return redirect(url_for("owner_dashboard"))

    # Verifica e-mail duplicado
    if Usuario.query.filter_by(email=admin_email).first():
        flash(
            f"Erro: Já existe um usuário no sistema com o e-mail {admin_email}.",
            "error",
        )
        return redirect(url_for("owner_dashboard"))
    ...
    try:
        ...
        # Cria convite para o admin
        token = str(uuid.uuid4())
        data_expiracao = datetime.now(UTC) + timedelta(days=7)

        convite_admin = Convite(
            email=admin_email,
            token=token,
            usado=False,
            data_expiracao=data_expiracao,
        ...
        # Envia e-mail
        msg = Message(
            subject=f"Você foi convidado para administrar a empresa {razao_social} no sistema TrackBras",
            recipients=[admin_email],
            body=f'Olá, {admin_nome}!\n\nVocê foi convidado para ser o administrador da empresa "{razao_social}" no nosso sistema de gestão de transportes.\n\n'
            f"Clique no link abaixo para criar sua conta e começar a gerenciar sua equipe:\n\n{link_convite}\n\n"
        ...
        db.session.commit()
        flash(
            f'Empresa "{razao_social}" criada e convite enviado para {admin_email}!',
            "success",
        )
...
def owner_empresa_detalhes(empresa_id):
    """Gerencia detalhes e licença de uma empresa específica."""
    empresa = Empresa.query.get_or_404(empresa_id)

    # Garante que a empresa tenha uma licença
    ...
    if request.method == "POST":
        try:
            ...
            licenca.plano = request.form.get("plano", "Básico").strip()

            # Validação de números
            try:
                licenca.max_usuarios = int(request.form.get("max_usuarios", 5))
                licenca.max_veiculos = int(request.form.get("max_veiculos", 10))
            except ValueError:
                flash("Máximo de usuários e veículos devem ser números.", "error")
            ...
            if data_expiracao_str:
                try:
                    licenca.data_expiracao = datetime.strptime(
                        data_expiracao_str, "%Y-%m-%d"
                    ).date()
                except ValueError:
                    flash("Data inválida.", "error")
                    return render_template(
                        "owner_empresa_detalhes.html", empresa=empresa
                    )
            ...
            db.session.rollback()
            flash(f"Erro ao atualizar a licença: {str(e)}", "error")
            logger.error(f"Erro ao atualizar licença: {e}", exc_info=True)
...
def excluir_anexo_motorista(motorista_id):
    ...
    if anexo_url_para_excluir in anexos_atuais:
        try:
            ...
            if not bucket_name:
                flash(
                    "Erro de configuração: O nome do bucket do Cloudflare R2 não foi definido.",
                    "error",
                )
                return redirect(url_for("editar_motorista", motorista_id=motorista_id))
            ...
            key = anexo_url_para_excluir.replace(
                app.config["CLOUDFLARE_R2_PUBLIC_URL"] + "/", ""
            )
            s3_client.delete_object(Bucket=bucket_name, Key=key)
...
@app.route("/owner/delete_empresa/<int:empresa_id>", methods=["POST"])
@login_required
@owner_required
def owner_delete_empresa(empresa_id):
    """
    Exclui uma empresa e TODOS os seus dados associados em cascata.
    """
    empresa = Empresa.query.get_or_404(empresa_id)
    razao_social_deletada = empresa.razao_social

    try:
        # 1. Verificar se algum usuário da empresa tem bloqueios (ex: cobranças)
        usuarios_para_excluir = Usuario.query.filter_by(empresa_id=empresa.id).all()
        for usuario in usuarios_para_excluir:
            if Cobranca.query.filter_by(usuario_id=usuario.id).first():
                flash(
                    f'Erro: A empresa "{razao_social_deletada}" não pode ser excluída, pois o usuário {usuario.email} tem cobranças financeiras associadas.',
                    "error",
                )
                return redirect(url_for("owner_dashboard"))

        # 2. Excluir todos os dados dependentes em ordem
        # (Adicione outros modelos que dependem da empresa aqui se necessário)
        Viagem.query.filter_by(empresa_id=empresa.id).delete()
        Motorista.query.filter_by(empresa_id=empresa.id).delete()
        Veiculo.query.filter_by(empresa_id=empresa.id).delete()
        Cliente.query.filter_by(empresa_id=empresa.id).delete()
        Convite.query.filter_by(empresa_id=empresa.id).delete()
        Licenca.query.filter_by(empresa_id=empresa.id).delete()
        ...
        db.session.commit()
        flash(
            f'Empresa "{razao_social_deletada}" e todos os seus dados foram excluídos com sucesso!',
            "success",
        )

    except Exception as e:
        db.session.rollback()
        flash(f"Erro ao excluir a empresa: {str(e)}", "error")
        logger.error(f"Erro ao excluir empresa {empresa_id}: {e}", exc_info=True)
...
def cadastrar_veiculo():
    if request.method == "POST":
        try:
            # --- Funções auxiliares para limpeza e conversão de dados ---
            def clean_string(s):
                val = request.form.get(s, "").strip()
            ...
            def to_float(s):
                ...
                if not val:
                    return None
                # Limpa a formatação de moeda para salvar no banco
                clean_val = (
                    val.replace("R$", "").strip().replace(".", "").replace(",", ".")
                )
            ...
            # Validação de placa única para evitar duplicatas
            if (
                Veiculo.query.filter_by(
                    placa=placa, empresa_id=current_user.empresa_id
                ).first()
                or Carreta.query.filter_by(
                    placa=placa, empresa_id=current_user.empresa_id
                ).first()
            ):
                flash(
                    f'Erro: A placa "{placa}" já está cadastrada na sua frota.', "error"
                )
                return render_template("cadastrar_veiculo.html", form_data=request.form)

            # --- Lógica de Upload de Anexos ---
            anexos_urls = []
            # CORREÇÃO APLICADA AQUI: Usando .getlist('anexos') para múltiplos arquivos
            files = request.files.getlist("anexos")
            ...
            if files and any(f and f.filename for f in files):
                ...
                bucket_name = app.config["CLOUDFLARE_R2_BUCKET"]
                public_url_base = app.config["CLOUDFLARE_R2_PUBLIC_URL"]
                s3_folder = "veiculos" if tipo_cadastro == "veiculo" else "carretas"

                for file in files:
                    filename = secure_filename(file.filename)
                    s3_path = f"{s3_folder}/{placa}/{uuid.uuid4()}-{filename}"
                    s3_client.upload_fileobj(
                        file,
                        bucket_name,
                        s3_path,
            ...
            if tipo_cadastro == "veiculo":
                novo_veiculo = Veiculo(
                    empresa_id=current_user.empresa_id,
                    placa=placa,
                    marca=clean_string("marca"),
                    modelo=clean_string("modelo"),
                    ano_fabricacao=to_int("ano_fabricacao"),
                    ano_modelo=to_int("ano_modelo"),
                    renavam=clean_string("renavam"),
                    categoria=clean_string("categoria"),
                    status=request.form.get("status", "Disponível"),
                    cor=clean_string("cor"),
                    combustivel=clean_string("combustivel"),
                    chassi=clean_string("chassi"),
                    numero_motor=clean_string("numero_motor"),
                    potencia_cv=to_int("potencia"),
                    tanque_combustivel_litros=to_int("tanque_combustivel"),
                    consumo_medio_km_l=to_float("consumo_medio"),
                    valor_aquisicao=to_float("valor_aquisicao"),
                    data_aquisicao=to_date("data_aquisicao"),
                    km_atual=to_float("km_atual"),
                    observacoes=clean_string("observacoes"),
                    fotos_urls=",".join(anexos_urls) if anexos_urls else None,
                )
                db.session.add(novo_veiculo)
                flash("Veículo (Cavalo) cadastrado com sucesso!", "success")
                redirect_url = url_for("consultar_veiculos")

            elif tipo_cadastro == "carreta":
                nova_carreta = Carreta(
                    empresa_id=current_user.empresa_id,
                    placa=placa,
                    marca=clean_string("marca"),
                    modelo=clean_string("modelo"),
                    ano=to_int("ano_fabricacao"),  # Carreta usa o campo 'ano'
                    renavam=clean_string("renavam"),
                    eixos=to_int("eixos"),
                    status="Disponível",
                    anexos=",".join(anexos_urls) if anexos_urls else None,
                )
                db.session.add(nova_carreta)
                flash("Carreta (Implemento) cadastrada com sucesso!", "success")
                redirect_url = url_for("consultar_carretas")
            ...
            db.session.rollback()
            logger.error(f"Erro na Central de Cadastro de Frota: {e}", exc_info=True)
            flash(f"Ocorreu um erro inesperado ao salvar: {e}", "error")
            # Retorna os dados do formulário para o template em caso de erro
            return render_template("cadastrar_veiculo.html", form_data=request.form)

    # Método GET: Apenas renderiza a página
...
@master_required
def painel_gestor():
    """Renderiza a página do Painel do Gestor"""
    hoje = date.today()
    data_inicio_default = hoje.replace(day=1).strftime("%Y-%m-%d")
    ...
    return render_template(
        ...
        motoristas=motoristas,
        veiculos=veiculos,
        active_page="painel_gestor",
    )


@app.route("/api/painel_gestor/dados")
@login_required
@master_required
def api_dados_painel_gestor():
    """API que retorna todos os dados para o Painel Ultra, agora de forma refatorada."""
    try:
        # 1. Coleta e validação dos filtros (isso permanece na rota)
        data_inicio_str = request.args.get("data_inicio")
        data_fim_str = request.args.get("data_fim")
        ...
        # 2. Chama a função auxiliar para obter todos os dados de uma só vez
        dados_do_painel = _get_painel_gestor_data(
            empresa_id=current_user.empresa_id,
        ...
        # 3. Retorna os dados em formato JSON, desempacotando o dicionário
        return jsonify({"success": True, **dados_do_painel})

    except Exception as e:
        logger.error(f"Erro na API do painel gestor (refatorada): {e}", exc_info=True)
        return jsonify({"success": False, "message": str(e)}), 500


@app.route("/api/ia/generate_insights")
@login_required
@master_required
def api_generate_ia_insights():
    """
    Usa os dados consolidados do painel para gerar insights com a API do Gemini.
    """
    if not GEMINI_API_KEY:
        return jsonify(
            {
                ...
                "insights": [
                    {
                        "type": "alert",
                        "title": "IA Desativada",
                        "message": "A chave da API do Gemini não foi configurada no servidor.",
                        "priority": "high",
                    }
                ],
            }
    ...
    try:
        # 1. Coleta os mesmos filtros da requisição
        data_inicio_str = request.args.get("data_inicio")
        data_fim_str = request.args.get("data_fim")
        motorista_id = request.args.get("motorista_id", type=int)
        veiculo_id = request.args.get("veiculo_id", type=int)
        ...
        # 2. REUTILIZA A FUNÇÃO AUXILIAR para buscar os dados
        dados_do_painel = _get_painel_gestor_data(
            empresa_id=current_user.empresa_id,
            data_inicio=data_inicio,
            data_fim=data_fim,
        ...
        # 3. Seleciona apenas os dados mais relevantes para enviar à IA
        dados_para_analise = dados_do_painel.get("ai_insights_data", {})

        # 4. Cria o prompt para o Gemini
        prompt = f"""
        Você é um consultor especialista em logística e gestão de frotas para uma empresa de transportes.
        Analise os seguintes dados de performance em formato JSON e gere 4 insights acionáveis e preditivos em português do Brasil.
        Seu objetivo é identificar anomalias, oportunidades de melhoria e performances excepcionais que um gestor precisa saber.
        Seja direto, claro e foque no impacto financeiro ou operacional.

        Para cada insight, retorne um objeto JSON com os seguintes campos:
        - "type": pode ser "warning" (anomalia/ponto de atenção), "success" (performance boa/destaque), "info" (oportunidade de melhoria), ou "alert" (problema urgente).
        - "title": um título curto e impactante para o insight (ex: "Alerta de Consumo Elevado").
        - "message": uma mensagem clara e concisa explicando o insight, o porquê ele é importante e, se possível, uma sugestão prática.
        - "priority": pode ser "high", "medium", ou "low".

        Retorne APENAS um array JSON válido contendo os 4 objetos de insight, nada mais.

        DADOS PARA ANÁLISE:
        {json.dumps(dados_para_analise, indent=2, ensure_ascii=False)}
        """

        # 5. Chama a API do Gemini com o nome do modelo CORRIGIDO
        ...
        return jsonify({"success": True, "insights": insights_json})

    except Exception as e:
        logger.error(f"Erro ao gerar insights com Gemini: {e}", exc_info=True)
        return (
            jsonify(
                {"success": False, "message": f"Erro ao comunicar com a IA: {str(e)}"}
            ),
            500,
        )


# Em app.py - SUBSTITUA TODA A FUNÇÃO ANTERIOR POR ESTA VERSÃO FORMATADA:


@app.route("/api/painel_gestor/chat", methods=["POST"])
@login_required
@master_required
def api_chat_ia():
    """
    API do Chat IA - Roteador Inteligente V5: Detecta intenção de listar vs. contar.
    (Versão Completa, Formatada com Black)
    """
    if not GEMINI_API_KEY:
        return jsonify({"success": False, "resposta": "A IA não está configurada."})
    ...
    try:
        ...
        if not pergunta:
            return jsonify({"success": False, "resposta": "Mensagem vazia."}), 400

        contexto_dados = {}
        contexto_titulo = "Geral"
        resposta_direta = None

        # --- ROTEADOR INTELIGENTE V5 ---

        # 1. Perguntas "COMO FAZER" (Respostas Diretas)
        if "como" in pergunta_lower or "onde" in pergunta_lower:
            if (
                "lançar" in pergunta_lower
                or "criar" in pergunta_lower
                or "nova" in pergunta_lower
            ) and "viagem" in pergunta_lower:
                resposta_direta = "Para lançar uma nova viagem:\n1. Vá para **Viagens > Iniciar Viagem**.\n2. Preencha os detalhes (origem, destino(s), motorista, veículo, datas).\n3. Opcional: Clique em **'Calcular Rota e Custos'**.\n4. Clique em **'Criar Viagem'**."
            elif (
                "excluir" in pergunta_lower or "editar" in pergunta_lower
            ) and "viagem" in pergunta_lower:
                resposta_direta = "Para editar ou excluir uma viagem:\n1. Vá para **Viagens > Consultar Viagens**.\n2. Encontre a viagem na lista.\n3. Use os botões de **Editar** (lápis) ou **Excluir** (lixeira) na linha correspondente. Lembre-se que viagens já iniciadas ou concluídas podem ter restrições."
            elif (
                "cadastrar" in pergunta_lower or "novo" in pergunta_lower
            ) and "motorista" in pergunta_lower:
                resposta_direta = "Para cadastrar um novo motorista:\n1. Vá para **Cadastros > Motoristas > Cadastrar Motorista**.\n2. Preencha todas as informações obrigatórias nas abas.\n3. Clique em **Salvar**."
            elif "desativar" in pergunta_lower and "motorista" in pergunta_lower:
                resposta_direta = "Para desativar um motorista:\n1. Vá para **Cadastros > Motoristas > Consultar Motoristas**.\n2. Encontre o motorista e clique em **Editar**.\n3. Na aba 'Dados Pessoais', altere o campo **Situação** para 'INATIVO' ou similar.\n4. Salve as alterações."
            elif (
                "cadastrar" in pergunta_lower or "nova" in pergunta_lower
            ) and "carreta" in pergunta_lower:
                resposta_direta = "Para cadastrar uma nova carreta (implemento):\n1. Vá para **Cadastros > Frota > Cadastrar Veículo/Carreta**.\n2. Selecione a opção **'Carreta (Implemento)'**.\n3. Preencha os dados da carreta.\n4. Clique em **Salvar**."
            elif ("lançar" in pergunta_lower or "registrar" in pergunta_lower) and (
                "despesa" in pergunta_lower or "receita" in pergunta_lower
            ):
                resposta_direta = "Para lançar despesas ou receitas manualmente:\n1. Vá para **Financeiro > Fluxo de Caixa**.\n2. Clique em **'Novo Lançamento'**.\n3. Preencha os detalhes, incluindo tipo (Receita/Despesa), valor, data de vencimento, categoria, etc.\n4. Clique em **Salvar**."
            elif (
                "cadastrar" in pergunta_lower or "novo" in pergunta_lower
            ) and "cliente" in pergunta_lower:
                resposta_direta = "Para cadastrar um novo cliente:\n1. Vá para **Cadastros > Clientes > Cadastrar Cliente**.\n2. Preencha os dados do cliente (Pessoa Física ou Jurídica).\n3. Clique em **Salvar**."
            elif (
                "emitir" in pergunta_lower or "gerar" in pergunta_lower
            ) and "cobrança" in pergunta_lower:
                resposta_direta = "Para emitir uma cobrança manual (Nota de Débito):\n1. Vá para **Financeiro > Cobranças > Gerar Cobrança**.\n2. Selecione o cliente.\n3. Marque as viagens concluídas e não cobradas que deseja incluir.\n4. Defina a data de vencimento.\n5. Clique em **'Gerar Nota de Débito'**."
            elif (
                "lançar" in pergunta_lower or "nova" in pergunta_lower
            ) and "manutenção" in pergunta_lower:
                resposta_direta = "Para lançar uma nova manutenção:\n1. Vá para **Oficina > Dashboard**.\n2. Clique em **'Nova Manutenção'**.\n3. Selecione o veículo/carreta, informe o odômetro/hodômetro.\n4. Escolha o tipo (Preventiva/Corretiva). Se for Preventiva, selecione o plano.\n5. Defina o status (Agendada/Em Andamento).\n6. Clique em **'Registrar Manutenção'**."
            elif (
                ("registrar" in pergunta_lower or "nova" in pergunta_lower)
                and ("movimentação" in pergunta_lower or "movimentar" in pergunta_lower)
                and "pneu" in pergunta_lower
            ):
                resposta_direta = "Para registrar uma movimentação de pneu:\n1. Vá para **Borracharia > Dashboard**.\n2. Utilize os botões nas seções apropriadas.\n3. Siga as instruções do modal."
            elif (
                "convidar" in pergunta_lower or "novo" in pergunta_lower
            ) and "usuário" in pergunta_lower:
                resposta_direta = "Para convidar um novo usuário:\n1. Vá para **Configurações > Gerenciar Usuários e Convites**.\n2. Insira o e-mail e selecione o Papel.\n3. Clique em **'Enviar Convite'**."
            elif (
                "fechar" in pergunta_lower or "encerrar" in pergunta_lower
            ) and "folha" in pergunta_lower:
                resposta_direta = "Para fechar a folha de pagamento:\n1. Vá para **Financeiro > Folha de Pagamento**.\n2. Encontre a folha e clique em **Detalhes**.\n3. Verifique os valores e clique em **'Fechar Folha de Pagamento'**."
            # ... (Adicione outras respostas diretas aqui) ...

        if resposta_direta:
            return jsonify({"success": True, "resposta": resposta_direta})

        # --- Palavras-chave que indicam intenção de LISTAR ---
        palavras_listar = ["quais", "listar", "lista", "mostre", "ver"]
        quer_listar = any(palavra in pergunta_lower for palavra in palavras_listar)

        # 2. Perguntas sobre DADOS (Chama Funções Específicas)

        # Manutenção (Prioridade Alta)
        if (
            "manutenção" in pergunta_lower
            or "plano" in pergunta_lower
            or "insumo" in pergunta_lower
        ):
            contexto_titulo = "Manutenções e Planos"
            kwargs_manut = {}
            if quer_listar:
                kwargs_manut["listar_ativas"] = True
            if "em andamento" in pergunta_lower:
                kwargs_manut["status"] = "Em Andamento"
            elif "agendada" in pergunta_lower:
                kwargs_manut["status"] = "Agendada"
            match_placa = re.search(
                r"([a-zA-Z]{3}-?\d{4}|[a-zA-Z]{3}\d[a-zA-Z]\d{2})", pergunta
            )
            if match_placa:
                kwargs_manut["placa"] = match_placa.group(1).replace("-", "").upper()
            try:
                contexto_dados = _get_resumo_manutencoes(
                    current_user.empresa_id, **kwargs_manut
                )
            except NameError:
                contexto_dados = {
                    "aviso": "A busca de dados de Manutenções ainda não foi implementada."
                }

        # Veículos (Inclui Caminhão)
        elif (
            "veículo" in pergunta_lower
            or "carro" in pergunta_lower
            or "caminhão" in pergunta_lower
            or "placa" in pergunta_lower
            or "consumo" in pergunta_lower
            or "quilometr" in pergunta_lower
            or "km" in pergunta_lower
        ):
            contexto_titulo = "Veículos"
            kwargs_veiculo = {}
            if quer_listar:
                if (
                    "disponível" in pergunta_lower
                    or "disponiveis" in pergunta_lower
                    or "livres" in pergunta_lower
                ):
                    kwargs_veiculo["listar_disponiveis"] = True
                elif not ("manutenção" in pergunta_lower):
                    kwargs_veiculo["listar_todos_status"] = True
            ...
            placa_extraida = None
            if match_placa:
                placa_extraida = match_placa.group(1).replace("-", "").upper()
                kwargs_veiculo["placa_dados"] = placa_extraida
                kwargs_veiculo["placa"] = placa_extraida

            if "disponível" in pergunta_lower or "disponiveis" in pergunta_lower:
                kwargs_veiculo["status"] = "Disponível"
            if "em rota" in pergunta_lower or "em viagem" in pergunta_lower:
                kwargs_veiculo["status"] = "Em Rota"

            contexto_dados = _get_resumo_veiculos(
                current_user.empresa_id, **kwargs_veiculo
            )

        # Viagens
        elif "viagem" in pergunta_lower or "viagens" in pergunta_lower:
            contexto_titulo = "Viagens"
            kwargs_viagem = {}
            if quer_listar:
                if "pendente" in pergunta_lower:
                    kwargs_viagem["listar_pendentes"] = True
                elif (
                    "em andamento" in pergunta_lower
                    or "ativa" in pergunta_lower
                    or "aberta" in pergunta_lower
                ):
                    kwargs_viagem["listar_em_andamento"] = True
                elif "concluída" in pergunta_lower:
                    kwargs_viagem["listar_concluidas"] = True
                else:
                    kwargs_viagem["listar_todas"] = True

            if "pendente" in pergunta_lower:
                kwargs_viagem["status"] = "pendente"
            elif (
                "em andamento" in pergunta_lower
                or "ativa" in pergunta_lower
                or "aberta" in pergunta_lower
            ):
                kwargs_viagem["status"] = "em_andamento"
            elif "concluída" in pergunta_lower:
                kwargs_viagem["status"] = "concluida"

            match_id = re.search(r"viagem\s+(\d+)", pergunta_lower)
            if match_id:
                contexto_dados = {
                    "aviso": f"Busca de detalhes específicos da viagem #{match_id.group(1)} ainda não implementada."
                }
            else:
                match_motorista = re.search(
                    r"motorista\s+([\w\s]+)", pergunta, re.IGNORECASE
                )
                if match_motorista:
                    motorista_obj = Motorista.query.filter(
                        Motorista.empresa_id == current_user.empresa_id,
                        Motorista.nome.ilike(f"%{match_motorista.group(1).strip()}%"),
                    ).first()
                    if motorista_obj:
                        kwargs_viagem["motorista_id"] = motorista_obj.id
                match_placa = re.search(
                    r"([a-zA-Z]{3}-?\d{4}|[a-zA-Z]{3}\d[a-zA-Z]\d{2})", pergunta
                )
                if match_placa:
                    veiculo_obj = Veiculo.query.filter(
                        Veiculo.empresa_id == current_user.empresa_id,
                        Veiculo.placa.ilike(
                            f'%{match_placa.group(1).replace("-","")}%'
                        ),
                    ).first()
                    if veiculo_obj:
                        kwargs_viagem["veiculo_id"] = veiculo_obj.id
                contexto_dados = _get_resumo_viagens(
                    current_user.empresa_id, **kwargs_viagem
                )

        # Folha de Pagamento
        elif (
            "folha" in pergunta_lower
            or "salário" in pergunta_lower
            or "provento" in pergunta_lower
            or "desconto" in pergunta_lower
        ):
            contexto_titulo = "Folha de Pagamento"
            contexto_dados = {
                "aviso": "A busca detalhada na Folha de Pagamento ainda não foi implementada no chat."
            }

        # Relatórios / Painel Gestor
        elif (
            "relatório" in pergunta_lower
            or "indicador" in pergunta_lower
            or "custo médio" in pergunta_lower
            or "margem" in pergunta_lower
            or "lucrativo" in pergunta_lower
            or "eficiente" in pergunta_lower
            or "rentabilidade" in pergunta_lower
            or "resumo" in pergunta_lower
            or "painel gestor" in pergunta_lower
        ):
            contexto_titulo = "Relatórios e Indicadores (Painel Gestor)"
            hoje = date.today()
            dados_do_painel = _get_painel_gestor_data(
                empresa_id=current_user.empresa_id,
            ...
            contexto_dados = dados_do_painel.get("ai_insights_data", {})
            contexto_dados["kpis_principais_painel"] = dados_do_painel.get("kpis", {})

        # Usuários
        elif (
            "usuário" in pergunta_lower
            or "administrador" in pergunta_lower
            or "permissão" in pergunta_lower
            or "papel" in pergunta_lower
        ):
            contexto_titulo = "Usuários e Permissões"
            contexto_dados = {
                "aviso": "A busca de dados de Usuários ainda não foi implementada."
            }

        # Pneus
        elif (
            "pneu" in pergunta_lower
            or "borracharia" in pergunta_lower
            or "recapagem" in pergunta_lower
            or "sulco" in pergunta_lower
        ):
            contexto_titulo = "Pneus e Borracharia"
            contexto_dados = {
                "aviso": "A busca de dados de Pneus ainda não foi implementada."
            }

        # Cobranças / Faturamento
        elif "cobrança" in pergunta_lower or "faturado" in pergunta_lower:
            contexto_titulo = "Clientes e Cobranças"
            kwargs_cliente = {
                "calcular_faturamento": True if "faturado" in pergunta_lower else False
            }
            contexto_dados = _get_resumo_clientes(
                current_user.empresa_id, **kwargs_cliente
            )

        # Clientes (Geral)
        elif "cliente" in pergunta_lower:
            contexto_titulo = "Clientes"
            kwargs_cliente = {}
            match_nome = re.search(r"cliente\s+([\w\s./-]+)", pergunta, re.IGNORECASE)
            if match_nome:
                kwargs_cliente["nome"] = match_nome.group(1).strip()
            contexto_dados = _get_resumo_clientes(
                current_user.empresa_id, **kwargs_cliente
            )
        ...
        # Financeiro
            ...
            "financeiro" in pergunta_lower
            or "conta" in pergunta_lower
            or "pagar" in pergunta_lower
            or "receber" in pergunta_lower
            or "fluxo de caixa" in pergunta_lower
            or "saldo" in pergunta_lower
            or "despesa" in pergunta_lower
            or "receita" in pergunta_lower
            or "vencido" in pergunta_lower
            or "centro de custo" in pergunta_lower
            or "unidade de negócio" in pergunta_lower
        ):
            contexto_titulo = "Financeiro / Fluxo de Caixa"
            kwargs_financeiro = {}
            if "semana" in pergunta_lower:
                kwargs_financeiro["periodo_dias"] = 7
            elif "mês" in pergunta_lower:
                kwargs_financeiro["periodo_dias"] = 30
            if "vencido" in pergunta_lower:
                kwargs_financeiro["buscar_vencidos"] = True
            contexto_dados = _get_resumo_financeiro(
                current_user.empresa_id, **kwargs_financeiro
            )

        # Carretas
        elif "carreta" in pergunta_lower:
            contexto_titulo = "Carretas"
            kwargs_carreta = {}
            match_placa = re.search(
                r"([a-zA-Z]{3}-?\d{4}|[a-zA-Z]{3}\d[a-zA-Z]\d{2})", pergunta
            )
            if match_placa:
                kwargs_carreta["placa"] = match_placa.group(1).replace("-", "").upper()
            if "manutenção" in pergunta_lower:
                kwargs_carreta["status"] = "Em Manutenção"
            contexto_dados = {
                "aviso": "A busca de dados de Carretas ainda não foi implementada."
            }

        # Motoristas (Geral)
        elif "motorista" in pergunta_lower:
            contexto_titulo = "Motoristas"
            kwargs_motorista = {}
            match_nome = re.search(r"motorista\s+([\w\s]+)", pergunta, re.IGNORECASE)
            nome_extraido = match_nome.group(1).strip() if match_nome else None
            if "cnh" in pergunta_lower and nome_extraido:
                kwargs_motorista["nome_cnh"] = nome_extraido
            elif nome_extraido:
                kwargs_motorista["nome"] = nome_extraido
            if "ativo" in pergunta_lower:
                kwargs_motorista["status"] = "ativo"
            contexto_dados = _get_resumo_motoristas(
                current_user.empresa_id, **kwargs_motorista
            )

        # Fallback para o Painel Gestor
        else:
            contexto_titulo = "Geral do Painel Gestor (Mês Atual)"
            hoje = date.today()
            dados_do_painel = _get_painel_gestor_data(
                empresa_id=current_user.empresa_id,
                data_inicio=hoje.replace(day=1),
        ...
        prompt = f"""
        Você é um assistente especialista do sistema de gestão de frotas TrackBras.
        Responda à pergunta do usuário usando os dados de contexto fornecidos.
        Se os dados de contexto contiverem um "aviso", informe ao usuário que você ainda não tem acesso a essa área específica.
        **Se o contexto incluir uma lista (ex: 'lista_disponiveis', 'lista_manutencoes_ativas_agendadas', 'lista_em_andamento'), use essa lista para responder perguntas sobre "quais" ou "listar". Caso contrário, use as contagens.**
        Seja claro, direto e use Markdown (ex: **negrito**). Responda em português do Brasil.
        Se a pergunta for ambígua, peça mais detalhes.

        ---
        CONTEXTO ATUAL (Foco em: {contexto_titulo}):
        {json.dumps(contexto_dados, indent=2, ensure_ascii=False, default=str)} 
        ---
        ...
        model = genai.GenerativeModel(
            "gemini-2.0-flash"
        )  # Usando o modelo que funciona
        response = model.generate_content(prompt)

        return jsonify({"success": True, "resposta": response.text})

    except Exception as e:
        logger.error(f"Erro no chat com Gemini V5: {e}", exc_info=True)
        return (
            jsonify(
                {
                    "success": False,
                    "resposta": f"Desculpe, ocorreu um erro na IA: {str(e)}",
                }
            ),
            500,
        )
...
@app.route("/api/painel_gestor/export-pdf", methods=["POST"])
@login_required
@master_required
def api_export_pdf():
    ...
    try:
        ...
        hoje = date.today()
        data_inicio = hoje.replace(day=1)
        data_fim_datetime = datetime.combine(hoje, datetime.max.time())

        viagens_mes = Viagem.query.filter(
            Viagem.empresa_id == current_user.empresa_id,
            Viagem.status == "concluida",
            Viagem.data_fim >= data_inicio,
            Viagem.data_fim <= data_fim_datetime,
        ...
        receita_mes = sum(getattr(v, "valor_recebido", 0) or 0 for v in viagens_mes)
        custo_mes = sum(getattr(v, "custo_real_completo", 0) or 0 for v in viagens_mes)
        distancia_total = sum(
            getattr(v, "distancia_percorrida", 0) or 0 for v in viagens_mes
        )
        ...
        elements.append(
            Paragraph(
                f"Relatório do Painel Gestor - {hoje.strftime('%B de %Y')}", title_style
            )
        )
        ...
        kpi_data = [
            ["Métrica", "Valor"],
            ["Total de Viagens", str(len(viagens_mes))],
            ["Receita Total", f"R$ {receita_mes:,.2f}"],
            ["Custo Total", f"R$ {custo_mes:,.2f}"],
            ["Lucro", f"R$ {receita_mes - custo_mes:,.2f}"],
            ["Distância Total", f"{int(distancia_total)} km"],
            [
                "Custo/KM",
                f"R$ {(custo_mes/distancia_total) if distancia_total > 0 else 0:.2f}",
            ],
            [
                "Margem de Lucro",
                f"{((receita_mes - custo_mes) / receita_mes * 100) if receita_mes > 0 else 0:.1f}%",
            ],
        ]
        ...
        return send_file(
            ...
            mimetype="application/pdf",
            as_attachment=True,
            download_name=f"painel_gestor_{hoje.strftime('%Y-%m-%d')}.pdf",
        )
...
@app.route("/api/veiculo/<int:veiculo_id>/kpis")
@login_required
def get_veiculo_kpis_api(veiculo_id):
    veiculo = Veiculo.query.filter_by(
        id=veiculo_id, empresa_id=current_user.empresa_id
    ).first_or_404()

    viagens = (
        Viagem.query.filter_by(veiculo_id=veiculo.id)
        .options(
            db.joinedload(Viagem.custo_viagem), db.joinedload(Viagem.abastecimentos)
        )
        .all()
    ...
    total_km = veiculo.km_atual or 0.0
    ...
    return jsonify({"success": True, "kpis": stats})


# API para fazer upload de um anexo (foto) para o veículo
@app.route("/api/veiculo/<int:veiculo_id>/upload_anexo", methods=["POST"])
@login_required
def api_upload_anexo_veiculo(veiculo_id):
    veiculo = Veiculo.query.filter_by(
        id=veiculo_id, empresa_id=current_user.empresa_id
    ).first_or_404()
    ...
    try:
        ...
        fotos_atuais.append(file_url)
        veiculo.fotos_urls = ",".join(filter(None, fotos_atuais))
        db.session.commit()
        ...
        db.session.rollback()
        logger.error(
            f"Erro no upload de anexo para veículo {veiculo_id}: {e}", exc_info=True
        )
        return jsonify({"success": False, "message": f"Erro no servidor: {e}"}), 500


# API para excluir um anexo (foto) do veículo
@app.route("/api/veiculo/excluir_foto", methods=["POST"])
@login_required
def api_excluir_anexo_veiculo():
    data = request.get_json()
    veiculo_id = data.get("veiculo_id")
    foto_url = data.get("foto_url")

    if not veiculo_id or not foto_url:
        return jsonify({"success": False, "message": "Dados incompletos."}), 400
    ...
    fotos_atuais = veiculo.fotos_urls.split(",") if veiculo.fotos_urls else []
    if foto_url not in fotos_atuais:
        return jsonify({"success": False, "message": "Anexo não encontrado."}), 404
    ...
    try:
        ...
        bucket_name = app.config["CLOUDFLARE_R2_BUCKET"]
        public_url_base = app.config["CLOUDFLARE_R2_PUBLIC_URL"]
        key = foto_url.replace(f"{public_url_base}/", "")
        s3_client.delete_object(Bucket=bucket_name, Key=key)

        fotos_atuais.remove(foto_url)
        veiculo.fotos_urls = ",".join(fotos_atuais) or None
        db.session.commit()
        ...
        db.session.rollback()
        logger.error(
            f"Erro ao excluir anexo do veículo {veiculo_id}: {e}", exc_info=True
        )
        return (
            jsonify({"success": False, "message": f"Erro ao remover anexo: {e}"}),
            500,
...
def consultar_veiculos():
    search_query = request.args.get("search", "").strip()
    query = Veiculo.query.filter_by(
        empresa_id=current_user.empresa_id, is_administrativo=False
    )
    ...
    veiculos_json_para_js = [v.to_dict() for v in veiculos_obj]

    # 3. Passa AMBAS as variáveis para o template com nomes diferentes
    return render_template(
        "consultar_veiculos.html",
        veiculos=veiculos_obj,  # Para a tabela (objetos)
        veiculos_json=veiculos_json_para_js,  # Para o JavaScript (JSON)
        search_query=search_query,
...
def editar_veiculo(veiculo_id):
    ...
    if request.method == "POST":
        try:
            # A lógica de upload de novas fotos foi movida para a API assíncrona
            # e não é mais necessária aqui no POST do formulário principal.
            ...
            veiculo.motorista_padrao_id = to_int(
                request.form.get("motorista_padrao_id")
            )
            ...
            # Mesmo em caso de erro, recarregamos a página com os dados necessários
            return render_template(
                "editar_veiculo.html",
                veiculo=veiculo,
                motoristas=motoristas,
    ...
    # --- CORREÇÃO PRINCIPAL NO MÉTODO GET ---
    # Gera o token CSRF e o passa para o template
    return render_template(
        "editar_veiculo.html",
        veiculo=veiculo,
        motoristas=motoristas,
...
def processar_nfe_api():
    ...
    for file in files:
        if file and file.filename.endswith(".xml"):
            ...
            try:
                ...
                db.session.rollback()
                logger.error(
                    f"Erro ao criar cliente a partir do XML {viagem_info['nome_arquivo']}: {e}",
                    exc_info=True,
                )
                continue
    ...
    if not viagens_processadas:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Nenhum dado de viagem pôde ser extraído dos arquivos XML fornecidos.",
                }
            ),
            400,
        )
    ...
    return jsonify(
        {"success": True, "viagens": viagens_processadas, "message": mensagem_sucesso}
    )
    ...
    try:
        if item_id.startswith("manual_"):
            ...
            lancamento = LancamentoFluxoCaixa.query.filter_by(
                id=lancamento_id, empresa_id=empresa_id
            ).first()
            ...
            data_criacao_field = (
                getattr(lancamento, "data_criacao", None)
                or getattr(lancamento, "created_at", None)
                or getattr(lancamento, "data_cadastro", None)
                or getattr(lancamento, "data_lancamento", None)
            )

            data_atualizacao_field = (
                getattr(lancamento, "data_atualizacao", None)
                or getattr(lancamento, "updated_at", None)
                or getattr(lancamento, "data_modificacao", None)
            )
            ...
            dados_lancamento = {
                ...
                "valor_formatado": f"{lancamento.valor_total:,.2f}".replace(",", "X")
                .replace(".", ",")
                .replace("X", "."),
                "data_emissao": (
                    data_criacao_field.strftime("%d/%m/%Y")
                    if data_criacao_field
                    else None
                ),
                "data_vencimento": lancamento.data_vencimento.strftime("%d/%m/%Y"),
                "data_pagamento": (
                    lancamento.data_pagamento.strftime("%d/%m/%Y")
                    if lancamento.data_pagamento
                    else None
                ),
                "data_criacao": (
                    data_criacao_field.strftime("%d/%m/%Y %H:%M")
                    if data_criacao_field
                    else None
                ),
                "data_atualizacao": (
                    data_atualizacao_field.strftime("%d/%m/%Y %H:%M")
                    if data_atualizacao_field
                    else None
                ),
                ...
                "parcela": (
                    f"{parcela_numero}/{parcela_total}" if parcela_total > 1 else None
                ),
            ...
            numero_nfe = (
                lancamento.chave_acesso[25:34]
                if len(lancamento.chave_acesso) >= 34
                else lancamento.chave_acesso[-9:]
            )
            ...
            dados_lancamento = {
                ...
                .replace("X", "."),
                "data_emissao": (
                    lancamento.data_emissao.strftime("%d/%m/%Y")
                    if lancamento.data_emissao
                    else None
                ),
                "data_vencimento": lancamento.data_vencimento.strftime("%d/%m/%Y"),
                "data_pagamento": (
                ...
                "data_criacao": (
                    lancamento.data_lancamento.strftime("%d/%m/%Y %H:%M")
...
def api_detalhes_lancamento(item_id):
    ...
    try:
        ...
        nfe_original = None
        is_nfe = lancamento.documento_numero and lancamento.documento_numero.startswith(
            "NFE-"
        )
        if is_nfe:
            chave_acesso_base = lancamento.documento_numero.split("-")[1]
            nfe_original = NFeImportada.query.filter_by(
                chave_acesso=chave_acesso_base, empresa_id=current_user.empresa_id
            ).first()
        ...
        dados_lancamento = {
            ...
            .replace("X", "."),
            "data_emissao": (
                nfe_original.data_emissao.strftime("%d/%m/%Y")
                if nfe_original
                else lancamento.data_lancamento.strftime("%d/%m/%Y")
            ),
            "data_vencimento": lancamento.data_vencimento.strftime("%d/%m/%Y"),
            "data_pagamento": (
                lancamento.data_pagamento.strftime("%d/%m/%Y")
            ...
            "documento": (
                nfe_original.chave_acesso
                if nfe_original
                else lancamento.documento_numero
            ),
            "status": lancamento.status_pagamento,
            "meio_pagamento": lancamento.meio_pagamento,
            "parcela": (
                f"{lancamento.parcela_numero}/{lancamento.parcela_total}"
                if lancamento.parcela_total and lancamento.parcela_total > 1
                else None
            ),
        ...
        logger.error(
            f"Erro ao buscar detalhes do lançamento {item_id}: {e}", exc_info=True
        )
        return (
            jsonify(
                {
                    "success": False,
                    "message": f"Erro interno ao buscar detalhes: {str(e)}",
                }
            ),
            500,
        )
...
def importar_nfe_page():
    ...
    return render_template(
        "importar_nfe.html", motoristas=motoristas, veiculos=veiculos
    )
...
def get_motorista_details_api(motorista_id):
    """
    API completa para buscar todos os dados necessários para o modal de detalhes do motorista.
    """
    motorista = Motorista.query.filter_by(
        id=motorista_id, empresa_id=current_user.empresa_id
    ).first_or_404()

    # 1. Histórico de Atendimento (Viagens)
    viagens = (
        Viagem.query.filter(Viagem.motorista_id == motorista.id)
        .options(
            db.joinedload(
                Viagem.veiculo
            )  # Carrega dados do veículo para evitar mais queries
        )
        .order_by(Viagem.data_inicio.desc())
        .all()
    )
    ...
    for v in viagens:
        atendimentos_data.append(
            {
                "data_viagem": v.data_inicio.strftime("%d/%m/%Y %H:%M"),
                "cliente": v.cliente,
                "veiculo_placa": v.veiculo.placa if v.veiculo else "N/A",
    ...
    # 2. Histórico de Pagamentos (Itens da Folha de Pagamento)
    itens_pagamento = (
        ItemFolhaPagamento.query.join(FolhaPagamento)
        .filter(
            FolhaPagamento.motorista_id == motorista_id,
            ItemFolhaPagamento.viagem_id.isnot(
                None
            ),  # Filtra apenas pagamentos ligados a viagens
        )
        .options(db.joinedload(ItemFolhaPagamento.folha))  # Carrega dados da folha
        .order_by(FolhaPagamento.data_pagamento.desc())
        .all()
    )
    ...
    for item in itens_pagamento:
        pagamentos_data.append(
            {
                "data_pagamento": (
                    item.folha.data_pagamento.strftime("%d/%m/%Y")
                    if item.folha.data_pagamento
                    else "Não Pago"
                ),
                "descricao": item.descricao,
                "valor": item.valor,
                "status_folha": item.folha.status,
                "viagem_id": item.viagem_id,
                "folha_id": item.folha_pagamento_id,
            }
        )

    # Adiciona o salário base também
    folhas_com_salario = FolhaPagamento.query.filter(
        FolhaPagamento.motorista_id == motorista_id,
        FolhaPagamento.salario_base_registro > 0,
    ).all()
    for folha in folhas_com_salario:
        pagamentos_data.append(
            {
                "data_pagamento": (
                    folha.data_pagamento.strftime("%d/%m/%Y")
                    if folha.data_pagamento
                    else "Não Pago"
                ),
                "descricao": f"Salário Base (Ref. {folha.mes_referencia}/{folha.ano_referencia})",
                "valor": folha.salario_base_registro,
                "status_folha": folha.status,
                "viagem_id": None,  # Salário não está atrelado a uma viagem específica
                "folha_id": folha.id,
            }
        )

    # Ordena a lista final de pagamentos por data
    pagamentos_data.sort(key=lambda x: x["data_pagamento"], reverse=True)

    # 3. Anexos
    anexos_data = motorista.anexos.split(",") if motorista.anexos else []
...
@app.route("/api/motorista/<int:motorista_id>/kpis")
@login_required
def get_motorista_kpis_api(motorista_id):
    motorista = Motorista.query.filter_by(
        id=motorista_id, empresa_id=current_user.empresa_id
    ).first_or_404()

    viagens = Viagem.query.filter(Viagem.motorista_id == motorista.id).all()

    total_receita = sum(v.valor_recebido or 0 for v in viagens)
    total_km = sum(
        v.distancia_percorrida or 0 for v in viagens if v.distancia_percorrida
    )

    stats = {
        "total_viagens": len(viagens),
        "total_distancia": round(total_km, 2),
        "total_receita": round(total_receita, 2),
    }
    return jsonify({"success": True, "kpis": stats})


@app.route("/api/motorista/<int:motorista_id>/upload_anexo", methods=["POST"])
@login_required
def api_upload_anexo_motorista(motorista_id):
    motorista = Motorista.query.filter_by(
        id=motorista_id, empresa_id=current_user.empresa_id
    ).first_or_404()
...
@app.route("/api/motorista/excluir_anexo", methods=["POST"])
@login_required
def api_excluir_anexo_motorista():
    data = request.get_json()
    motorista_id = data.get("motorista_id")
    anexo_url = data.get("anexo_url")

    if not motorista_id or not anexo_url:
        return jsonify({"success": False, "message": "Dados incompletos."}), 400
    ...
    try:
        ...
        anexos_atuais.remove(anexo_url)
        motorista.anexos = ",".join(anexos_atuais) or None
        db.session.commit()
        ...
        db.session.rollback()
        logger.error(
            f"Erro ao excluir anexo do motorista {motorista_id}: {e}", exc_info=True
        )
        return (
            jsonify({"success": False, "message": f"Erro ao remover anexo: {e}"}),
            500,
...
def criar_viagem_api():
    """API principal que recebe dados do formulário e cria a viagem no banco."""
    try:
        data = request.get_json()
        ...
        for campo in campos_obrigatorios:
            if not data.get(campo):
                erros[campo] = f"{campo} é obrigatório"

        # Validar destinos
        destinos = data.get("enderecos_destino", [])
        if not destinos or len(destinos) == 0:
            erros["enderecos_destino"] = "Pelo menos um destino é obrigatório"

        if erros:
            return (
                jsonify(
                    {"success": False, "message": "Erro na validação", "fields": erros}
                ),
                400,
            )
        ...
        # ========== VALIDAR MOTORISTA ==========
        motorista = Motorista.query.filter_by(
            id=int(data["motorista_id"]), empresa_id=current_user.empresa_id
        ).first()
        if not motorista:
            return (
                jsonify({"success": False, "message": "Motorista não encontrado"}),
                404,
            )

        # ========== VALIDAR VEÍCULO ==========
        veiculo = Veiculo.query.filter_by(
            id=int(data["veiculo_id"]),
            empresa_id=current_user.empresa_id,
            status="Disponível",
        ).first()
        if not veiculo:
            return jsonify({"success": False, "message": "Veículo não disponível"}), 404

        # ========== VALIDAR CARRETA (SE FORNECIDA) ==========
        carreta_id = None
        if data.get("carreta_id"):
            carreta = Carreta.query.filter_by(
                id=int(data["carreta_id"]),
                empresa_id=current_user.empresa_id,
                status="Disponível",
            ).first()
            if not carreta:
                return (
                    jsonify({"success": False, "message": "Carreta não disponível"}),
                    404,
                )
            carreta_id = carreta.id

        # ========== CALCULAR ROTA COM DISTÂNCIA REAL ==========
        todos_enderecos = [data["endereco_saida"]] + destinos
        rota_otimizada, distancia_km, duracao_segundos, geometria, erro_rota = (
            calcular_rota_otimizada_ors(todos_enderecos)
        )
        ...
        if erro_rota:
            return (
                jsonify(
                    {"success": False, "message": f"Erro ao calcular rota: {erro_rota}"}
                ),
                400,
            )

        # ========== CALCULAR CUSTOS BASEADO EM DISTÂNCIA REAL ==========
        CUSTO_COMBUSTIVEL_KM = 0.80
        CUSTO_MOTORISTA_KM = 0.50
        CUSTO_DESGASTE_KM = 0.30

        custo_combustivel = distancia_km * CUSTO_COMBUSTIVEL_KM
        custo_motorista_base = distancia_km * CUSTO_MOTORISTA_KM
        custo_desgaste = distancia_km * CUSTO_DESGASTE_KM

        # ========== DADOS DE FRETE ==========
        dados_frete = data.get("dados_frete", {})
        custo_frete = 0
        peso_toneladas = None
        material_transportado = None
        custo_motorista_variavel = None

        if dados_frete:
            peso_toneladas = float(dados_frete.get("peso_toneladas") or 0)
            valor_por_tonelada = float(dados_frete.get("valor_por_tonelada") or 0)
            material_transportado = dados_frete.get("material")
            custo_motorista_variavel = float(
                dados_frete.get("pagamento_motorista_frete") or 0
            )
            custo_frete = custo_motorista_variavel

        # ========== DESPESAS PERSONALIZADAS ==========
        despesas_personalizadas = data.get("despesas_personalizadas", [])
        total_despesas_custom = sum(
            float(d.get("valor", 0)) for d in despesas_personalizadas
        )

        # ========== CÁLCULO TOTAL DE CUSTOS ==========
        custo_total_automatico = (
            custo_combustivel + custo_motorista_base + custo_desgaste + custo_frete
        )
        custo_total = custo_total_automatico + total_despesas_custom
        valor_recebido = float(data.get("valor_recebido") or 0)
        lucro_estimado = valor_recebido - custo_total
        ...
        nova_viagem = Viagem(
            ...
            custo_motorista_variavel=custo_motorista_variavel,
        )
        ...
        # ========== SALVAR DESPESAS PERSONALIZADAS ==========
        for despesa in despesas_personalizadas:
            if despesa.get("descricao") and float(despesa.get("valor", 0)) > 0:
                # Se tiver modelo DespesaViagem, descomente:
                # despesa_obj = DespesaViagem(
                #     viagem_id=nova_viagem.id,
                #     descricao=despesa.get('descricao'),
                #     valor=float(despesa.get('valor'))
                # )
                # db.session.add(despesa_obj)
                pass
            ...
            jsonify(
                {
                    ...
                    "custo_estimado": f"{custo_total:.2f}".replace(".", ","),
                    "lucro_estimado": f"{lucro_estimado:.2f}".replace(".", ","),
                    "roteiro": rota_otimizada,
                    "custos_detalhes": {
                        "combustivel": f"{custo_combustivel:.2f}",
                        "motorista_base": f"{custo_motorista_base:.2f}",
                        "desgaste": f"{custo_desgaste:.2f}",
                        "frete": f"{custo_frete:.2f}",
                        "despesas_custom": f"{total_despesas_custom:.2f}",
                    },
                }
            ),
            201,
        ...
        logger.error(f"Erro de conversão: {e}", exc_info=True)
        return (
            jsonify({"success": False, "message": "Erro ao processar dados numéricos"}),
            400,
        )
        ...
        return (
            jsonify({"success": False, "message": "Erro interno ao criar viagem"}),
            500,
        )


@app.route("/api/viagem/calcular-estimativa", methods=["POST"])
@login_required
def calcular_estimativa_api():
    """
    API para calcular estimativa com distância REAL antes de confirmar.
    Substitui o botão 'Calcular Estimativa' que usava distância fixa de 150km.
    """
    try:
        data = request.get_json()
        todos_enderecos = [data.get("endereco_saida")] + data.get(
            "enderecos_destino", []
        )

        if len(todos_enderecos) < 2:
            return (
                jsonify(
                    {"success": False, "message": "Origem e destino são obrigatórios"}
                ),
                400,
            )

        # ========== CALCULAR ROTA REAL COM ORS ==========
        _, distancia_km, duracao_segundos, _, erro = calcular_rota_otimizada_ors(
            todos_enderecos
        )
        ...
        if erro:
            return (
                jsonify(
                    {"success": False, "message": f"Erro ao calcular rota: {erro}"}
                ),
                400,
            )

        # ========== CALCULAR CUSTOS COM DISTÂNCIA REAL ==========
        CUSTO_COMBUSTIVEL_KM = 0.80
        CUSTO_MOTORISTA_KM = 0.50
        CUSTO_DESGASTE_KM = 0.30

        custos = {
            "combustivel": round(distancia_km * CUSTO_COMBUSTIVEL_KM, 2),
            "motorista_base": round(distancia_km * CUSTO_MOTORISTA_KM, 2),
            "desgaste": round(distancia_km * CUSTO_DESGASTE_KM, 2),
            "frete": 0,
        }
        ...
        dados_frete = data.get("dados_frete", {})
        if dados_frete:
            peso = float(dados_frete.get("peso_toneladas") or 0)
            valor_por_ton = float(dados_frete.get("valor_por_tonelada") or 0)
            custos["frete"] = round(peso * valor_por_ton, 2)

        custos["total_automatico"] = round(sum(custos.values()), 2)
        ...
        logger.error(f"Erro ao calcular estimativa: {e}", exc_info=True)
        return (
            jsonify({"success": False, "message": "Erro ao calcular estimativa"}),
            500,
        )
...
def importar_notas_fiscais():
    ...
    if request.method == "GET":
        ...
        notas_importadas = (
            NFeImportada.query.filter_by(empresa_id=current_user.empresa_id)
            .order_by(NFeImportada.data_emissao.desc())
            .all()
        )
...
@app.route(
    "/financeiro/fluxo_caixa/editar/<int:lancamento_id>", methods=["GET", "POST"]
)
@login_required
@master_required
def editar_lancamento_fluxo(lancamento_id):
    ...
    lancamento = LancamentoFluxoCaixa.query.filter_by(
        id=lancamento_id, empresa_id=current_user.empresa_id
    ).first_or_404()
    ...
    if request.method == "POST":
        try:
            ...
            lancamento.data_vencimento = datetime.strptime(
            ...
            logger.error(
                f"Erro ao editar lançamento {lancamento_id}: {e}", exc_info=True
            )
...
def api_excluir_lancamento_fluxo():
    ...
    if not item_id or not item_id.startswith("manual_"):
        return (
            jsonify(
                {
                    "success": False,
                    "message": "ID do item inválido ou não é um lançamento manual.",
                }
            ),
            400,
        )
    ...
    try:
        ...
        if lancamento_clicado.status_pagamento == "PAGO":
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Não é possível excluir lançamentos já pagos.",
                    }
                ),
                400,
            )
        ...
        is_nfe_related = (
            lancamento_clicado.documento_numero
            and lancamento_clicado.documento_numero.startswith("NFE-")
        )
        ...
        if (
            is_nfe_related
            and lancamento_clicado.observacoes
            and "Chave de acesso: " in lancamento_clicado.observacoes
        ):
        ...
        if is_nfe_related:
            return jsonify(
                {
                    "success": True,
                    "message": "Lançamento(s) e seu rateio foram excluídos! A nota fiscal foi reabilitada.",
                }
            )
        else:
            return jsonify(
                {
                    "success": True,
                    "message": "Lançamento manual e seu rateio foram excluídos com sucesso!",
                }
            )
...
def api_salvar_rateio():
    data = request.get_json()
    item_id_str = data.get("item_id", "")
    destinos_data = data.get(
        "destinos", []
    )  # Recebe a lista de destinos (veiculos/carretas)
    ...
    # Busca todas as parcelas que pertencem ao mesmo grupo do lançamento clicado
    lancamentos_do_grupo = []
    if lancamento_clicado.documento_numero and (
        lancamento_clicado.documento_numero.startswith("NFE-")
        or lancamento_clicado.documento_numero.startswith("MANUAL-")
    ...
    try:
            ...
            if lancamento.status_pagamento == "PAGO":
                ...
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": "Não é possível alterar o rateio de um lançamento com parcelas já pagas.",
                        }
                    ),
                    400,
                )

            # Exclui o rateio antigo desta parcela para recriá-lo
            RateioCusto.query.filter_by(lancamento_id=lancamento.id).delete()

            if destinos_data:
                for destino in destinos_data:
                    # **AQUI ESTÁ A NOVA LÓGICA**
                    # Separa o tipo e o id (ex: 'carreta_5' vira 'carreta' e '5')
                    tipo_destino, id_destino = destino["id"].split("_")

                    percentual = float(destino["percentual"])
                    valor_rateado_parcela = lancamento.valor_total * (percentual / 100)

                    # Cria o novo registro de rateio usando o modelo RateioCusto
                    novo_rateio = RateioCusto(
                        lancamento_id=lancamento.id,
                        rateavel_id=int(id_destino),  # Salva o ID
                        rateavel_type=tipo_destino,  # Salva o TIPO ('veiculo' ou 'carreta')
        ...
        return jsonify(
            {
                "success": True,
                "message": "Rateio salvo com sucesso para todas as parcelas!",
            }
        )
        ...
        db.session.rollback()
        logger.error(
            f"Erro ao salvar rateio para o grupo do lançamento {lancamento_id_clicado}: {e}",
            exc_info=True,
        )
        return jsonify({"success": False, "message": f"Erro interno: {str(e)}"}), 500
...
def api_novo_lancamento_completo():
    ...
    try:
        ...
        data_emissao = (
            datetime.strptime(data_emissao_str, "%Y-%m-%d").date()
            if data_emissao_str
            else date.today()
        )
        # ▲▲▲ FIM DO TRECHO ▲▲▲
        ...
        if not parcelas_data:
            ...
            parcelas_data = [
                {
                    "data_vencimento": data.get(
                        "data_vencimento"
                    ),  # Supondo que venha no corpo se não for parcelado
            ...
            if abs(soma_rateios - valor_total_nota) > 0.01:
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": "A soma dos valores do rateio não confere com o valor total do lançamento.",
                        }
                    ),
                    400,
                )
        ...
        for i, parcela_info in enumerate(parcelas_data, 1):
            ...
            data_vencimento_parcela = datetime.strptime(
                parcela_info["data_vencimento"], "%Y-%m-%d"
            ).date()
            ...
            novo_lancamento = LancamentoFluxoCaixa(
                ...
                tipo=data["tipo"],
                descricao=(
                    f"{data['descricao']} - Parcela {i}/{len(parcelas_data)}"
                    if len(parcelas_data) > 1
                    else data["descricao"]
                ),
            ...
            if tem_rateio:
                for rateio_info in rateios_data:
                    percentual_rateio = (
                        float(rateio_info["valor"]) / valor_total_nota
                    ) * 100
                    valor_rateado_para_parcela = valor_parcela * (
        ...
        return jsonify(
            {
                "success": True,
                "message": f"Lançamento criado com sucesso em {len(parcelas_data)} parcela(s)!",
            }
        )
...
def api_buscar_rateio(item_id):
    ...
    try:
        ...
        if lancamento_clicado.tipo != "DESPESA":
            return (
                jsonify(
                    {"success": False, "message": "Rateio só é aplicável a despesas."}
                ),
                400,
            )
        ...
        primeiro_lancamento_com_rateio = next(
            (l for l in lancamentos_do_grupo if l.tem_rateio), None
        )

        veiculos_data = []
        if primeiro_lancamento_com_rateio:
            rateios_db = RateioVeiculo.query.filter_by(
                lancamento_id=primeiro_lancamento_com_rateio.id
            ).all()
            for r in rateios_db:
        ...
        logger.error(
            f"Erro ao buscar dados de rateio para {item_id}: {e}", exc_info=True
        )
        return jsonify({"success": False, "message": f"Erro interno: {str(e)}"}), 500
...
def api_novo_lancamento_com_rateio():
    ...
    try:
        ...
        data_vencimento = datetime.strptime(
            data.get("data_vencimento"), "%Y-%m-%d"
        ).date()
        ...
        if parcelado and parcelas_data:
            ...
            for i, parcela in enumerate(parcelas_data, 1):
                novo_lancamento = LancamentoFluxoCaixa(
                    ...
                    data_vencimento=datetime.strptime(
                        parcela.get("data_vencimento"), "%Y-%m-%d"
                    ).date(),
                    data_pagamento=data_pagamento,
        ...
        if tem_rateio and veiculos_rateio:
            ...
            lancamentos = (
                [novo_lancamento]
                if not parcelado
                else db.session.query(LancamentoFluxoCaixa)
                .filter(
                    LancamentoFluxoCaixa.empresa_id == current_user.empresa_id,
            ...
            for lancamento in lancamentos:
                for veiculo in veiculos_rateio:
                    valor_rateado = lancamento.valor_total * (
                        float(veiculo["percentual"]) / 100
                    )
...
def api_exportar_relatorio_rateio():
    ...
    try:
        ...
        # Query base
        query = (
            db.session.query(LancamentoFluxoCaixa, RateioVeiculo, Veiculo)
            .join(RateioVeiculo, LancamentoFluxoCaixa.id == RateioVeiculo.lancamento_id)
            .join(Veiculo, RateioVeiculo.veiculo_id == Veiculo.id)
            .filter(
                LancamentoFluxoCaixa.empresa_id == current_user.empresa_id,
        ...
        if data_inicio:
            query = query.filter(
                LancamentoFluxoCaixa.data_vencimento
        ...
        return send_file(
            output,
            as_attachment=True,
            download_name=f'relatorio_rateio_{datetime.now(UTC).strftime("%Y%m%d_%H%M%S")}.xlsx',
            mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )
        ...
        if item_id.startswith("manual_"):
            ...
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Lançamentos de NFe não podem ser editados. Use a tela de importação fiscal para reprocessar.",
                    }
                ),
                400,
            )
...
def api_estornar_pagamento():
    ...
    if item_id_str.startswith("manual_"):
        ...
        return (
            jsonify({"success": False, "message": "Tipo de lançamento inválido."}),
            400,
        )
...
def api_salvar_edicao_lancamento():
    ...
    if not lancamento_id:
        return (
            jsonify({"success": False, "message": "ID do lançamento é obrigatório."}),
            400,
        )
    ...
    if lancamento.status_pagamento == "PAGO":
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Não é possível editar lançamentos pagos.",
                }
            ),
            400,
        )
    ...
    try:
        ...
        db.session.commit()
        return jsonify(
            {"success": True, "message": "Lançamento atualizado com sucesso!"}
        )
...
def api_duplicar_lancamento():
    ...
    if not original or original.empresa_id != current_user.empresa_id:
        return (
            jsonify(
                {"success": False, "message": "Lançamento original não encontrado."}
            ),
            404,
        )
    ...
    novo_lancamento = LancamentoFluxoCaixa(
        ...
        data_vencimento=datetime.strptime(
            data["nova_data_vencimento"], "%Y-%m-%d"
        ).date(),
...
def api_alterar_categoria_em_massa():
    ...
    return jsonify(
        {
            "success": True,
            "message": f"{atualizados} item(s) tiveram a categoria alterada.",
        }
    )
...
def relatorio_analitico_fluxo():
    ...
    try:
        ...
        analise_categorias = {}
        analise_fornecedores = defaultdict(
            lambda: {"receitas": 0, "despesas": 0, "saldo": 0}
        )
        ...
        for lanc in lancamentos_manuais:
            ...
            if lanc.tipo == "RECEITA":
                ...
                if lanc.fornecedor_cliente:
                    analise_fornecedores[lanc.fornecedor_cliente][
                        "receitas"
                    ] += lanc.valor_total
            else:
            ...
            analise_categorias[categoria]["total"] = (
                analise_categorias[categoria]["receitas"]
                - analise_categorias[categoria]["despesas"]
            )
            ...
            analise_mensal[mes_ano]["saldo"] = (
                analise_mensal[mes_ano]["receitas"]
                - analise_mensal[mes_ano]["despesas"]
            )
        ...
        return render_template(
            "relatorio_analitico_fluxo.html",
...
def preparar_cte_de_viagem_nova(viagem_id):
    ...
    try:
        ...
        viagem = (
            Viagem.query.filter_by(id=viagem_id, empresa_id=current_user.empresa_id)
            .options(
                db.joinedload(Viagem.motorista_formal), db.joinedload(Viagem.veiculo)
            )
            .first_or_404()
        )
        ...
        cliente_destinatario = Cliente.query.filter_by(
            nome_razao_social=viagem.cliente, empresa_id=current_user.empresa_id
        ).first()
...
def api_exportar_fluxo_excel():
    ...
    try:
        ...
        query = LancamentoFluxoCaixa.query.options(
            ...
            db.joinedload(LancamentoFluxoCaixa.rateios).joinedload(
                RateioVeiculo.veiculo
            ),
        ...
        if data_emissao_inicio:
            query = query.filter(
                LancamentoFluxoCaixa.data_lancamento
                >= datetime.strptime(data_emissao_inicio, "%Y-%m-%d")
            )
        if data_emissao_fim:
            query = query.filter(
                LancamentoFluxoCaixa.data_lancamento
                <= datetime.strptime(data_emissao_fim, "%Y-%m-%d")
            )
        if categoria_filtro:
            query = query.filter(
                LancamentoFluxoCaixa.categoria.ilike(f"%{categoria_filtro}%")
            )
        if status_filtro:
            status_pagos = ["PAGO", "Pago"]
            if status_filtro == "PAGO":
                query = query.filter(
                    LancamentoFluxoCaixa.status_pagamento.in_(status_pagos)
                )
            else:
                query = query.filter(
                    LancamentoFluxoCaixa.status_pagamento.notin_(status_pagos)
                )
        if tipo_filtro:
            query = query.filter(LancamentoFluxoCaixa.tipo == tipo_filtro)
        if meio_pagamento_filtro:
            query = query.filter(
                LancamentoFluxoCaixa.meio_pagamento == meio_pagamento_filtro
            )
        if unidade_negocio_filtro_id:
            query = query.filter(
                LancamentoFluxoCaixa.unidade_negocio_id == unidade_negocio_filtro_id
            )

        lancamentos = query.order_by(
            LancamentoFluxoCaixa.unidade_negocio_id,
            LancamentoFluxoCaixa.data_vencimento,
        ).all()
        ...
        for lanc in lancamentos:
            unidade_nome = (
                lanc.unidade_negocio.nome if lanc.unidade_negocio else "Sem Unidade"
            )
        ...
        def create_sheet(sheet_name, data):
            ...
            for lanc in data:
                is_nfe = lanc.documento_numero and lanc.documento_numero.startswith(
                    "NFE-"
                )
                chave_acesso = (
                    lanc.observacoes.split("Chave de acesso: ")[1]
                    if is_nfe and "Chave de acesso: " in lanc.observacoes
                    else ""
                )
                ...
                if lanc.tem_rateio:
                    if lanc.rateios:
                        alocacao_rateio = ", ".join(
                            [r.veiculo.placa for r in lanc.rateios if r.veiculo]
                        )
                ...
                row = [
                    ...
                    lanc.meio_pagamento,
                    (
                        f"{lanc.parcela_numero}/{lanc.parcela_total}"
                        if lanc.parcela_total and lanc.parcela_total > 1
                        else "1/1"
                    ),
            ...
            sheet.cell(
                row=summary_row_start, column=10, value="TOTAL RECEITAS:"
            ).font = summary_font
            sheet.cell(
                row=summary_row_start, column=11, value=total_receitas
            ).number_format = currency_format
            sheet.cell(row=summary_row_start, column=11).font = summary_font

            sheet.cell(
                row=summary_row_start + 1, column=10, value="TOTAL DESPESAS:"
            ).font = summary_font
            sheet.cell(
                row=summary_row_start + 1, column=11, value=total_despesas
            ).number_format = currency_format
            sheet.cell(row=summary_row_start + 1, column=11).font = summary_font

            sheet.cell(row=summary_row_start + 2, column=10, value="SALDO:").font = (
                summary_font
            )
            sheet.cell(
                row=summary_row_start + 2,
                column=11,
                value=(total_receitas - total_despesas),
            ).number_format = currency_format
        ...
        logger.error(
            f"Erro ao exportar relatório detalhado de fluxo de caixa: {e}",
            exc_info=True,
        )
...
def enviar_contato():
    ...
    try:
        ...
        msg = Message(
            ...
            recipients=[
                "seu-email-de-vendas@gmail.com"
            ],  # LEMBRE-SE de colocar seu e-mail aqui
            body=corpo_email,
        )
        mail.send(msg)
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Ocorreu um erro interno ao enviar sua solicitação.",
                }
            ),
            500,
        )
        ...
        if not lancamentos_nfe:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Nenhum lançamento encontrado para esta NFe",
                    }
                ),
                404,
            )
...
def associar_notas_antigas_command():
    ...
    try:
        ...
        for empresa in empresas:
            ...
            certificado_principal = CertificadoDigital.query.filter_by(
                empresa_id=empresa.id, principal=True
            ).first()

            if not certificado_principal:
                print(
                    f"  -> Aviso: Nenhuma certificado principal encontrado para esta empresa. Pulando."
                )
                continue

            print(
                f"  -> Certificado principal encontrado: ID {certificado_principal.id}"
            )
            ...
            notas_atualizadas = NFeImportada.query.filter_by(
                empresa_id=empresa.id, certificado_id=None
            ).update({"certificado_id": certificado_principal.id})

            if notas_atualizadas > 0:
                print(
                    f"  -> {notas_atualizadas} nota(s) antiga(s) foram associadas com sucesso!"
                )
...
def listar_ctes():
    ...
    ctes = (
        CTeEmitido.query.filter_by(empresa_id=current_user.empresa_id)
        .order_by(CTeEmitido.created_at.desc())
        .all()
    )

    return render_template("cte/listar_ctes.html", ctes=ctes, active_page="cte")
...
def nova_cte_page():
    ...
    return render_template("cte/nova_cte.html", dados=dados, active_page="cte")
...
def api_emitir_cte():
    ...
    try:
        ...
        viagem_id_final = (
            int(viagem_id_str) if viagem_id_str and viagem_id_str != "None" else None
        )
        # --- FIM DA CORREÇÃO ---
...
def visualizar_cte(cte_id):
    ...
    return render_template("cte/visualizar_cte.html", cte=cte, active_page="cte")
...
def api_consultar_status_cte(cte_id):
    ...
    try:
        ...
        if not cte.chave_acesso:
            return (
                jsonify(
                    {"success": False, "message": "CT-e não possui chave de acesso"}
                ),
                400,
            )
...
def api_cancelar_cte(cte_id):
    ...
    try:
        ...
        if cte.status != "AUTORIZADO":
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Apenas CT-e autorizadas podem ser canceladas",
                    }
                ),
                400,
            )
        ...
        return jsonify({"success": True, "message": "CT-e cancelada com sucesso!"})
    ...
    try:
        ...
        dados = {
            ...
            "destinatario": (
                {
                    "nome": (
                        cliente_destinatario.nome_razao_social
                        if cliente_destinatario
                        else viagem.cliente
                    ),
                    "cnpj_cpf": (
                        cliente_destinatario.cpf_cnpj if cliente_destinatario else ""
                    ),
                    "endereco": (
                        cliente_destinatario.logradouro if cliente_destinatario else ""
                    ),
                    "cidade": (
                        cliente_destinatario.cidade if cliente_destinatario else ""
                    ),
                    "uf": cliente_destinatario.estado if cliente_destinatario else "",
                    "cep": cliente_destinatario.cep if cliente_destinatario else "",
                    "ie": (
                        getattr(cliente_destinatario, "inscricao_estadual", "")
                        if cliente_destinatario
                        else ""
                    ),
                }
            ...
            "motorista": (
                {
                    "nome": (
                        viagem.motorista_formal.nome if viagem.motorista_formal else ""
                    ),
                    "cpf": (
                        viagem.motorista_formal.cpf if viagem.motorista_formal else ""
                    ),
                }
                if viagem.motorista_formal
                else None
        ...
        logger.error(
            f"Erro ao buscar dados da viagem {viagem_id} para CT-e: {e}", exc_info=True
        )
        return jsonify({"success": False, "message": str(e)}), 500
...
def configuracoes_cte():
    ...
    parametros = CTeParametros.query.filter_by(
        empresa_id=current_user.empresa_id
    ).first()
    ...
    if request.method == "POST":
        try:
            ...
            parametros.aliquota_icms_padrao = float(
                request.form.get("aliquota_icms_padrao", 17.0)
            )
            parametros.natureza_operacao = request.form.get(
    ...
    return render_template(
        "cte/configuracoes_cte.html", parametros=parametros, active_page="cte"
    )
...
def api_dados_cliente_para_cte(cliente_id):
    ...
    try:
        ...
        logger.error(
            f"Erro ao buscar dados do cliente {cliente_id} para CT-e: {e}",
            exc_info=True,
        )
        return jsonify({"success": False, "message": str(e)}), 500
    ...
    if not nota:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Nota não encontrada na base local. Realize a consulta à SEFAZ primeiro.",
                }
            ),
            404,
        )
    ...
    if nota.status == "PROCESSADA":
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Esta nota já foi processada anteriormente.",
                }
            ),
            409,
        )
    ...
    dados_da_nota = {
        ...
        "certificado_origem": {
            ...
            "nome": (
                ...
                if certificado
                else "Certificado não identificado"
            ),
...
@app.route("/api/roteirizador/calcular", methods=["POST"])
@login_required
def api_calcular_rota_simples():
    """
    Recebe uma lista de endereços e retorna a distância e duração otimizada.
    Servirá como intermediário para o simulador de custos.
    """
    try:
        data = request.get_json()
        ...
        # Reutiliza sua função já existente!
        _, distancia_km, duracao_segundos, _, erro = calcular_rota_otimizada_ors(
            enderecos
        )
        ...
        logger.error(f"Erro na API de cálculo de rota simples: {e}", exc_info=True)
        return jsonify({"success": False, "message": str(e)}), 500
...
def api_ia_chat():
    """
    Endpoint placeholder para o assistente de IA.
    TODO: No futuro, integrar com um modelo de linguagem real (ex: API do Gemini).
    """
    data = request.get_json()
    mensagem_usuario = data.get("message", "").lower()

    # Lógica de resposta simulada
    if "motorista mais lucrativo" in mensagem_usuario:
        resposta = "Com base nos dados, o motorista mais lucrativo no último mês foi o João Silva. (Esta é uma resposta simulada)."
    elif "veículo com maior custo" in mensagem_usuario:
        resposta = "O veículo com placa ABC-1234 apresentou o maior custo por KM. Recomendo uma análise de manutenção. (Esta é uma resposta simulada)."
    else:
        resposta = "Estou processando sua solicitação... (Esta é uma resposta simulada). Como mais posso ajudar?"
...
def salvar_custo_viagem():
    ...
    if not viagem_id:
        return (
            jsonify({"success": False, "message": "ID da viagem não foi fornecido."}),
            400,
        )
    ...
    if not viagem:
        return (
            jsonify(
                {"success": False, "message": "Viagem não encontrada ou acesso negado."}
            ),
            404,
        )
    ...
    try:
            ...
            for anexo in request.files.getlist("anexos_despesa"):
                if anexo and anexo.filename != "":
                    ...
                    s3_client.upload_fileobj(
                        ...
                        bucket_name,
                        s3_path,
                        ExtraArgs={"ContentType": anexo.content_type},
                    )
                    urls_anexos.append(f"{public_url_base}/{s3_path}")
        ...
        custo_total_geral = (
            (custo.pedagios or 0)
            + (custo.alimentacao or 0)
            + (custo.hospedagem or 0)
            + (custo.outros or 0)
        )
...
def excluir_anexo_custo():
    ...
    try:
        ...
        if anexo_url not in anexos_atuais:
            return (
                jsonify({"success": False, "message": "URL do anexo não corresponde."}),
                404,
            )
...
def consultar_despesas(viagem_id):
    try:
        ...
        custo_dict = {
            ...
            "descricao_outros": (
                custo_viagem.descricao_outros if custo_viagem else "Nenhuma"
            ),
            "anexos": (
                custo_viagem.anexos.split(",")
                if custo_viagem and custo_viagem.anexos
                else []
            ),
        }
    ...
    try:
        ...
        if novo_status in ["concluida", "cancelada"]:
            if not viagem.data_fim:
                viagem.data_fim = datetime.now(UTC)
            if viagem.veiculo:
                viagem.veiculo.status = "Disponível"  #

        elif novo_status == "em_andamento" and status_antigo in [
            "concluida",
            "cancelada",
        ]:
            ...
            if viagem.veiculo:
                outra_viagem_ativa = Viagem.query.filter(
                    ...
                    Viagem.veiculo.has(
                        status="Em Rota"
                    ),  # Certifique-se de que o filtro aqui também está correto.
                    Viagem.id != viagem.id,
                ).first()

                if outra_viagem_ativa:
                    flash(
                        f"Erro: O veículo {viagem.veiculo.placa} já está em uso na viagem #{outra_viagem_ativa.id}.",
                        "error",
                    )
                    db.session.rollback()
                    return (
                        jsonify(
                            {
                                "success": False,
                                "message": f"Veículo já está em outra viagem.",
                            }
                        ),
                        409,
                    )
        ...
        db.session.rollback()
        logger.error(
            f"Erro ao atualizar status da viagem {viagem_id}: {e}", exc_info=True
        )
        return jsonify({"success": False, "message": str(e)}), 500
    ...
    if viagem.custo_viagem:
        if viagem.custo_viagem.pedagios:
            despesas_detalhadas.append(
                {
                    "tipo": "Pedágios",
                    "descricao": "Total em pedágios",
                    "valor": viagem.custo_viagem.pedagios,
                }
            )
        if viagem.custo_viagem.alimentacao:
            despesas_detalhadas.append(
                {
                    "tipo": "Alimentação",
                    "descricao": "Total em alimentação",
                    "valor": viagem.custo_viagem.alimentacao,
                }
            )
        if viagem.custo_viagem.hospedagem:
            despesas_detalhadas.append(
                {
                    "tipo": "Hospedagem",
                    "descricao": "Total em hospedagem",
                    "valor": viagem.custo_viagem.hospedagem,
                }
            )
        if viagem.custo_viagem.outros:
            despesas_detalhadas.append(
                {
                    "tipo": "Outros",
                    "descricao": viagem.custo_viagem.descricao_outros
                    or "Sem descrição",
                    "valor": viagem.custo_viagem.outros,
                }
            )

    # Renderiza um novo template com os detalhes
    return render_template(
        "detalhes_despesas_modal.html",
...
def consultar_viagens():
    ...
    if data_inicio:
        try:
            query = query.filter(
                Viagem.data_inicio >= datetime.strptime(data_inicio, "%Y-%m-%d")
            )
        except ValueError:
    ...
    if data_fim:
        try:
            ...
            data_fim_obj = datetime.strptime(data_fim, "%Y-%m-%d") + timedelta(
                days=1, seconds=-1
            )
    ...
    for v in viagens_objetos:
        ...
        v.motorista_telefone = (
            v.motorista_formal.telefone if v.motorista_formal else None
        )

        # Busca o cliente no banco de dados para obter o telefone
        cliente_obj = Cliente.query.filter_by(
            nome_razao_social=v.cliente, empresa_id=current_user.empresa_id
        ).first()
...
def gerenciar_despesas_viagem(viagem_id):
    ...
    custo = CustoViagem.query.filter_by(viagem_id=viagem_id).first()
    abastecimentos = (
        Abastecimento.query.filter_by(viagem_id=viagem_id)
...
def finalizar_viagem(viagem_id):
    ...
    viagem = Viagem.query.filter_by(
        id=viagem_id, empresa_id=current_user.empresa_id, status="em_andamento"
    ).first_or_404()
    ...
    try:
        ...
        if (
            viagem.odometro_inicial is not None
            and odometro_final < viagem.odometro_inicial
        ):
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Odômetro final não pode ser menor que o inicial.",
                    }
                ),
                400,
            )
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Odômetro final inválido. Por favor, insira um número válido.",
                }
            ),
            400,
        )
    ...
    socketio.emit(
        "status_viagem_atualizado",
        {"viagem_id": viagem.id, "status": "concluida"},
        room="admins",
    )
...
def relatorios():
    try:
        ...
        if data_inicio_str:
            query = query.filter(
                Viagem.data_inicio >= datetime.strptime(data_inicio_str, "%Y-%m-%d")
            )
        if data_fim_str:
            data_fim_obj = datetime.strptime(data_fim_str, "%Y-%m-%d") + timedelta(
                days=1
            )
            query = query.filter(Viagem.data_inicio < data_fim_obj)
        if motorista_id_filter:
        ...
        motoristas_stats_dict = defaultdict(
            lambda: {"id": None, "nome": "N/A", "viagens": 0, "receita": 0.0}
        )  # Renomeado para clareza
        veiculos_stats = defaultdict(
            lambda: {
                "id": None,
                "modelo": "N/A",
                "placa": "N/A",
        ...
        for v in viagens_filtradas:
            ...
            if v.custo_viagem:
                ...
                dados_grafico_categorias["Alimentação"] += (
                    v.custo_viagem.alimentacao or 0
                )
            ...
            if v.cliente:
                ...
                clientes_stats[v.cliente]["lucro"] = (
                    clientes_stats[v.cliente]["receita"]
                    - clientes_stats[v.cliente]["custo"]
                )

            if v.motorista_formal:
                motoristas_stats_dict[v.motorista_formal.id].update(
                    {"id": v.motorista_formal.id, "nome": v.motorista_formal.nome}
                )
                motoristas_stats_dict[v.motorista_formal.id]["viagens"] += 1
                motoristas_stats_dict[v.motorista_formal.id][
                    "receita"
                ] += receita_viagem
        ...
        motoristas_ordenados = sorted(
            motoristas_stats_dict.values(), key=lambda m: m["receita"], reverse=True
        )

        total_custo = total_custo_combustivel + total_custo_outros
        consumo_medio_geral = (
            (total_distancia / total_litros) if total_litros > 0 else 0
        )

        motoristas_para_filtro = (
            Motorista.query.filter_by(empresa_id=current_user.empresa_id)
            .order_by(Motorista.nome)
            .all()
        )
        veiculos_para_filtro = (
            Veiculo.query.filter_by(empresa_id=current_user.empresa_id)
            .order_by(Veiculo.placa)
            .all()
        )
...
@oficina_borracharia_required
def oficina():
    """Rota principal do dashboard da oficina, com o novo fluxo de aprovação."""
    empresa_id = current_user.empresa_id

    # 1. BUSCAR VEÍCULOS E CARRETAS SEPARADAMENTE
    veiculos_obj = (
        Veiculo.query.filter_by(empresa_id=empresa_id, is_administrativo=False)
        .order_by(Veiculo.modelo)
        .all()
    )
    carretas_obj = (
        Carreta.query.filter_by(empresa_id=empresa_id).order_by(Carreta.modelo).all()
    )

    # 2. CRIAR UMA LISTA UNIFICADA PARA O MODAL DE "NOVA MANUTENÇÃO"
    frota_para_modal = []
    for v in veiculos_obj:
        frota_para_modal.append(
            {
                "id": v.id,
                "display_name": f"{v.modelo} ({v.placa})",
                "km_rodados": v.km_rodados or 0,
                "tipo": "veiculo",
            }
        )
    for c in carretas_obj:
        frota_para_modal.append(
            {
                "id": c.id,
                "display_name": f"{c.modelo} ({c.placa}) - Carreta",
                "km_rodados": 0,
                "tipo": "carreta",
            }
        )
    frota_para_modal.sort(key=lambda x: x["display_name"])

    # 3. PROCESSAR DADOS DOS VEÍCULOS E CARRETAS PARA A TABELA PRINCIPAL
    veiculos_data = []  # Esta lista agora conterá tanto veículos quanto carretas
    alertas = []
    limite_km_alerta = 1000
    limite_dias_alerta = 30

    for veiculo in veiculos_obj:
        # Lógica de processamento de planos e alertas para veículos
        planos_progresso = []
        progresso_maximo = -1
        next_maint_summary = {}
        for plano_assoc in veiculo.planos_associados:
            ...
            progresso_km = 0
            if (
                plano_assoc.intervalo_km
                and plano_assoc.intervalo_km > 0
                and veiculo.km_rodados is not None
            ):
                km_desde_ultima = (veiculo.km_rodados or 0) - (
                    plano_assoc.km_ultima_manutencao or 0
                )
                if km_desde_ultima < 0:
                    km_desde_ultima = 0
            ...
            if (
                not alerta_gerado
                and plano_assoc.intervalo_meses
                and plano_assoc.data_ultima_manutencao
            ):
                data_proxima = plano_assoc.data_ultima_manutencao + timedelta(
                    days=plano_assoc.intervalo_meses * 30
                )
        ...
        if veiculo.status == "Em Manutenção":
            manutencao_ativa = Manutencao.query.filter_by(
                veiculo_id=veiculo.id, status="Em Andamento"
            ).first()
            if manutencao_ativa:
        ...
        veiculos_data.append(
            {
                "id": veiculo.id,
                "modelo": f"{veiculo.modelo} - Veículo",
                "placa": veiculo.placa,
                "km_rodados": veiculo.km_rodados or 0,
                "status": veiculo.status,
                "manutencao_id": manutencao_id_ativa,
                "proxima_manutencao": (
                    next_maint_summary if progresso_maximo > -1 else None
                ),
                "planos_progresso": sorted(
                    planos_progresso, key=lambda x: x["progresso"], reverse=True
                ),
            }
        )

    for carreta in carretas_obj:
        veiculos_data.append(
            {
                "id": carreta.id,
                "modelo": f"{carreta.modelo} - Carreta",
                "placa": carreta.placa,
                "km_rodados": 0,
    ...
    veiculos_data.sort(key=lambda x: x["modelo"])

    # 4. BUSCAR MANUTENÇÕES POR STATUS PARA AS NOVAS ABAS E MODAIS
    manutencoes_ativas = Manutencao.query.filter(
        Manutencao.empresa_id == empresa_id,
        Manutencao.status.in_(["Em Andamento", "Agendada"]),
    ).all()

    manutencoes_para_aprovar = Manutencao.query.filter(
        Manutencao.empresa_id == empresa_id, Manutencao.status == "Aguardando Aprovação"
    ).all()

    manutencoes_concluidas = (
        Manutencao.query.filter(
            Manutencao.empresa_id == empresa_id, Manutencao.status == "Concluída"
        )
        .order_by(Manutencao.data_saida.desc())
        .limit(50)
        .all()
    )  # Limita a 50 para performance

    # 5. KPIs e Alertas de Estoque (lógica original mantida)
    hoje = date.today()
    ...
    return render_template(
        ...
        manutencoes_para_aprovar=manutencoes_para_aprovar,
        manutencoes_concluidas=manutencoes_concluidas,
        todos_veiculos=veiculos_data,
        frota_para_modal=frota_para_modal,
        kpis=kpis,
        alertas_de_estoque=alertas_de_estoque,
    )


@app.route("/oficina/finalizar_leitor_qr")
@login_required
@oficina_borracharia_required
def leitor_qr_page():
    """Renderiza uma página mobile com um leitor de QR Code."""
    return render_template("leitor_qr_mobile.html")
...
def iniciar_manutencao_oficina():
    ...
    if plano_id_str and plano_id_str.isdigit():
        ...
        plano_assoc = VeiculoPlano.query.filter_by(
    ...
    flash(
        f"Manutenção registrada! Status do veículo {veiculo.placa} atualizado.",
        "success",
    )
    return redirect(url_for("oficina"))
...
def finalizar_manutencao_oficina():
    ...
    try:
        ...
        db.session.commit()
        flash(
            f"Manutenção finalizada e estoque atualizado! Veículo {veiculo.placa} agora está disponível.",
            "success",
        )
    except Exception as e:
        db.session.rollback()
        flash(
            f"Manutenção finalizada, mas ocorreu um erro ao atualizar o estoque: {e}",
            "error",
        )
        logger.error(
            f"Erro na baixa de estoque para manutenção {manutencao_id}: {e}",
            exc_info=True,
        )
        ...
        planos_atribuidos = [
            {
                ...
                "data_ultima_manutencao": (
                    assoc.data_ultima_manutencao.strftime("%Y-%m-%d")
                    if assoc.data_ultima_manutencao
                    else None
                ),
            }
    ...
    if request.method == "POST":
        try:
            ...
            for plano_data in data:
                plano = PlanoDeManutencao.query.filter_by(
                ...
                nova_assoc = VeiculoPlano(
                    ...
                    km_ultima_manutencao=plano_data.get(
                        "km_ultima_manutencao"
                    ),  # Salva o valor exato (número ou null)
                    data_ultima_manutencao=data_ultima,
                )
                db.session.add(nova_assoc)
                if plano_data.get("atualizar_padrao"):
                    plano.intervalo_km_padrao = (
                        int(plano_data["intervalo_km"])
                        if plano_data.get("intervalo_km")
                        else None
                    )
                    plano.intervalo_meses_padrao = (
                        int(plano_data["intervalo_meses"])
                        if plano_data.get("intervalo_meses")
                        else None
                    )
            ...
            db.session.rollback()
            logger.error(
                f"Erro ao salvar planos para veiculo {veiculo_id}: {e}", exc_info=True
            )
...
def add_manutencao_item(manutencao_id):
    ...
    try:
        ...
        custo_atualizado = (
            db.session.query(
                func.sum(ManutencaoItem.quantidade * ManutencaoItem.custo_unitario)
            )
            .filter_by(manutencao_id=manutencao.id)
            .scalar()
        )
...
def api_historico_veiculo(veiculo_id):
    query = (
        db.session.query(
            Manutencao, PlanoDeManutencao.descricao.label("plano_descricao")
        )
        .outerjoin(
            VeiculoPlano,
            and_(
...
def get_historico_manutencoes():
    query = Manutencao.query.join(Veiculo).filter(
        Veiculo.empresa_id == current_user.empresa_id, Manutencao.status == "Concluída"
    )
    ...
    if data_fim_str := request.args.get("data_fim"):
        query = query.filter(
            Manutencao.data_saida <= datetime.strptime(data_fim_str, "%Y-%m-%d").date()
        )
...
def gerenciar_insumos_page():
    ...
    insumos = (
        Insumo.query.filter_by(empresa_id=current_user.empresa_id)
        .order_by(Insumo.descricao)
        .all()
    )
...
def api_criar_insumo():
    ...
    try:
        custo = (
            float(data.get("custo_unitario_medio"))
            if data.get("custo_unitario_medio")
            else 0.0
        )
        qtd_estoque = (
            float(data.get("quantidade_em_estoque"))
            if data.get("quantidade_em_estoque")
            else 0.0
        )
        ponto_ressuprimento = (
            float(data.get("ponto_ressuprimento"))
            if data.get("ponto_ressuprimento")
            else None
        )
        ...
        return (
            jsonify(
                {"success": False, "message": "Já existe um insumo com esta descrição."}
            ),
            409,
        )
    except Exception as e:
...
def exportar_insumos_excel():
    ...
    try:
        ...
        filename = f"catalogo_insumos_{datetime.now(UTC).strftime('%Y-%m-%d')}.xlsx"
        return send_file(
            output,
            as_attachment=True,
            download_name=filename,
...
def api_ajustar_estoque_insumo(insumo_id):
    ...
    insumo = Insumo.query.filter_by(
        id=insumo_id, empresa_id=current_user.empresa_id
    ).first_or_404()
    data = request.json
    try:
        ...
        if novo_estoque < 0:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": f"Ajuste inválido. O estoque não pode ficar negativo. Estoque atual: {insumo.quantidade_em_estoque}",
                    }
                ),
                400,
            )
        ...
        if quantidade_ajuste > 0:
            message = (
                f"Entrada de {quantidade_ajuste} unidade(s) registrada com sucesso!"
            )
        else:
            message = (
                f"Baixa de {abs(quantidade_ajuste)} unidade(s) registrada com sucesso!"
            )
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Quantidade inválida. Use apenas números.",
                }
            ),
            400,
        )
...
def api_salvar_kit_do_plano(plano_id):
    ...
    try:
        plano.intervalo_km_padrao = (
            int(data["intervalo_km"]) if data.get("intervalo_km") else None
        )
        plano.intervalo_meses_padrao = (
            int(data["intervalo_meses"]) if data.get("intervalo_meses") else None
        )
        ...
        db.session.commit()
        return jsonify(
            {"success": True, "message": "Plano e Kit de peças salvos com sucesso!"}
        )
    except Exception as e:
        db.session.rollback()
...
def api_deletar_insumo(insumo_id):
    ...
    # Capturamos o erro específico de integridade do banco de dados.
        ...
        return (
            ...
            409,
        )  # HTTP 409 Conflict é um bom status para este caso.
...
def api_exportar_documentos_fiscais():
    ...
    try:
        ...
        if not chaves_acesso:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Nenhuma nota fiscal foi selecionada para exportação.",
                    }
                ),
                400,
            )
        ...
        if not notas_para_exportar:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Nenhuma das notas selecionadas foi encontrada.",
                    }
                ),
                404,
            )
        ...
        filename = f"Exportacao_Contabil_{datetime.now(UTC).strftime('%Y-%m-%d')}.zip"
        return send_file(
            zip_buffer,
        ...
        return (
            jsonify(
                {"success": False, "message": f"Ocorreu um erro interno: {str(e)}"}
            ),
            500,
        )
    ...
    for veiculo in veiculos:
        ...
        for plano_assoc in veiculo.planos_associados:
            ...
            if km_restantes > 0:
                ...
                custo_estimado_plano = (
                    db.session.query(
                        func.sum(PlanoInsumo.quantidade * Insumo.custo_unitario_medio)
                    )
                    .join(Insumo)
                    .filter(PlanoInsumo.plano_id == plano_assoc.plano_id)
                    .scalar()
                    or 0.0
                )
    ...
    try:
        ...
        if search_query := args.get("search"):
            ...
            query = query.outerjoin(
                Motorista, Viagem.motorista_id == Motorista.id
            ).filter(
                or_(
                    Viagem.cliente.ilike(search_term),
                    Motorista.nome.ilike(search_term),
        ...
        for row_num, viagem in enumerate(viagens, 2):
            ...
            custo_pedagios = (
                viagem.custo_viagem.pedagios if viagem.custo_viagem else 0.0
            )
            custo_alimentacao = (
                viagem.custo_viagem.alimentacao if viagem.custo_viagem else 0.0
            )
            custo_hospedagem = (
                viagem.custo_viagem.hospedagem if viagem.custo_viagem else 0.0
            )
            ...
            destinos_intermediarios = ", ".join(
                [d.endereco for d in destinos_ordenados[:-1]]
            )

            # Preenchimento das células
            sheet.cell(row=row_num, column=1, value=viagem.id)
            sheet.cell(
                row=row_num, column=2, value=viagem.status.replace("_", " ").title()
            )
            sheet.cell(
                row=row_num, column=3, value=viagem.data_inicio.strftime("%d/%m/%Y")
            )
            sheet.cell(
                row=row_num, column=4, value=viagem.data_inicio.strftime("%H:%M:%S")
            )
            sheet.cell(
                row=row_num,
                column=5,
                value=(
                    viagem.data_fim.strftime("%d/%m/%Y") if viagem.data_fim else "N/A"
                ),
            )
            sheet.cell(
                row=row_num,
                column=6,
                value=(
                    viagem.data_fim.strftime("%H:%M:%S") if viagem.data_fim else "N/A"
                ),
            )
            ...
            sheet.cell(
                ...
                column=9,
                value=(
                    viagem.motorista_formal.nome if viagem.motorista_formal else "N/A"
                ),
            )
            sheet.cell(
                row=row_num,
                column=10,
                value=viagem.veiculo.placa if viagem.veiculo else "N/A",
            )
            sheet.cell(
                row=row_num,
                column=11,
                value=viagem.veiculo.modelo if viagem.veiculo else "N/A",
            )
            ...
            sheet.cell(
                row=row_num, column=19, value=viagem.valor_recebido or 0
            ).number_format = "R$ #,##0.00"
            sheet.cell(
                row=row_num, column=20, value=viagem.custo_real_completo
            ).number_format = "R$ #,##0.00"
            sheet.cell(
                row=row_num, column=21, value=viagem.lucro_real
            ).number_format = "R$ #,##0.00"
            sheet.cell(
                row=row_num, column=22, value=custo_combustivel or 0
            ).number_format = "R$ #,##0.00"
            sheet.cell(
                row=row_num, column=23, value=custo_pedagios or 0
            ).number_format = "R$ #,##0.00"
            sheet.cell(
                row=row_num, column=24, value=custo_alimentacao or 0
            ).number_format = "R$ #,##0.00"
            sheet.cell(
                row=row_num, column=25, value=custo_hospedagem or 0
            ).number_format = "R$ #,##0.00"
            sheet.cell(
                row=row_num, column=26, value=custo_outros or 0
            ).number_format = "R$ #,##0.00"
        ...
        return send_file(
            output,
            as_attachment=True,
            download_name=f"relatorio_detalhado_viagens_{datetime.now(UTC).strftime('%Y%m%d_%H%M')}.xlsx",
            mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )
    except Exception as e:
...
def get_active_trip():
    ...
    if viagem:
        horario_chegada = (
            (viagem.data_inicio + timedelta(seconds=viagem.duracao_segundos)).strftime(
                "%d/%m/%Y %H:%M"
            )
            if viagem.duracao_segundos
            else "Não calculado"
        ...
        if viagem.motorista_id:
            ...
            if usuario_com_cpf:
                ...
                motorista_formal_cpf = Motorista.query.filter_by(
                    cpf_cnpj=viagem.motorista_cpf_cnpj
                ).first()
...
@app.route("/cadastrar_carreta", methods=["GET", "POST"])
@login_required
def cadastrar_carreta():
    """Renderiza o formulário e processa o cadastro de uma nova carreta."""
    if request.method == "POST":
        try:
            placa = request.form.get("placa", "").strip().upper()

            # Validação para garantir que a placa seja única na empresa
            if (
                Veiculo.query.filter_by(
                    placa=placa, empresa_id=current_user.empresa_id
                ).first()
                ...
                flash(
                    f'Erro: A placa "{placa}" já está cadastrada para um veículo ou carreta.',
                    "error",
                )
                return render_template("cadastrar_carreta.html", form_data=request.form)
            ...
            nova_carreta = Carreta(
                ...
                eixos=(
                    int(request.form.get("eixos"))
                    if request.form.get("eixos")
                    else None
                ),
                status="Disponível",
            )
            ...
            db.session.add(nova_carreta)
            db.session.commit()
            flash("Carreta cadastrada com sucesso!", "success")
            return redirect(url_for("consultar_carretas"))

        except Exception as e:
            db.session.rollback()
            logger.error(f"Erro ao cadastrar carreta: {e}", exc_info=True)
            flash(f"Ocorreu um erro inesperado ao cadastrar a carreta: {e}", "error")
            return render_template("cadastrar_carreta.html", form_data=request.form)

    return render_template("cadastrar_carreta.html", active_page="consultar_carretas")


@app.route("/consultar_carretas")
@login_required
def consultar_carretas():
    """Exibe a lista de todas as carretas da empresa."""
    search_query = request.args.get("search", "").strip()
    query = Carreta.query.filter_by(empresa_id=current_user.empresa_id)

    if search_query:
        search_filter = f"%{search_query}%"
        query = query.filter(
            or_(
                Carreta.placa.ilike(search_filter),
                Carreta.modelo.ilike(search_filter),
                Carreta.marca.ilike(search_filter),
            )
        )

    carretas = query.order_by(Carreta.placa.asc()).all()

    # --- LÓGICA DE KPIs ADICIONADA ---
    # Este bloco calcula as estatísticas antes de renderizar a página.
    stats = {
        "total": len(carretas),
        "disponivel": sum(1 for c in carretas if c.status == "Disponível"),
        "em_viagem": sum(1 for c in carretas if c.status == "Em Viagem"),
        "em_manutencao": sum(1 for c in carretas if c.status == "Em Manutenção"),
    }

    return render_template(
        "consultar_carretas.html",
        carretas=carretas,
        search_query=search_query,
        stats=stats,  # <-- A variável 'stats' agora está sendo enviada para o template
        active_page="consultar_carretas",
    )


@app.route("/editar_carreta/<int:carreta_id>", methods=["GET", "POST"])
@login_required
def editar_carreta(carreta_id):
    """Renderiza o formulário e processa a edição de uma carreta existente."""
    carreta = Carreta.query.filter_by(
        id=carreta_id, empresa_id=current_user.empresa_id
    ).first_or_404()
    ...
    if request.method == "POST":
        try:
            ...
            # Validação para garantir que a nova placa não colida com outros registros
            if nova_placa != carreta.placa:
                if (
                    Veiculo.query.filter_by(
                        placa=nova_placa, empresa_id=current_user.empresa_id
                    ).first()
                    or Carreta.query.filter(
                        Carreta.placa == nova_placa,
                        Carreta.id != carreta_id,
                        Carreta.empresa_id == current_user.empresa_id,
                    ).first()
                ):
                    flash(f'Erro: A placa "{nova_placa}" já está em uso.', "error")
                    return redirect(url_for("editar_carreta", carreta_id=carreta_id))

            carreta.placa = nova_placa
            carreta.marca = request.form.get("marca")
            carreta.modelo = request.form.get("modelo")
            carreta.ano = (
                int(request.form.get("ano")) if request.form.get("ano") else None
            )
            carreta.renavam = request.form.get("renavam")
            carreta.eixos = (
                int(request.form.get("eixos")) if request.form.get("eixos") else None
            )
            carreta.status = request.form.get("status")

            db.session.commit()
            flash("Dados da carreta atualizados com sucesso!", "success")
            return redirect(url_for("consultar_carretas"))

        except Exception as e:
            db.session.rollback()
            logger.error(f"Erro ao editar carreta {carreta_id}: {e}", exc_info=True)
            flash(f"Ocorreu um erro ao salvar as alterações: {e}", "error")
            return redirect(url_for("editar_carreta", carreta_id=carreta_id))

    return render_template(
        "editar_carreta.html", carreta=carreta, active_page="consultar_carretas"
    )


@app.route("/excluir_carreta/<int:carreta_id>")
@login_required
def excluir_carreta(carreta_id):
    """Exclui uma carreta, se não estiver associada a uma viagem ativa."""
    carreta = Carreta.query.filter_by(
        id=carreta_id, empresa_id=current_user.empresa_id
    ).first_or_404()

    # Trava de segurança: Verifica se a carreta está em uma viagem "em andamento"
    viagem_ativa = Viagem.query.filter_by(
        carreta_id=carreta_id, status="em_andamento"
    ).first()
    if viagem_ativa:
        flash(
            f"Erro: A carreta {carreta.placa} não pode ser excluída pois está em uso na viagem #{viagem_ativa.id}.",
            "error",
        )
        return redirect(url_for("consultar_carretas"))

    try:
        db.session.delete(carreta)
        db.session.commit()
        flash("Carreta excluída com sucesso!", "success")
    except Exception as e:
        db.session.rollback()
        flash(f"Erro ao excluir a carreta: {e}", "error")

    return redirect(url_for("consultar_carretas"))


@app.route("/api/carretas_disponiveis")
@login_required
def api_carretas_disponiveis():
    """API que retorna uma lista de carretas com status 'Disponível' para o formulário de viagem."""
    try:
        carretas = (
            Carreta.query.filter_by(
                empresa_id=current_user.empresa_id, status="Disponível"
            )
            .order_by(Carreta.placa)
            .all()
        )

        carretas_data = [
            {"id": c.id, "placa": c.placa, "modelo": c.modelo or "Sem modelo"}
            for c in carretas
        ]
        return jsonify(carretas_data)
    except Exception as e:
        logger.error(f"Erro na API de carretas disponíveis: {e}", exc_info=True)
        return jsonify([])
...
def configuracoes():
    if request.method == "POST":
        # Esta parte lida com a atualização dos dados pessoais do usuário logado
    ...
    # Esta parte (método GET) prepara os dados para exibir a página
    empresa = (
        db.session.get(Empresa, current_user.empresa_id)
    ...
    # Apenas Admin e Master podem gerenciar outros usuários e convites
    if current_user.role in ["Admin", "Master"] and current_user.empresa_id:
        # Busca os usuários ativos da empresa
        usuarios = (
            Usuario.query.filter_by(empresa_id=current_user.empresa_id)
            .order_by(Usuario.nome)
            .all()
        )
        # Busca os convites que ainda não foram usados e não expiraram
        convites_pendentes = (
            Convite.query.filter(
                Convite.empresa_id == current_user.empresa_id,
                Convite.usado == False,
                Convite.data_expiracao > datetime.now(UTC),
            )
            .order_by(Convite.data_criacao.desc())
            .all()
        )

    # Obtém a data de hoje para passar para o template (para o alerta de licença)
...
@app.route("/reenviar_convite/<int:convite_id>", methods=["POST"])
@login_required
@master_required
def reenviar_convite(convite_id):
    """
    Recria o token, atualiza a data de expiração e reenvia o e-mail de um convite pendente.
    """
    convite = Convite.query.filter_by(
        id=convite_id, empresa_id=current_user.empresa_id, usado=False
    ).first_or_404()

    try:
        convite.token = str(uuid.uuid4())
        # --- CORREÇÃO APLICADA AQUI ---
        convite.data_expiracao = datetime.now(UTC) + timedelta(days=3)

        link_convite = url_for("registrar_usuario", token=convite.token, _external=True)
        msg = Message(
            subject=f"Lembrete de Convite para o sistema TrackBras",
            recipients=[convite.email],
            body=f"Olá! Este é um lembrete do seu convite para se registrar no sistema TrackBras como {convite.role}.\n\nClique no novo link abaixo para se cadastrar:\n\n{link_convite}\n\nEste novo link é válido por 3 dias.",
        )
        mail.send(msg)

        db.session.commit()
        flash(f"Convite para {convite.email} reenviado com sucesso!", "success")
    except Exception as e:
        db.session.rollback()
        flash(f"Erro ao reenviar convite: {e}", "error")
        logger.error(f"Erro ao reenviar convite {convite_id}: {e}", exc_info=True)

    return redirect(url_for("configuracoes"))


@app.route("/owner/gerenciar_usuarios")
@login_required
@owner_required
def owner_gerenciar_usuarios():
    """Página de gerenciamento global de usuários e convites para o Owner."""
    # Busca TODOS os usuários, exceto o próprio Owner, e carrega a empresa relacionada
    usuarios = (
        Usuario.query.filter(Usuario.role != "Owner")
        .options(db.joinedload(Usuario.empresa))
        .order_by(Usuario.nome)
        .all()
    )

    # Busca TODOS os convites pendentes de TODAS as empresas
    convites_pendentes = (
        Convite.query.filter_by(usado=False)
        .options(db.joinedload(Convite.empresa))  # Carrega o nome da empresa do convite
        .order_by(Convite.data_criacao.desc())
        .all()
    )

    return render_template(
        "owner_gerenciar_usuarios.html",
        usuarios=usuarios,
        convites_pendentes=convites_pendentes,
    )


@app.route("/owner/reenviar_convite/<int:convite_id>", methods=["POST"])
@login_required
@owner_required
def owner_reenviar_convite(convite_id):
    """Rota para o Owner reenviar qualquer convite pendente."""
    convite = Convite.query.filter_by(id=convite_id, usado=False).first_or_404()
    try:
        convite.token = str(uuid.uuid4())
        convite.data_expiracao = datetime.now(UTC) + timedelta(days=3)
        ...
        db.session.rollback()
        flash(f"Erro ao reenviar convite: {e}", "error")
        logger.error(f"Owner erro ao reenviar convite {convite_id}: {e}", exc_info=True)

    return redirect(url_for("owner_gerenciar_usuarios"))


@app.route("/owner/cancelar_convite/<int:convite_id>", methods=["POST"])
@login_required
@owner_required
def owner_cancelar_convite(convite_id):
    """Rota para o Owner cancelar qualquer convite pendente."""
    convite = Convite.query.filter_by(id=convite_id, usado=False).first_or_404()
    try:
        db.session.delete(convite)
        db.session.commit()
        flash(f"O convite para {convite.email} foi cancelado.", "success")
    except Exception as e:
        db.session.rollback()
        flash(f"Erro ao cancelar o convite: {e}", "error")

    return redirect(url_for("owner_gerenciar_usuarios"))


@app.route("/cancelar_convite/<int:convite_id>", methods=["POST"])
@login_required
@master_required
def cancelar_convite(convite_id):
    """
    Exclui um convite pendente do banco de dados.
    """
    convite = Convite.query.filter_by(
        id=convite_id, empresa_id=current_user.empresa_id, usado=False
    ).first_or_404()
...
def editar_usuario(usuario_id):
    ...
    if request.method == "POST":
        ...
        # --- 1ª CORREÇÃO: Adicionado 'Oficina/Borracharia' à lista de papéis válidos ---
        if role not in ["Motorista", "Master", "Admin", "Oficina/Borracharia"]:
        ...
        if (
            cpf_cnpj
            and cpf_cnpj != usuario.cpf_cnpj
            and Usuario.query.filter_by(cpf_cnpj=cpf_cnpj).first()
        ):
        ...
        usuario.email = email
        usuario.role = role
        # --- 2ª CORREÇÃO: Garante que 'Master' também seja considerado um administrador ---
        usuario.is_admin = role in ["Admin", "Master"]
        usuario.cpf_cnpj = (
            cpf_cnpj if cpf_cnpj else None
        )  # Atualiza CPF/CNPJ do usuário
...
def gerenciar_empresa():
    ...
    if current_user.role not in ["Admin", "Master"]:
        flash(
            "Acesso negado. Apenas administradores podem gerenciar a empresa.", "error"
        )
        return redirect(url_for("index"))
...
def excluir_usuario(usuario_id):
    ...
    if Cobranca.query.filter_by(usuario_id=usuario.id).first():
        flash(
            "Erro: Não é possível excluir este usuário, pois ele possui cobranças financeiras registradas.",
            "error",
        )
        return redirect(url_for("configuracoes"))
    # ▲▲▲ FIM DA VERIFICAÇÃO ▲▲▲
...
def motorista_dashboard():
    ...
    motorista = Motorista.query.filter_by(
        cpf_cnpj=current_user.cpf_cnpj, empresa_id=current_user.empresa_id
    ).first()
...
def atualizar_localizacao():
    ...
    try:
        ...
        motorista_id_para_localizacao = (
            motorista_formal.id if motorista_formal else None
        )

        if not motorista_id_para_localizacao:
            logger.warning(
                f"Usuário {current_user.email} tentou atualizar localização sem motorista formal vinculado por CPF/CNPJ."
            )
            return jsonify(
                {
                    "success": False,
                    "message": "Motorista formal não encontrado para vincular localização.",
                }
            )
...
def selecionar_viagem(viagem_id):
    ...
    if not current_user.cpf_cnpj:
        return jsonify(
            {
                "success": False,
                "message": "Seu perfil de usuário não possui CPF/CNPJ. Preencha-o nas configurações para iniciar viagens.",
            }
        )
    ...
    ):  # 'Pendente' precisa ser 'pendente' conforme o default do modelo
        return jsonify(
            {
                "success": False,
                "message": "Viagem já foi iniciada ou está em outro status",
            }
        )

    viagem.motorista_cpf_cnpj = (
        current_user.cpf_cnpj
    )  # Vincula pelo CPF/CNPJ do usuário

    # Opcional: Se o usuário logado tiver um motorista formal vinculado, use o ID desse motorista também
    motorista_formal = Motorista.query.filter_by(
        usuario_id=current_user.id, cpf_cnpj=current_user.cpf_cnpj
    ).first()
    if motorista_formal:
        viagem.motorista_id = (
            motorista_formal.id
        )  # Linka com o ID do motorista formal se ele existir

    viagem.status = "em_andamento"  # 'Ativa' precisa ser 'em_andamento' conforme o default do modelo
    viagem.data_inicio = datetime.now(UTC)
...
def viagens_pendentes():
    if current_user.role != "Motorista":
        return (
            jsonify({"success": False, "message": "Acesso restrito a motoristas."}),
            403,
        )
    ...
    try:
        print(
            f"[INFO] Buscando para o usuário: {current_user.email} (Empresa ID: {current_user.empresa_id}, CPF: {current_user.cpf_cnpj})"
        )

        # PASSO 1: O sistema tenta encontrar o perfil "Motorista" que corresponde ao "Usuário" logado.
        motorista_formal = Motorista.query.filter_by(
            cpf=current_user.cpf_cnpj, empresa_id=current_user.empresa_id
        ).first()

        if not motorista_formal:
            print(
                "[ERRO CRÍTICO] Nenhum perfil de 'Motorista' encontrado com o CPF/CNPJ e Empresa do usuário logado. A busca não pode continuar."
            )
            return jsonify({"success": True, "viagens": []})

        motorista_id = motorista_formal.id
        print(
            f"[INFO] Perfil de motorista formal encontrado: ID={motorista_id}, Nome='{motorista_formal.nome}'"
        )
        ...
        print(
            f"       3. E para o motorista ID: {motorista_id} (ou CPF/CNPJ: {current_user.cpf_cnpj})"
        )
        ...
        if viagens_encontradas:
            print(
                f"[SUCESSO] Foram encontradas {len(viagens_encontradas)} viagem(ns) pendentes para este motorista."
            )
        else:
            print(
                "[FALHA] Nenhuma viagem encontrada que cumpra TODOS os três critérios acima."
            )
        ...
        for v in viagens_encontradas:
            lista_de_destinos = [
                d.endereco for d in sorted(v.destinos, key=lambda x: x.ordem)
            ]
            viagens_data.append(
                {
                    "id": v.id,
        ...
        logger.error(
            f"Erro ao obter viagens pendentes para o motorista {current_user.id}: {str(e)}",
            exc_info=True,
        )
        return (
            jsonify(
    ...
    try:
        ...
        with app.app_context():
            ...
            if Usuario.query.count() == 0:
                ...
                logger.info(
                    "Banco de dados já contém dados. Semeação não foi executada."
                )
...
def get_address_geoapify(lat, lon):
    try:
        ...
        logger.error(
            f"Erro de rede/API na geocodificação Geoapify: {str(e)}", exc_info=True
        )
    except Exception as e:
        logger.error(
            f"Erro inesperado na geocodificação Geoapify: {str(e)}", exc_info=True
        )
    return "Endereço não encontrado"
...
def ultima_localizacao(viagem_id):
    ...
    return jsonify(
        {
            "success": False,
            "message": "Nenhuma localização encontrada para esta viagem.",
        }
    )
...
def perfil_motorista(motorista_id):
    ...
    return render_template(
        "perfil_motorista.html", motorista=motorista, viagens=viagens, stats=stats
    )
...
def gerar_romaneio(viagem_id):
    ...
    romaneio = Romaneio.query.filter_by(
        viagem_id=viagem_id, empresa_id=current_user.empresa_id
    ).first()
    ...
    if request.method == "POST":
        try:
            ...
            db.session.rollback()
            logger.error(
                f"Erro ao salvar romaneio para viagem {viagem_id}: {e}", exc_info=True
            )
    ...
    if romaneio:
        return render_template(
            "cadastro_romaneio.html", viagem=viagem, romaneio=romaneio
        )
    else:
        motorista_nome = "N/A"
        if viagem.motorista_formal:
...
def handle_join_trip_room(data):
    ...
    if viagem_id:
        ...
        logger.info(
            f"Cliente {request.sid} entrou na sala de consulta trip_{viagem_id}"
        )

        # Sala específica para o motorista da viagem
        join_room(f"driver_{viagem_id}")
        logger.info(
            f"Cliente {request.sid} entrou na sala do motorista driver_{viagem_id}"
        )
...
def get_cliente_details_api(cliente_id):
    ...
    # Query otimizada para buscar as viagens e já carregar os dados relacionados
    viagens = (
        Viagem.query.filter_by(
            cliente=cliente.nome_razao_social, empresa_id=current_user.empresa_id
        )
        .options(
            db.joinedload(Viagem.custo_viagem),
            db.joinedload(Viagem.abastecimentos),
            db.joinedload(Viagem.motorista_formal),  # Carrega o motorista
            db.joinedload(Viagem.veiculo),  # Carrega o veículo
        )
        .order_by(Viagem.data_inicio.desc())
        .all()
    )

    # --- Lógica de cálculo de estatísticas (permanece a mesma) ---
    total_receita = 0
    ...
    # --- Lógica do Histórico de Viagens (permanece a mesma) ---
    viagens_data = [
        {
            "id": v.id,
            "data_inicio": v.data_inicio.strftime("%d/%m/%Y"),
    ...
    # <<< NOVO BLOCO: Preparar dados para o Histórico de Atendimentos >>>
    atendimentos_data = []
    for v in viagens:
        atendimentos_data.append(
            {
                "data_viagem": v.data_inicio.strftime("%d/%m/%Y %H:%M"),
                "motorista_nome": (
                    v.motorista_formal.nome if v.motorista_formal else "N/A"
                ),
                "veiculo_placa": v.veiculo.placa if v.veiculo else "N/A",
                "veiculo_modelo": v.veiculo.modelo if v.veiculo else "N/A",
                "origem": v.endereco_saida,
                "destino": v.endereco_destino,
                "status": v.status.replace("_", " ").title(),
                "viagem_id": v.id,
    ...
    cliente_data = {"id": cliente.id, "nome_razao_social": cliente.nome_razao_social}
    ...
    return jsonify(
        {
            ...
            "atendimentos": atendimentos_data,  # <-- Adiciona o novo histórico à resposta da API
        }
    )
...
def visualizar_romaneio(romaneio_id):
    ...
    romaneio = (
        Romaneio.query.join(Viagem)
        .filter(
            Romaneio.id == romaneio_id, Viagem.empresa_id == current_user.empresa_id
        )
        .first_or_404()
    )
    ...
    cliente_obj = Cliente.query.filter_by(
        nome_razao_social=romaneio.viagem.cliente, empresa_id=current_user.empresa_id
    ).first()
...
def handle_atualizar_localizacao_socket(data):
    ...
    try:
        ...
        if (
            trip_last_geocode is None
            or (current_time - trip_last_geocode).total_seconds()
            > GEOCODE_INTERVAL_SECONDS
        ):
            should_geocode = True
        ...
        if should_geocode:
            ...
            last_geocode_time[viagem.id] = (
                current_time  # Atualiza o tempo do último geocode
            )
        ...
        if endereco:
            emit(
                ...
                {"endereco": endereco, "viagem_id": viagem.id},
                room=f"driver_{viagem.id}",
            )
...
def central_documentos():
    ...
    custos_com_anexos = (
        CustoViagem.query.join(Viagem)
        .filter(
            Viagem.empresa_id == current_user.empresa_id,
            CustoViagem.anexos.isnot(None),
            CustoViagem.anexos != "",
        )
        .options(db.joinedload(CustoViagem.viagem).joinedload(Viagem.veiculo))
        .all()
    )
    ...
    abastecimentos_com_anexos = (
        Abastecimento.query.join(Viagem)
        .filter(
            Viagem.empresa_id == current_user.empresa_id,
            Abastecimento.anexo_comprovante.isnot(None),
            Abastecimento.anexo_comprovante != "",
        )
        .options(db.joinedload(Abastecimento.viagem).joinedload(Viagem.veiculo))
        .all()
    )
    ...
    despesas_com_anexos = (
        DespesaVeiculo.query.join(Veiculo)
        .filter(
            Veiculo.empresa_id == current_user.empresa_id,
            DespesaVeiculo.anexos.isnot(None),
            DespesaVeiculo.anexos != "",
        )
        .options(db.joinedload(DespesaVeiculo.veiculo))
        .all()
    )
    ...
    if search_query:
        for doc in documentos_consolidados:
            if (
                search_query in doc["tipo"].lower()
                or search_query in doc["nome_entidade"].lower()
                or search_query in doc["nome_arquivo"].lower()
            ):
...
def relatorio_rentabilidade_veiculo():
    ...
    try:
        ...
        viagens_periodo = (
            ...
            .order_by(Viagem.data_inicio.asc())
            .all()
        )
                ...
                relatorio_custos_detalhado.append(
                    {
                        ...
                        "categoria": "Combustível",
                        "fornecedor_motorista": (
                            viagem.motorista_formal.nome
                            if viagem.motorista_formal
                            else "N/A"
                        ),
                        "documento": f"Viagem #{viagem.id}",
                        "descricao": f"{abast.litros:.2f}L @ R${abast.preco_por_litro:.2f}/L",
                        "valor": float(abast.custo_total or 0),
                        "viagem_obj": viagem,
                    }
            ...
            relatorio_custos_detalhado.append(
                {
                    ...
                    "fornecedor_motorista": rateio.lancamento.fornecedor_cliente
                    or "N/A",
        ...
        if motoristas_ids:
            ...
            folhas_pagas = (
                ...
                .options(db.joinedload(FolhaPagamento.motorista))
                .all()
            )  # Removido o joinedload para 'itens'
            # ===================== FIM DA CORREÇÃO FINAL ======================

            for folha in folhas_pagas:
                for item in folha.itens.filter(
                    ItemFolhaPagamento.tipo == "Provento",
                    ItemFolhaPagamento.viagem_id.isnot(None),
                ):
                    viagem_associada = db.session.get(Viagem, item.viagem_id)
                    if viagem_associada and viagem_associada.veiculo_id in veiculo_ids:
                        relatorio_custos_detalhado.append(
                            {
                                "data": folha.data_pagamento.date(),
                                "veiculo_obj": viagem_associada.veiculo,
                                "categoria": "Pagamento Pessoal",
                                "fornecedor_motorista": folha.motorista.nome,
                                "documento": f"Folha {folha.mes_referencia}/{folha.ano_referencia}",
                                "descricao": item.descricao,
                                "valor": float(item.valor),
                                "viagem_obj": viagem_associada,
                            }
                        )
        ...
        kpis = {
            "receita_total": receitas_total,
            "custo_total": custo_total,
            "pagamento_motoristas": pagamento_motoristas_total,
            "lucro_liquido": lucro_liquido,
            "custo_km": (custo_total / km_total) if km_total > 0 else 0.0,
            "receita_km": (receitas_total / km_total) if km_total > 0 else 0.0,
            "margem_lucro": (
                (lucro_liquido / receitas_total * 100) if receitas_total > 0 else 0.0
            ),
        }
        ...
        return render_template(
            ...
            kpis=kpis,
            receitas=viagens_com_receita,
            custos=relatorio_custos_detalhado,
            grafico_custos={
...
def api_detalhes_viagem_rentabilidade(viagem_id):
    ...
    try:
        ...
        custo_outros = viagem.custo_viagem.outros if viagem.custo_viagem else 0.0
        custo_total_viagem = (
            custo_combustivel + custo_pedagios + custo_alimentacao + custo_outros
        )
        ...
        distancia_a_exibir = (
            viagem.distancia_percorrida
            if viagem.distancia_percorrida and viagem.distancia_percorrida > 0
            else viagem.distancia_km
        )
        ...
        dados_viagem = {
            ...
            "destinos_intermediarios": [
                d.endereco for d in sorted(viagem.destinos, key=lambda x: x.ordem)[:-1]
            ],
            ...
            "veiculo": (
                f"{viagem.veiculo.placa} - {viagem.veiculo.modelo}"
                if viagem.veiculo
                else "N/A"
            ),
            "financeiro": {
        ...
        logger.error(
            f"Erro ao buscar detalhes da viagem {viagem_id}: {e}", exc_info=True
        )
        return jsonify({"success": False, "message": str(e)}), 500
...
def get_veiculo_despesas_consolidadas(veiculo_id):
        ...
        extrato_final.append(
            {
                ...
                "descricao": despesa.servicos_executados
                or despesa.descricao_problema
                or "N/A",
        ...
        extrato_final.append(
            {
                ...
                "valor": despesa.custo_total,
                "anexos": (
                    [despesa.anexo_comprovante] if despesa.anexo_comprovante else []
                ),
            }
        )
    ...
    viagens_do_veiculo = (
        Viagem.query.filter_by(veiculo_id=veiculo_id)
        .options(db.joinedload(Viagem.custo_viagem))
        .all()
    )
    for viagem in viagens_do_veiculo:
        if viagem.custo_viagem:
            ...
            if custo.pedagios and custo.pedagios > 0:
                extrato_final.append(
                    {
                        "data": viagem.data_inicio.isoformat(),
                        "categoria": "Pedágio",
                        "descricao": f"Ref. Viagem #{viagem.id}",
                        "valor": custo.pedagios,
                        "anexos": anexos_custo,
                    }
                )
            ...
            if custo.outros and custo.outros > 0:
                extrato_final.append(
                    {
                        "data": viagem.data_inicio.isoformat(),
                        "categoria": "Outras Desp. (Viagem)",
                        "descricao": custo.descricao_outros,
                        "valor": custo.outros,
                        "anexos": anexos_custo,
                    }
                )
...
def get_veiculo_details_api(veiculo_id):
    """
    API completa para buscar todos os dados necessários para o modal de detalhes do veículo.
    """
    veiculo = Veiculo.query.filter_by(
        id=veiculo_id, empresa_id=current_user.empresa_id
    ).first_or_404()
    ...
    historico_manutencoes = [
        {
            ...
            "custo": m.custo_total or 0.0,
        }
        for m in sorted(manutencoes, key=lambda x: x.data_entrada, reverse=True)
    ]

    # 3. Anexos
    anexos_data = veiculo.fotos_urls.split(",") if veiculo.fotos_urls else []
...
def lancar_receita_veiculo(veiculo_id):
    ...
    try:
        ...
        logger.error(
            f"Erro ao lançar receita para o veículo {veiculo_id}: {e}", exc_info=True
        )
...
def fluxo_caixa():
    ...
    # Coleta de todos os filtros da URL (lógica original mantida)
    ...
    # Query base para os lançamentos exibidos na tela (lógica original mantida)
    query = LancamentoFluxoCaixa.query.filter(
        LancamentoFluxoCaixa.empresa_id == current_user.empresa_id,
        LancamentoFluxoCaixa.data_vencimento.between(data_inicio_obj, data_fim_obj),
    )

    # Aplicação dos filtros (lógica original mantida)
    if data_emissao_inicio:
        query = query.filter(
            LancamentoFluxoCaixa.data_lancamento
            >= datetime.strptime(data_emissao_inicio, "%Y-%m-%d")
    ...
    # --- LÓGICA PARA TÍTULOS VENCIDOS ---
    # Query separada que busca TODOS os títulos vencidos, ignorando os filtros de data da tela principal.
    titulos_vencidos_query = (
        LancamentoFluxoCaixa.query.filter(
            LancamentoFluxoCaixa.empresa_id == current_user.empresa_id,
            LancamentoFluxoCaixa.status_pagamento.in_(
                ["PENDENTE", "A Pagar"]
            ),  # Garante que só pegue o que não foi pago
            LancamentoFluxoCaixa.data_vencimento < hoje,
        )
        .order_by(LancamentoFluxoCaixa.data_vencimento.asc())
        .all()
    )

    # Formata os dados para serem facilmente consumidos pelo JavaScript
    titulos_vencidos_json = []
    for lancamento in titulos_vencidos_query:
        dias_atraso = (hoje - lancamento.data_vencimento).days
        titulos_vencidos_json.append(
            {
                "id": f"manual_{lancamento.id}",
                "descricao": lancamento.descricao,
                "valor": lancamento.valor_total,
                "dias_atraso": dias_atraso,
                "data_vencimento_br": lancamento.data_vencimento.strftime(
                    "%d/%m/%Y"
                ),  # Formato para exibição no modal
                "data_vencimento_url": lancamento.data_vencimento.isoformat(),  # Formato AAAA-MM-DD para a URL
            }
        )
    # --- FIM DA LÓGICA DE VENCIDOS ---

    # Restante da sua lógica original (mantida intacta)
    ...
    categorias = sorted(
        set(
            [
                cat[0]
                for cat in db.session.query(LancamentoFluxoCaixa.categoria)
                .filter(
                    LancamentoFluxoCaixa.empresa_id == current_user.empresa_id,
    ...
    return render_template(
        ...
        titulos_vencidos_json=json.dumps(titulos_vencidos_json),
    )
        ...
        fluxo.append(
            {
                ...
                "observacoes": lancamento.observacoes,
                "is_vencido": (lancamento.data_vencimento < date.today())
                and lancamento.status_pagamento in ["PENDENTE", "A Pagar"],
                "tem_rateio": lancamento.tem_rateio,
                "objeto": lancamento,
            }
        )
...
def calcular_totais_fluxo(fluxo_consolidado):
    ...
    for item in fluxo_consolidado:
        ...
        if item["data_vencimento"] == hoje and item["status"] in [
            "PENDENTE",
            "A Pagar",
        ]:
            if item["tipo_movimento"] == "RECEITA":
    ...
    saldo_pendente = (total_receitas - total_receitas_pagas) - (
        total_despesas - total_despesas_pagas
    )
    ...
    return {
        ...
        "vencido_total_formatado": formatar_brl(
            abs(vencido_total)
        ),  # Mostra o valor absoluto dos vencidos
    }
        ...
        try:
            ...
            if parcela_total == 1:
                ...
                for i in range(1, parcela_total + 1):
                    data_parcela = data_vencimento + timedelta(
                        days=(i - 1) * 30
                    )  # Mensais por padrão
                    ...
                    parcela = LancamentoFluxoCaixa(
                        ...
                        fornecedor_cliente=fornecedor_cliente,
                        documento_numero=(
                            f"{documento_numero}-{i:02d}" if documento_numero else None
                        ),
                        observacoes=observacoes,
                        parcela_numero=i,
                        parcela_total=parcela_total,
            ...
            db.session.commit()
            flash(
                f"Lançamento criado com sucesso! {parcela_total} parcela(s) gerada(s).",
                "success",
            )
            return redirect(url_for("fluxo_caixa"))
...
def api_marcar_pago_fluxo():
    ...
    if not meio_pagamento:
        return (
            jsonify({"success": False, "message": "Meio de pagamento é obrigatório."}),
            400,
        )
...
def fix_old_trip_tokens():
    ...
    """
    try:
        viagens_sem_token = Viagem.query.filter(
            Viagem.public_tracking_token.is_(None)
        ).all()

        if not viagens_sem_token:
            return (
                "<h1>Tudo certo!</h1><p>Nenhuma viagem precisava de correção.</p>",
                200,
            )
        ...
        return (
            f"<h1>Correção Concluída!</h1><p>{count} viagem(ns) foram atualizadas com sucesso.</p>",
            200,
        )
...
def parse_nfe_xml_content(xml_content_str):
    ...
    try:
        ...
        if emit is None:
            return None  # Adiciona uma verificação de segurança
        ...
        if emit_cnpj_node is not None:
            ...
            logger.error(
                f"XML com chave {chave_acesso} não possui CNPJ ou CPF do emitente."
            )
            return None
        ...
        emit_nome = (
            emit_nome_node.text
            if emit_nome_node is not None
            else "Emitente Desconhecido"
        )
        # --- FIM DA CORREÇÃO ---

        ide = root.find(".//nfe:ide", ns)
        data_emissao_str = getattr(ide.find("nfe:dhEmi", ns), "text", None) or getattr(
            ide.find("nfe:dEmi", ns), "text", None
        )
...
def api_importar_xmls():
    ...
    for file in files:
        if file and file.filename.lower().endswith(".xml"):
            try:
                ...
                if NFeImportada.query.filter_by(
                    chave_acesso=chave, empresa_id=current_user.empresa_id
                ).first():
                ...
                notas_ja_existentes += 1
                logger.warning(
                    f"Chave duplicada encontrada ao tentar salvar: {file.filename}"
                )
                ...
                db.session.rollback()
                logger.error(
                    f"Erro genérico ao processar o arquivo {file.filename}: {e}",
                    exc_info=True,
                )
                erros_parse += 1
    ...
    if notas_ja_existentes > 0:
        msg_sucesso += (
            f" {notas_ja_existentes} nota(s) já existia(m) e foram ignorada(s)."
        )
    if erros_parse > 0:
        msg_sucesso += f" {erros_parse} arquivo(s) não puderam ser lidos."

    return jsonify(
        {"success": True, "message": msg_sucesso, "novas_notas": novas_notas_dict}
    )
...
def consultar_sefaz_command():
    ...
    logger.info(f"Horário de início: {datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info("=====================================================")
    ...
    with app.app_context():
        ...
        empresas_para_consultar = (
            ...
            .join(CertificadoDigital, Empresa.id == CertificadoDigital.empresa_id)
            .filter(CertificadoDigital.data_validade >= datetime.now(UTC).date())
            .distinct()
            .all()
        )
        ...
        if not empresas_para_consultar:
            logger.info(
                "Nenhuma empresa com certificados válidos encontrada. Encerrando tarefa."
            )
            return

        logger.info(
            f"Encontradas {len(empresas_para_consultar)} empresa(s) para consultar."
        )

        for empresa in empresas_para_consultar:
            logger.info(
                f"--- Consultando para a empresa: {empresa.razao_social} (ID: {empresa.id}) ---"
            )
            try:
                ...
                if resultado.get("success"):
                    logger.info(
                        f"Resultado para empresa {empresa.id}: {resultado.get('message', 'Sucesso.')}"
                    )
                else:
                    logger.error(
                        f"Falha na consulta para empresa {empresa.id}: {resultado.get('message', 'Erro desconhecido.')}"
                    )

            except Exception as e:
                logger.error(
                    f"ERRO CRÍTICO ao processar empresa {empresa.id}: {e}",
                    exc_info=True,
                )
                db.session.rollback()
